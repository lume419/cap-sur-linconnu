...
//     CE DERNIER CAS N'EXISTAIT PAS (20e audit du 21/09/2026). Un fil qui meurt à chaque chargement relançait sans
//     fin le cycle « démarrer, mourir, attendre le délai de garde » : défiler() rendait la main à « pas de fil et
//     délai de garde en cours », puis à « le fil chauffe », et le repli promis ici n'était JAMAIS atteint — le
//     visiteur n'obtenait rien du tout. Pire, un fil qui démarre mais n'annonce jamais ses polices bloquait la file
//     pour toujours, sans minuteur d'aucune sorte. Deux gardes ont été ajoutés : un délai d'ANNONCE des polices
//     (DEMARRAGE_MAX_MS), et un compte d'échecs de démarrage consécutifs (FIL_ECHECS_MAX) au-delà duquel la file
//     part en repli — puis reconsidère le fil après RECONSIDERATION_MS ;
...
// Délai laissé au fil pour ANNONCER ses polices chargées (20e audit du 21/09/2026). Il lit 24 polices, ~1,6 s ;
// quinze secondes est déjà dix fois la mesure. Sans ce délai, un fil qui démarre mais n'annonce jamais rien laissait
// la file bloquée pour toujours sur « le fil chauffe, la file l'attend » : aucun export, aucun repli, aucune trace.
const DEMARRAGE_MAX_MS = borneEnv('PDF_DEMARRAGE_MAX_MS', 15000, 50, 120000);
// Échecs de démarrage CONSÉCUTIFS au-delà desquels le fil est réputé hors service et la file part en repli dans le
// processus principal. Le repli est dégradé (il bloque la boucle d'événements le temps du calcul), mais il rend un
// document : sans ce compte, un fil qui meurt à chaque chargement relançait indéfiniment le cycle « démarrer, mourir,
// attendre le délai de garde » et le visiteur n'obtenait RIEN, jamais.
const FIL_ECHECS_MAX = borneEnv('PDF_FIL_ECHECS_MAX', 3, 1, 100);
// Après condamnation, le fil est reconsidéré une fois passé ce délai : une cause passagère (mémoire, descripteurs)
// ne doit pas condamner le fil jusqu'au redémarrage du serveur.
const RECONSIDERATION_MS = borneEnv('PDF_RECONSIDERATION_MS', 60000, 100, 3600000);
// Fil volontairement défaillant, POUR LES TESTS UNIQUEMENT : « mort » = un fil qui s'arrête aussitôt chargé,
// « muet » = un fil vivant qui n'annonce jamais ses polices. Deux chemins qu'aucun test ne pouvait atteindre
// autrement, et ce sont précisément ceux où le service restait muet (20e audit du 21/09/2026). Aucun chemin de
// fichier n'est accepté : la variable ne choisit qu'entre deux programmes d'une ligne écrits ici.
const FIL_CASSE = process.env.PDF_FIL_CASSE === 'mort' ? 'mort' : (process.env.PDF_FIL_CASSE === 'muet' ? 'muet' : null);
...
let dernierÉchec = 0;
let échecsDémarrage = 0;      // échecs CONSÉCUTIFS de démarrage du fil (remis à zéro dès qu'un fil annonce ses polices)
let minuteurDémarrage = null; // délai d'annonce des polices (voir DEMARRAGE_MAX_MS)
...
function démarrer(){
  ...
  const w = FIL_CASSE === 'mort' ? new Worker('process.exit(1);', { eval: true })
    : FIL_CASSE === 'muet' ? new Worker('setInterval(function(){}, 3600000);', { eval: true })
    : new Worker(FICHIER);
  fil = w;
  prêt = false;
  statut = 'démarrage';
  // Le fil doit annoncer ses polices ; s'il ne dit rien, il est perdu comme s'il était mort (20e audit).
  clearTimeout(minuteurDémarrage);
  minuteurDémarrage = setTimeout(function(){
    if(fil !== w || prêt) return;
    échecsDémarrage++;
    statut = 'démarrage sans réponse (' + échecsDémarrage + ' de suite)';
    console.warn('[pdf] fil sans annonce de polices après ' + DEMARRAGE_MAX_MS + ' ms, ' + échecsDémarrage + ' échec(s) de suite');
    fil = null; prêt = false; dernierÉchec = Date.now();
    w.terminate();
    perdreLeFil(w, new Error('fil sans annonce de polices'));
  }, DEMARRAGE_MAX_MS);
  if(minuteurDémarrage.unref) minuteurDémarrage.unref();
  démarrage = new Promise(function(resolve){
    w.on('message', function(msg){
      if(!msg) return;
      if(msg.type === 'ready'){
        clearTimeout(minuteurDémarrage); minuteurDémarrage = null;
        échecsDémarrage = 0;
        prêt = !msg.fontsError;
...
function perdreLeFil(w, err){
  // Perdu avant d'avoir annoncé ses polices : c'est un échec de DÉMARRAGE, et c'est ce compte qui décide, au bout de
  // FIL_ECHECS_MAX, de servir en repli plutôt que de laisser le visiteur sans réponse (20e audit du 21/09/2026).
  if(fil === w && !prêt){ échecsDémarrage++; if(échecsDémarrage >= FIL_ECHECS_MAX) statut = 'fil hors service (' + échecsDémarrage + ' démarrages manqués), mise en page dans le processus'; }
  if(fil === w){ clearTimeout(minuteurDémarrage); minuteurDémarrage = null; fil = null; prêt = false; dernierÉchec = Date.now(); }
  const j = enVol;
  if(!j){ défiler(); return; }
  enVol = null;
  clearTimeout(j.minuteur);
...
function défiler(){
  ...
  // Fil condamné (FIL_ECHECS_MAX démarrages manqués d'affilée) : on cesse de l'attendre et on sert en repli. Passé
  // RECONSIDERATION_MS sans nouvelle tentative, on lui redonne sa chance — une cause passagère ne doit pas condamner
  // le fil jusqu'au redémarrage du serveur (20e audit du 21/09/2026).
  if(échecsDémarrage >= FIL_ECHECS_MAX && Date.now() - dernierÉchec >= RECONSIDERATION_MS) échecsDémarrage = 0;
  const filHorsService = échecsDémarrage >= FIL_ECHECS_MAX;
  if(!fil && !filHorsService){ try { démarrer(); } catch(err){ console.warn('[pdf] fil impossible à démarrer :', kindOf(err)); échecsDémarrage++; dernierÉchec = Date.now(); } }
  ...
  // Le fil condamné, lui, ne se fait plus attendre : la file passe au repli, sans quoi le visiteur n'obtiendrait rien.
  if(!fil && !filHorsService && Date.now() - dernierÉchec < REDEMARRAGE_MS){ planifierReprise(); return; }
  const j = file.shift();
  occupé = true;
  if(fil && prêt){
    j.id = prochainId++;
...
function build(travail, abandonné){
  ...
  if(!fil && échecsDémarrage < FIL_ECHECS_MAX) démarrer();
...
function stop(){
  ...
  if(minuteurDémarrage){ clearTimeout(minuteurDémarrage); minuteurDémarrage = null; }
  arrêté = true;
  const w = fil;
...
function status(){
  return { statut: statut, prêt: prêt, fontsError: fontsError, enAttente: file.length, enCours: !!enVol,
    filHorsService: échecsDémarrage >= FIL_ECHECS_MAX, échecsDémarrage: échecsDémarrage };
}
