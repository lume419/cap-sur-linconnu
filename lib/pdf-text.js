...
function loadFonts(){
  ...
  fonts.forEach(function(f){ memoiseLayout(f.font); if(f.boldFont) memoiseLayout(f.boldFont); });
  return fonts;
}

// MISE EN FORME OPENTYPE MÉMORISÉE (17e audit du 20/09/2026). Profil processeur d'un export en dzongkha : 90 % du temps
// dans fontkit (applyChainingContext 31 %, coverageSequenceMatches 25 %, applyLookups 20 %), 3 % dans ce fichier. Un
// bloc de texte coûtait 3,6 ms en français contre 60 ms en dzongkha et 32 en bengali — d'où les quatre langues dont la
// mise en page s'arrêtait avant la fin sur un voyage maximal (budget PDF_BUILD_BUDGET_MS).
// pdfkit a bien un cache (layoutCache), mais layout() le COURT-CIRCUITE dès qu'on lui passe des fonctionnalités
// OpenType — ce que nous faisons toujours (PDF_FEATURES, indispensable : sans lui pdfkit met en forme mot par mot et
// recolle de gauche à droite, ce qui casse l'arabe et l'hébreu). Le cache est donc posé ici, sur les polices fontkit
// partagées par tous les documents : le même mot mis en forme deux fois ne l'est plus qu'une, dans un export comme
// d'un export à l'autre. C'est ce qui rend le gain intéressant sur les écritures indiennes et tibétaine, où
// breakLines mesure chaque syllabe séparément et où les mêmes syllabes reviennent sans cesse.
// COPIE OBLIGATOIRE : pdfkit MODIFIE le résultat (layoutRun multiplie chaque position par son échelle et y ajoute
// advanceWidth). Rendre deux fois le même objet le ferait mettre à l'échelle deux fois — texte aux espacements
// aberrants. Chaque appel reçoit donc une copie neuve des positions ; les glyphes, eux, ne sont jamais modifiés
// (pdfkit n'y lit que id, advanceWidth et codePoints) et restent partagés. Le prototype est conservé : advanceWidth et
// bbox sont des accesseurs qui recalculent depuis les positions, il faut qu'ils voient la copie.
const LAYOUT_CACHE_MAX = 6000;   // mises en forme gardées par police (une entrée = un mot ou une syllabe)
function cloneRun(run){
  const copy = Object.create(Object.getPrototypeOf(run));
  Object.assign(copy, run);
  copy.positions = run.positions.map(function(p){ return Object.assign(Object.create(Object.getPrototypeOf(p)), p); });
  return copy;
}
function memoiseLayout(font){
  if(font.__layoutMemo) return font;
  const cache = font.__layoutMemo = new Map();
  const orig = font.layout;
  font.layout = function(string, userFeatures, script, language, direction){
    if(typeof string !== 'string') return orig.call(this, string, userFeatures, script, language, direction);
    const key = string + '\u0000' + (userFeatures ? String(userFeatures) : '') + '\u0000' + (script || '') +
      '\u0000' + (language || '') + '\u0000' + (direction || '');
    let run = cache.get(key);
    if(run === undefined){
      run = orig.call(this, string, userFeatures, script, language, direction);
      if(cache.size >= LAYOUT_CACHE_MAX) cache.clear();
      cache.set(key, run);
    }
    return cloneRun(run);
  };
...
function trimGlyphCaches(maxGlyphs){
  ...
  fonts.forEach(function(f){
    [f.font, f.boldFont].forEach(function(font){
      // Les deux caches d'une même police sont LIÉS, et c'est pour ça qu'ils se vident ensemble (18e audit du
      // 21/09/2026) : une mise en forme mémorisée retient les objets Glyph qu'elle a utilisés, donc vider
      // font._glyphs ne libère rien tant que __layoutMemo les tient. Mesuré avant correction, 30 exports de
      // 1 200 idéogrammes neufs chacun : 195 Mo de tas (414 Mo de RSS) dont 54 Mo retenus par le seul cache de
      // mise en forme, qui à 605 entrées n'avait jamais atteint son propre seuil de 3 000.
      if(font && font._glyphs && Object.keys(font._glyphs).length > maxGlyphs){
        font._glyphs = {};
        if(font.__layoutMemo) font.__layoutMemo.clear();
        trimmed++;
      } else if(font && font.__layoutMemo && font.__layoutMemo.size > LAYOUT_CACHE_MAX){
        // Filet pour une police dont les glyphes ne débordent pas mais dont les mises en forme s'accumulent.
        font.__layoutMemo.clear();
      }
    });
  });
  return trimmed;
...
// Texte entièrement dessinable (13e audit du 19/09/2026) : chaque caractère, après la normalisation NFC que drawText
// applique, a un glyphe dans AU MOINS UNE des polices chargées. Sinon assignFonts retombe sur une police qui ne le
// contient pas et le lecteur affiche un carré vide (glyphe 0, .notdef) — « ⟲ » (U+27F2), badge du retour à l'écran,
// n'existe dans aucune police de pdf-fonts/. Sert aux libellés courts qu'il vaut mieux remplacer que rendre illisibles
// (badge des étapes, voir pdfClientBadge dans server.js).
function hasAnyGlyph(ch){
  const cp = String(ch).codePointAt(0);
  if(cp === undefined) return false;
  return loadFonts().some(f => hasGlyph(f, cp));
}
function canRender(text){
  if(typeof text !== 'string') return false;
  for(const ch of text.normalize('NFC')){ if(!hasAnyGlyph(ch)) return false; }
  return true;
}
function warmUp(){
  loadFonts();
...
// Points de code que les polices à variante GRASSE savent dessiner (une seule aujourd'hui, NotoSans : 2 965 points).
// Sert au processus principal pour répondre comme hasBoldVariant SANS charger les 140 Mo de polices : le fil de
// travail (lib/pdf-worker.js) les lui envoie une fois au démarrage (17e audit du 20/09/2026).
function boldCodePoints(){
  const out = new Set();
  loadFonts().forEach(function(f){ if(f.boldFont) (f.boldFont.characterSet || []).forEach(function(cp){ out.add(cp); }); });
  return Array.from(out);
}
module.exports = { registerFonts, drawText, textWidth, isRtlLang, loadFonts, warmUp, trimGlyphCaches, hasBoldVariant, hasAnyGlyph, canRender, boldCodePoints, _visualLines };
