'use strict';
// Texte multilingue pour les PDF (pdfkit) : polices Noto embarquées (dossier fonts/, licence SIL OFL 1.1), choix de la
// police CARACTÈRE PAR CARACTÈRE, ordre d'affichage bidirectionnel (arabe, persan, ourdou, kurde sorani, divehi, hébreu),
...
// Ordre = priorité de repli. `scripts` : écritures pour lesquelles la police est préférée (les autres polices ne servent
// qu'en repli, caractère par caractère).
const FONT_DEFS = [
...
const RTL_LANGS = new Set(['ar', 'fa', 'ur', 'ckb', 'dv', 'he', 'yi']);
...
function safeWidth(doc, font, str, size){
  useFont(doc, font).fontSize(size);
  try { return doc.widthOfString(str, { features: PDF_FEATURES }); } catch(e){ return str.length * size * 0.5; }
}
...
// 'right' | 'center' ; défaut selon la langue), lineGap, maxLines. Met doc.y sous la dernière ligne ; renvoie la hauteur.
...
function runText(para, r){
  ...
  if(rtl === fontkitReverses(str)) return str;
...
function textWidth(doc, str, opts){
  const text = String(str == null ? '' : str).normalize('NFC');
...
module.exports = { registerFonts, drawText, textWidth, isRtlLang, loadFonts, warmUp, _visualLines };
