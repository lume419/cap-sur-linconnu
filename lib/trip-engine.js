...
function buildRealRoute(startLat, startLon, startLandmass, startZone, maxRadiusKm, numStops, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, shortHop, depNorm){
  ...
  for(var i=0; i<numStops; i++){
    ...
    if(isLast){
      ...
      var strictPossible = !extendedFirst || legCapKm(0) >= minHop;
...
function finalizeLeg(distanceKm, speed, transportKey, tollEnabled, country, from, to){
  ...
  if(tollClass && distanceKm >= TOLL_MIN_DISTANCE_KM && from && to){
    ...
    // Pays facturables sur cette étape : ceux de ses deux extrémités (un port de ferry n'a pas de pays : celui du lieu
    // habité le plus proche). Un point n'est facturé qu'au barème de SON pays, et seulement si ce pays est l'un des
    // deux — sans cette seconde condition, un point de l'agglomération genevoise dont le voisin le plus proche est un
    // village français recevait le barème français sur un trajet entièrement suisse (8e audit du 18/09/2026). Un point
    // sans lieu habité autour (bras de mer longé, désert) prend le pays de l'étape quand elle n'en traverse qu'un seul.
    var paysEtape = {};
    ...
    var paysDuPoint = function(lat, lon){
      ...
      if(c) return paysEtape[c] ? c : null;
      return listePays.length === 1 ? listePays[0] : null;
    };
    var parts = tollGrid.tolledParts(from.lat, from.lon, to.lat, to.lon, distanceKm * TOLL_ROAD_FACTOR / ROAD_FACTOR, paysDuPoint);
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var candidates = (!extended0 || legCapKm(0) >= minHop0) ? search0() : [];
