...
function restrictionsForLeg(transportKey, from, to, seenCountries){
  ...
  if(transportKey === 'van'){
    ...
    [from && from.country, to.country].concat(to.countriesCrossed || []).forEach(function(cc){
...
function finalizeFerryLeg(transportKey, route, parts, speed, tollEnabled, fromPoint, toPoint){
  ...
  pieces.forEach(function(p){
    ...
    if(p.tollInfo){
      toll = toll ? Object.assign({}, toll, { amount: Math.round((toll.amount + p.tollInfo.amount) * 10) / 10,
        amountMax: Math.round(((toll.amountMax || 0) + (p.tollInfo.amountMax || 0)) * 10) / 10, // fourchette : bornes additionnées
        amountMin: Math.round(((toll.amountMin || 0) + (p.tollInfo.amountMin || 0)) * 10) / 10,
  ...
  if(transportKey !== 'velo' && parts.fromPort){
    var portPt = function(p){ return { lat: p.lat, lon: p.lon, country: countryAtPoint(p.lat, p.lon) }; };
    var crossed = countriesAlong(fromPoint, portPt(parts.fromPort));
    countriesAlong(portPt(parts.toPort), toPoint).forEach(function(cc){ if(crossed.indexOf(cc) < 0) crossed.push(cc); });
...
// pays garde la vitesse du mode (facteur 1, comme un pays sans relevé).
...
function pointSpeedFactor(p){
  ...
  var lm = typeof p.landmass === 'string' ? p.landmass : landmassOf(p);
  return (lm && SPEED_LANDMASSES[cc] && SPEED_LANDMASSES[cc][lm]) ? COUNTRY_SPEED[cc] : 1;
}
...
function finalizeLeg(distanceKm, speed, transportKey, tollEnabled, country, from, to){
  ...
  if(transportKey === 'moto'){
    ...
    // Pays seulement TRAVERSÉ (12e audit du 19/09/2026) : Kota Bharu → Bukit Kayu Hitam traverse la Thaïlande, Nanning →
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var dayDrive0 = function(x){ return 2 * x.distKm / (speed * countrySpeedFactor(cityCoord, x.commune, x.commune.country)) <= DAY_TRIP_MAX_HOURS; };
    var dayCapKm = Math.floor(DAY_TRIP_MAX_HOURS / 2 * speed * countrySpeedFactor(cityCoord, null, cityCoord.country));
    ...
    if(candidates.length===0){
      ...
      // la distance maximale par la route (même message que pour un séjour, voir minDistanceUnreachable).
      if(minDist > 0 && minDist > dayCapKm){ LAST_TRIP_DIAGNOSTIC = { minDistanceUnreachable: true, returnCapKm: dayCapKm }; return []; }
    ...
    if(!picked0){
      if(minDist > 0 && !tripTimeUp()) LAST_TRIP_DIAGNOSTIC = minDist > dayCapKm ? { minDistanceUnreachable: true, returnCapKm: dayCapKm } : { minDistanceNotFound: true };
      return [];
    }
...
function generateTrip(params){
  ...
  for(var retry = 0; legs.length === 0 && retry < MIN_DIST_RETRIES && LAST_TRIP_DIAGNOSTIC && LAST_TRIP_DIAGNOSTIC.minDistanceNotFound &&
      !TRIP_TIMED_OUT && Date.now() < deadline; retry++){
