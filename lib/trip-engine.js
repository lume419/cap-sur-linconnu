...
function searchCommunes(query, limit, preferCountry, lang){
  ...
  function pushMatch(c, cp, alias){
    ...
    // La liste par clé n'est PAS minuscule : « Xincun » avec le code CN-30 en compte 512 (le commentaire disait « un à
    // trois lieux », mesuré faux le 23/09/2026). Une saisie de trois caractères comme « xin » calculait 246 855
    // distances, soit 28,7 ms de calcul synchrone par frappe contre 10,4 ms avant le regroupement par distance.
    // D'où le test d'enveloppe ci-dessous : au-delà de FUSION_MAX_KM en latitude seule, les deux points sont
    // forcément trop loin, et un écart de latitude coûte une soustraction là où la formule de haversine coûte quatre
    // fonctions trigonométriques. Le résultat est identique — seuls les couples réellement proches sont mesurés.
    var key = c.country + '|' + c.norm + '|' + (cp || '');
    var groupe = seenKeys[key];
    if(!groupe) groupe = seenKeys[key] = [];
    var prev = null;
    var dLatMax = FUSION_MAX_KM / 111.32; // 1° de latitude = 111,32 km partout
    for(var gi = 0; gi < groupe.length; gi++){
      var g = groupe[gi];
      if(c.lat - g.lat > dLatMax || g.lat - c.lat > dLatMax) continue;
      if(haversineKm(c.lat, c.lon, g.lat, g.lon) <= FUSION_MAX_KM){ prev = g; break; }
    }
    if(prev){
      ...
      if(c.pop <= prev.pop) return;
      // Lieu plus peuplé au même endroit : il remplace le précédent, à sa place dans la liste. Les noms alternatifs
      // déjà proposés étaient ceux de l'AUTRE fiche : ils ne le suivent pas.
      var n = {name:c.name, cp:cp, allCps:c.cps, pop:c.pop, lat:c.lat, lon:c.lon, dept:c.dept, country:c.country, commune:c.commune || null, _placeNorm:c.norm};
      if(alias) SI.offerAlias(n, alias.text, alias.norm, alias.lang, rawQuery, q, lang);
      matches[matches.indexOf(prev)] = n;
      groupe[groupe.indexOf(prev)] = n;
      return;
    }
    var m = {name:c.name, cp:cp, allCps:c.cps, pop:c.pop, lat:c.lat, lon:c.lon, dept:c.dept, country:c.country, commune:c.commune || null, _placeNorm:c.norm};
  ...
  if(!matches.length && rawQuery !== q && rawQuery.indexOf('-') >= 0 && rawQuery.length >= 3){
