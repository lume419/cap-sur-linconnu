...
const { COUNTRIES, TRANSPORT, FERRY_ROUTES, SEA_CROSSINGS, TOLL_RATE_BY_COUNTRY, TOLL_LANDMASSES,
  ...
  GR_POROS_MAINLAND_NAMES, VAN_RULES, MOTO_RULES, LODGING_RULES, lodgingPriceCap } = TripData;
    ...
    (VAN_RULES || []).forEach(function(r){
      ...
      if(r.type === 'lez' || r.type === 'ztl'){
        hit = haversineKm(to.lat, to.lon, near.lat, near.lon) <= near.km ||
          (to.country === r.country && !!to.norm && to.norm === zoneHostNorm(r) && haversineKm(to.lat, to.lon, near.lat, near.lon) <= ZONE_HOST_MAX_KM);
      }
...
function buildCommuneGrid(){
  ...
  NEIGHBOUR_PAIRS = null; // recalculé à la demande (neighbourCountries)
  CELLS_BY_COUNTRY = {};
  BORDER_REACH_CACHE = {};
  COMMUNES.forEach(function(c){
...
// Pays voisins : une même case de la grille des lieux (0,2°, ~22 km) contient des lieux des deux. Calculé une fois.
var NEIGHBOUR_PAIRS = null;
function neighbourCountries(a, b){
  if(!COUNTRIES_BY_CELL) return false;
  if(!NEIGHBOUR_PAIRS){
    NEIGHBOUR_PAIRS = new Set();
    COUNTRIES_BY_CELL.forEach(function(set){
      var list = Object.keys(set);
      for(var i = 0; i < list.length; i++) for(var j = i + 1; j < list.length; j++){
        NEIGHBOUR_PAIRS.add(list[i] + '|' + list[j]); NEIGHBOUR_PAIRS.add(list[j] + '|' + list[i]);
      }
    });
  }
  return NEIGHBOUR_PAIRS.has(a + '|' + b);
}
...
function finalizeLeg(distanceKm, speed, transportKey, tollEnabled, country, from, to){
  ...
  if(tollClass && from && to){
    ...
    // Départ et arrivée dans deux pays VOISINS : pas de transit par un pays tiers (10e audit, trouvé par la suite de
    // tests) — Saarbrücken → Bâle suit l'A5 allemande, pas l'Alsace (15,9 € « barème français »). Luxembourg → Genève
    // (pays non voisins) traverse bien la France.
    var sansTransit = listePays.length === 2 && neighbourCountries(listePays[0], listePays[1]);
    var parts = tollGrid.tolledParts(from.lat, from.lon, to.lat, to.lon, distanceKm * TOLL_ROAD_FACTOR / ROAD_FACTOR, { duPoint: paysDuPoint, extremites: paysEtape, interieur: insideCountry, sansTransit: sansTransit });
    var amount = 0, countries = [], rate = 0, tolledKm = 0;
    parts.forEach(function(part){
      ...
      tolledKm += part.km;
      rate = r;
      if(countries.indexOf(part.country) < 0) countries.push(part.country);
    });
    if(countries.length && amount > 0){
      // countries : pays dont le barème s'applique (libellé « barème … » à l'affichage, voir TOLL_SOURCE).
      // tolledKm : kilomètres effectivement facturés, toujours inférieurs ou égaux à la distance de l'étape.
      tollInfo = { enabled: !!tollEnabled, amount: Math.round(amount * 10) / 10, tollClass: tollClass, rate: rate,
        countries: countries, tolledKm: Math.round(tolledKm) };
    }
  }
  var chargeInfo = null;
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var evPick0 = function(x){
      return legAllowed(fromStart0, x.commune, cap0) &&
        legAllowed({ lat: x.commune.lat, lon: x.commune.lon, landmass: communeLandmass(x.commune), zone: zoneOf(x.commune) }, fromStart0, cap0);
    };
    ...
    var search0 = function(){
      var found = filterCandidates(findNearbyCommunes(cLat, cLon, minHop0, Math.min(hop, searchCap0), 15), preds0);
      if(found.length===0 && !tripTimeUp()){
        found = filterCandidates(findNearbyCommunes(cLat, cLon, minHop0, Math.min(hop*1.6, searchCap0), 0), preds0);
      }
      return found;
    };
