...
// Séparateurs ramenés à une espace avant comparaison. La liste ne comptait que le trait d'union, l'apostrophe droite
// et l'apostrophe courbe : tout le reste de la famille restait tel quel, et un nom qui l'emploie ne se trouvait qu'en
// tapant EXACTEMENT le même caractère (18e audit du 21/09/2026 ; CHIFFRES CORRIGÉS au 19e). Mesure sur les données
// publiées : 40 571 lieux et 1 133 alias portent une apostrophe modificative — « Chervonyy Donets‘ » (UA),
// « Jaganʻ » (PL) —, 147 un accent grave (« T`lminci », MK), 77 un tiret demi-cadratin (« Nago–Torbole », IT), 17 un
// accent aigu. Aucun clavier ne produit ces caractères à la place de l'apostrophe ou du trait d'union ordinaires.
// Conséquence mesurée : 475 groupes de lieux d'un même pays, soit 3 351 lieux, portent le MÊME nom écrit avec deux
// graphies différentes sans être réunis — au Yémen, 233 groupes (1 472 lieux), tous deux graphies d'un seul nom
// (« Shay`ān » et « Shay‘ān », « Ar Rubū` » et « Ar Rubū‘ ») : chercher l'une ne rendait jamais les lieux écrits avec
// l'autre. Le 18e audit annonçait « 14 335 lieux » : ce chiffre sommait, pour chaque forme normalisée, l'écart entre
// l'ancien et le nouveau nombre de lieux, en comptant au passage des groupes dont tous les membres s'écrivent
// pourtant à l'identique et n'ont fait que changer de clé. Le bénéfice réel est 4,3 fois plus modeste.
const SEP_RE = /[-'\u0060\u00B4\u02B9\u02BB\u02BC\u02BE\u02BF\u02C8\u0384\u05F3\u2018\u2019\u201B\u2010-\u2015\uFF0D]/g;
// Caractères RETIRÉS (et non remplacés par une espace) : ils se placent à l'intérieur d'un mot, une espace le couperait.
// Point médian du « l·l » catalan (« Compostel·la » se tape « Compostella ») et du basque « Ain·harp » (« Ainharp ») ;
// harakat arabes et tatweel, qui ne s'écrivent pas dans une saisie ordinaire — 17 004 alias en portent (« مَاَلقَة » pour
// Malaga) — 17 689 exactement, recompté au 19e audit, le 18e annonçant 17 004 —, introuvables jusqu'ici en tapant
// la forme non vocalisée, la seule que l'on tape en pratique.
const NORM_DROP_RE = /[\u00B7\u2027\u0640\u064B-\u0652\u0670]/g;
...
// LIGATURES d\u00E9pli\u00E9es avant comparaison (23/09/2026). Ce sont des caract\u00E8res qui valent DEUX lettres : \u00AB \u0153 \u00BB vaut
// \u00AB oe \u00BB, \u00AB \u00DF \u00BB vaut \u00AB ss \u00BB. La d\u00E9composition NFD ne les touche pas \u2014 elle s\u00E9pare une lettre de son accent, pas une
// ligature de ses composantes \u2014, si bien qu'un nom qui en porte une ne se trouvait qu'en tapant EXACTEMENT ce
// caract\u00E8re, qu'aucun clavier fran\u00E7ais ou anglais ne produit simplement.
// Mesur\u00E9 sur les donn\u00E9es publi\u00E9es : 5 584 lieux et 2 209 alias en portent une \u2014 \u00DF 4 857 (allemand et autrichien :
// \u00AB Gro\u00DFe \u00BB, \u00AB Stra\u00DF \u00BB), \u00E6 1 813 (nordique : \u00AB \u00C6\u00F0uv\u00EDk \u00BB), \u0153 1 068 (fran\u00E7ais : \u00AB Bel\u0153il \u00BB), \u00C6 50, \u0152 11, \u0133 1.
// Sur 206 de ces lieux tir\u00E9s au hasard, **11 seulement (5 %) se retrouvaient en tapant la forme d\u00E9pli\u00E9e** \u2014 les
// autres, soit environ 5 300 lieux, n'\u00E9taient atteignables qu'au caract\u00E8re pr\u00E8s. Le d\u00E9pliage r\u00E9unit en outre
// 69 groupes de noms (140 graphies) qui coexistaient sans \u00EAtre reconnus comme le m\u00EAme nom : \u00AB Gr\u00F6\u00DFing \u00BB et
// \u00AB Gr\u00F6ssing \u00BB, \u00AB Nu\u00DFberg \u00BB et \u00AB Nussberg \u00BB.
// Les lettres \u00E0 barre ou \u00E0 panse (\u00F8, \u00F0, \u00FE, \u0111, \u0142) ne sont PAS ici : ce sont des lettres \u00E0 part enti\u00E8re, pas des
// ligatures, et les d\u00E9plier demanderait un choix par langue (\u00FE vaut \u00AB th \u00BB en islandais, \u00F0 \u00AB d \u00BB ou \u00AB dh \u00BB).
const LIGATURES = { '\u0153': 'oe', '\u00E6': 'ae', '\u0133': 'ij', '\u00DF': 'ss', '\uFB00': 'ff', '\uFB01': 'fi', '\uFB02': 'fl', '\uFB03': 'ffi', '\uFB04': 'ffl', '\uFB05': 'st', '\uFB06': 'st' };
const LIGATURES_RE = /[\u0153\u00E6\u0133\u00DF\uFB00\uFB01\uFB02\uFB03\uFB04\uFB05\uFB06]/g;
...
const { COUNTRIES, TRANSPORT, FERRY_ROUTES, SEA_CROSSINGS, TOLL_RATE_BY_COUNTRY, TOLL_LANDMASSES,
  ...
  SICILY_PROVINCES, HR_POSTCODE_TO_ISLAND, CV_CONCELHO_TO_ISLAND, CV_ISLAND_BOXES, ISLAND_BOXES, ISLAND_ONLY_COUNTRIES, NO_TRIP_LANDMASSES, TENSION_ZONES, ISLAND_RULES, GR_ISLAND_PATTERNS,
...
function landmassOf(c){
  ...
  // d'Islande sur 15 tirages depuis Reykjavík).
  // Ce commentaire disait, jusqu'au 22/09/2026, que « les îles habitées au large (Vestmannaeyjar, Grímsey, Hrísey)
  // ne sont pas distinguées, faute de découpage dans les données : limite assumée ». C'ÉTAIT FAUX : elles le sont,
  // par les règles génériques ISLAND_RULES.IS lues plus haut (codes postaux 900 et 611, et un point nommé pour
  // Hrísey), et leurs trois liaisons de ferry existent avec leur source — Herjólfur pour Vestmannaeyjar (grille 2026),
  // Sæfari pour Grímsey. Vérifié : landmassOf rend « vestmannaeyjar », « grimsey » et « hrisey » pour ces trois-là,
  // et « iceland » pour Reykjavík. Le commentaire datait d'avant l'ajout de ces règles et n'avait pas été relu.
  ...
  // (Avşa/Marmara/Paşalimanı/Ekinlik, réseau GESTAŞ depuis Erdek) ne l'étaient PAS ici à l'origine — elles le sont
  // depuis, par les règles génériques ISLAND_RULES.TR lues plus haut, avec la grille tarifaire GESTAŞ 2026 de la
  // ligne Erdek–Avşa–Marmara–Paşalimanı–Ekinlik. Vérifié le 22/09/2026 : landmassOf rend « avsa », « marmaraIsland »,
  // « ekinlik » et « pasalimani ». Le paragraphe qui les disait non modélisées est corrigé ici. Anomalie GeoNames connue et non corrigée ici (hors
  ...
  if(c.country === 'CV'){
    ...
    // Concelho absent : 22 lieux publiés ne portent aucun code (leur champ vaut « CV », le code pays) et
    // retombaient tous ici, coupés de leur île alors qu'ils sont à moins de 2 km d'un lieu déjà classé. Repli
    // par boîte de coordonnées MESURÉE sur les lieux que le code classe (CV_ISLAND_BOXES dans trip-data.js) :
    // les neuf boîtes sont disjointes, donc le rattachement n'est pas un pari. Il ne s'applique QUE lorsque le
    // code manque — un code présent fait toujours foi, même si les coordonnées le contredisent.
    for(var cvKey in CV_ISLAND_BOXES){
      if(!Object.prototype.hasOwnProperty.call(CV_ISLAND_BOXES, cvKey)) continue;
      var cvB = CV_ISLAND_BOXES[cvKey];
      if(c.lat >= cvB[0] && c.lat <= cvB[1] && c.lon >= cvB[2] && c.lon <= cvB[3]) return cvKey;
    }
    // Hors de toute boîte : île indéterminée, on isole plutôt que de rattacher au hasard.
...
function finalizeFerryInfo(route, ferryClass){
  // LIAISON SANS VÉHICULES (23/09/2026). Un prix de classe absent veut dire « tarif non publié », JAMAIS « véhicule
  // refusé » : toute liaison était donc proposée à tous les modes. Or certaines n'embarquent que des PIÉTONS — le
  // Sævar d'Árskógssandur à Hrísey (Islande) en est une : l'île est sans voitures et la grille ne comporte aucun
  // tarif véhicule. La faire traverser par une voiture aurait publié une donnée fausse ; l'interdire aux modes
  // motorisés aurait retiré l'île de la couverture mondiale, alors qu'on la visite très bien en laissant sa voiture
  // au port — c'est ce que font les visiteurs. Le trajet est donc proposé à tous, mais TOUJOURS au tarif piéton, et
  // signalé comme tel : le véhicule reste au port, il ne monte pas à bord.
  if(route.passengerOnly) ferryClass = 'foot';
  ...
  // Signalé au client : le véhicule reste au port, seul le passager embarque (voir plus haut). « fareClass » était lu
  // par public/js/app.js (ferryFootFare) sans que rien ne le pose jamais : il sert enfin, et l'interface affiche le
  // tarif « par personne » — déjà traduit dans les 161 langues — au lieu d'un prix de véhicule qui n'existe pas.
  if(route.passengerOnly){ info.passengerOnly = true; info.fareClass = 'foot'; }
  return info;
}
...
// Classes de distance des vitesses médianes. LIMITE CHIFFRÉE (18e audit du 21/09/2026) : sur les 699 liaisons de
// FERRY_ROUTES, 160 seulement portent une durée PUBLIÉE (les autres sont déjà des estimations, ou du train-auto) et
// se répartissent très inégalement — 41 sous 15 km (médiane 12,0 km/h), 49 de 15 à 45 km (19,0), 28 de 45 à 100 km
// (20,7), 35 de 100 à 300 km (26,7) et SEPT au-delà de 300 km (27,1). La dernière classe, la plus longue, est donc la
// moins étayée : ses sept lignes vont de 17,2 km/h (Maurice ↔ Rodrigues, 620 km en 36 h) à 36,0 (Cadix ↔ Tenerife,
// 1 295 km en 36 h), et sa médiane sert à estimer 26 des 33 liaisons de 300 km et plus. Sur une traversée de 1 000 km,
// l'écart entre la plus lente et la plus rapide de ces sept lignes vaut 30 heures (58,1 h à 17,2 km/h contre 27,8 h
// à 36,0 ; le 18e audit annonçait 21 h) : la durée annoncée pour une
// traversée estimée de cette longueur est un ordre de grandeur, et elle est toujours marquée durationEstimated.
// Le repli sur la médiane générale ne se déclenche qu'en dessous de CINQ lignes dans la classe ; tests/data.test.js
// surveille ce seuil, pour qu'une classe ne bascule pas en silence sur la médiane de toutes les distances.
...
function normalizeCityName(s){
  s = String(s||'');
  // Chemin rapide ASCII : NORM_DROP_RE ne contient aucun caractère ASCII, seul SEP_RE s'y applique (l'accent grave,
  // lui, est bien ASCII et fait partie de SEP_RE).
  if(ASCII_RE.test(s)) return s.trim().toLowerCase().replace(SEP_RE,' ').replace(/\s+/g,' ').trim();
  // Ligatures dépliées AVANT la décomposition : « ﬁ » donne « fi », que NFD n'aura plus rien à faire, et « ß » donne
  // « ss » quelle que soit sa casse d'origine, la minuscule ayant déjà été appliquée.
  return s.trim().toLowerCase()
    .replace(LIGATURES_RE, function(c){ return LIGATURES[c]; })
    .normalize('NFD').replace(DIACRITICS_RE,'').replace(INVISIBLE_RE,'').replace(NORM_DROP_RE,'')
...
function parseCommunesFile(raw, country){
  return raw.split('\n').filter(Boolean).map(function(line){
    ...
    var dept = parts[3];
    var name = parts[4];
    // 6e champ FACULTATIF (23/09/2026) : la commune de rattachement d'un lieu-dit (« Le Marchais Vert » -> Beauchêne).
    // Absent partout ailleurs — un nom de lieu ne contient jamais de « ; » (vérifié sur les 239 fichiers de pays),
    // donc une ligne à cinq champs se lit exactement comme avant.
    var commune = parts[5] || null;
    return { name:name, norm:normalizeCityName(name), cps:cps, pop:pop, lat:lat, lon:lon, dept:dept, country:country, commune:commune };
  });
}
...
// Deux fiches de même nom et de même code postal à moins de FUSION_MAX_KM l'une de l'autre sont le même lieu (fiche
// en double, coordonnée affinée) : une seule suggestion. Au-delà, ce sont deux villes : deux suggestions. 10 km est
// large devant le dédoublonnage des générateurs (0,01°, ~1 km) et étroit devant les vrais homonymes mesurés, dont la
// moitié sont à plus de 100 km (22/09/2026). Partagé avec l'index sur disque (lib/search-index.js).
var FUSION_MAX_KM = 10;
...
function buildSearchIndex(){
  ...
  COMMUNES.forEach(function(c){
    ...
    // Code postal FACULTATIF (21/09/2026) : un lieu peut n'en avoir aucun — le code est une aide à la recherche,
    // pas une condition d'existence. Un code vide n'est pas indexé (il ne correspondrait à aucune saisie) ; le lieu,
    // lui, reste trouvable par son nom et par ses noms alternatifs comme tous les autres.
...
function searchCommunes(query, limit, preferCountry, lang){
  ...
  // CLÉ « pays|nom normalisé|code postal » : deux lieux DIFFÉRENTS d'un même pays qui la partagent n'en font qu'un
  // dans les suggestions. C'est voulu — sans elle, un même lieu trouvé à la fois par son nom, son code et un nom
  // alternatif apparaîtrait trois fois — mais ce n'est pas gratuit : mesuré sur les données publiées,
  // 634 832 lieux sur 4 801 562 (13,2 %) sont ainsi masqués par un homonyme de même code postal, et ne peuvent pas
  // être choisis comme départ. La part explose dans les pays SANS codes postaux publiés, où la colonne porte une
  // étiquette de région et où la clé se réduit donc à « pays|nom|région » : Chine 37,7 %, Népal 28,5 %,
  // Colombie 27,7 %, Brésil 25,5 %, Yémen 23,0 %… contre 3,5 % en Inde, qui a de vrais codes. 109 groupes opposent
  // deux villes de plus de 10 000 habitants (Ganta au Liberia : 13 802 et 63 523 habitants, 50 km d'écart).
  // Lever la limite demanderait une suggestion qui DÉSIGNE le lieu au lieu de son couple (nom, code postal) : rien
  // dans la ligne affichée (drapeau, nom, code postal) ne distinguerait aujourd'hui les deux Ganta, et les afficher
  // tous les deux remplirait la liste de lignes identiques — en Chine, jusqu'à 138 pour un même nom. Limite écrite,
  // mesurée, non corrigée.
  // CE QUI EST CORRIGÉ AU 20e AUDIT (21/09/2026) : LEQUEL des deux survit. L'index sur disque trie ses candidats par
  // population décroissante avant de dédoublonner, donc il garde le plus peuplé ; la recherche EN MÉMOIRE — celle
  // qui sert tant que l'index n'est pas construit, donc sur tout déploiement neuf — gardait le PREMIER rencontré
  // dans l'ordre du balayage, c'est-à-dire l'ordre des fichiers. Les deux chemins de recherche ne rendaient pas le
  // même lieu pour la même saisie. Ils gardent désormais tous deux le plus peuplé.
  function pushMatch(c, cp, alias){
    // Deux lieux d'un même pays qui portent le même nom normalisé ET le même code postal ne sont fusionnés que s'ils
    // sont AU MÊME ENDROIT (22/09/2026, demande de l'utilisateur). Auparavant la clé « pays|nom|code » les fusionnait
    // quelle que soit la distance : 635 715 lieux (13 %) étaient introuvables, dont Robīt (Éthiopie, 20 679 habitants)
    // masqué par un homonyme à 227 km, Mafinga (Tanzanie, 34 958) à 119 km, Al Jubayl (Arabie saoudite) à 180 km.
    // Mesuré : seuls 10,3 % des lieux masqués étaient à moins de 10 km de celui qui survivait — les 89,7 % autres
    // sont d'autres villes, dont la moitié à plus de 100 km. Le regroupement se fait donc par DISTANCE : au-delà de
    // FUSION_MAX_KM, ce sont deux villes, et les deux sont proposées. En deçà, c'est le même lieu vu deux fois (fiche
    // en double, coordonnée affinée) et le plus peuplé l'emporte, comme avant.
    // Le balayage ne décide RIEN : il empile les candidats, et regrouper() les réduit une fois tous connus.
    // C'est ce qui rend ce chemin identique à l'index sur disque (23/09/2026). Avant, chacun regroupait à sa façon :
    // le disque parcourt ses candidats par population DÉCROISSANTE et ne déplace jamais l'ancre d'un groupe ; la
    // mémoire les prenait dans l'ordre des fichiers ET remplaçait l'ancre dès qu'un lieu plus peuplé tombait à moins
    // de 10 km — ce qui déplaçait le point de référence en cours de route. Sur un groupe de trois fiches ou plus aux
    // distances non transitives (A–B ≤ 10, B–C ≤ 10, A–C > 10), le nombre de suggestions dépendait donc de l'ordre :
    // « Cuitaca » (Mexique) rendait UN résultat en mémoire et DEUX sur disque, c'est-à-dire un résultat différent
    // selon que l'index était construit ou non. Mesuré avant correction : 9 saisies divergentes sur 4 487.
    candidats.push({ c: c, cp: cp || '', alias: alias || null, ordre: ordreScan++ });
  }
  // Réduction des candidats en suggestions, avec EXACTEMENT l'algorithme de lib/search-index.js (voir collect) :
  // par clé « pays|nom|code », on trie par population décroissante — l'ordre de balayage ne sert que de départage
  // stable — puis on regroupe au plus proche, sans jamais déplacer l'ancre. Le tri portant sur la DONNÉE et non sur
  // l'ordre d'arrivée, le résultat ne dépend plus du chemin emprunté.
  function regrouper(){
    // Sélection bornée, comme l'index sur disque : une saisie de trois caractères ramène des dizaines de milliers de
    // candidats (« san »), et les trier tous coûtait 1,35 s de calcul synchrone PAR FRAPPE. On ne retient que les
    // TOP meilleurs par population, par un tas — O(n log TOP) au lieu de O(n log n) — et on ne repasse sur la liste
    // entière que si le regroupement n'a pas rendu assez de suggestions. Même repli, mêmes bornes que le disque.
    var TOP = Math.max(64, limit * 8);
    var res = reduire(candidats.length > TOP * 4 ? SI.topK(candidats, TOP, ordreCandidat) : candidats);
    if(res.length < limit && candidats.length > TOP * 4) res = reduire(candidats);
    return res;
  }
  function ordreCandidat(a, b){ return (b.c.pop - a.c.pop) || (a.ordre - b.ordre); }
  function reduire(candidats){
    var parCle = {}, ordreCles = [];
    candidats.forEach(function(k){
      var cle = k.c.country + '|' + k.c.norm + '|' + k.cp;
      if(!Object.prototype.hasOwnProperty.call(parCle, cle)){ parCle[cle] = []; ordreCles.push(cle); }
      parCle[cle].push(k);
    });
    var out = [];
    // 1° de latitude = 111,32 km partout : un écart de latitude supérieur suffit à écarter la paire, et il coûte une
    // soustraction là où la formule de haversine coûte quatre fonctions trigonométriques. Un groupe peut compter 512
    // fiches (« Xincun », code CN-30), et une saisie de trois caractères en calculait 246 855.
    var dLatMax = FUSION_MAX_KM / 111.32;
    ordreCles.forEach(function(cle){
      var groupe = parCle[cle].slice().sort(ordreCandidat);
      var ancres = [];
      groupe.forEach(function(k){
        var prev = null;
        for(var i = 0; i < ancres.length; i++){
          var g = ancres[i];
          if(k.c.lat - g.c.lat > dLatMax || g.c.lat - k.c.lat > dLatMax) continue;
          if(haversineKm(k.c.lat, k.c.lon, g.c.lat, g.c.lon) <= FUSION_MAX_KM){ prev = g; break; }
        }
        if(prev){
          // Même lieu déjà retenu par un nom alternatif : les autres noms alternatifs qui correspondent sont
          // candidats (voir aliasScore). Un lieu retenu par son nom ou son code n'a pas de parenthèse — même
          // condition que l'index sur disque.
          if(k.alias && prev.m._alias) SI.offerAlias(prev.m, k.alias.text, k.alias.norm, k.alias.lang, rawQuery, q, lang);
          return;
        }
        var m = {name:k.c.name, cp:k.cp, allCps:k.c.cps, pop:k.c.pop, lat:k.c.lat, lon:k.c.lon, dept:k.c.dept,
          country:k.c.country, commune:k.c.commune || null, _placeNorm:k.c.norm};
        if(k.alias) SI.offerAlias(m, k.alias.text, k.alias.norm, k.alias.lang, rawQuery, q, lang);
        k.m = m;
        ancres.push(k);
        out.push(k);
      });
    });
    // Ordre GLOBAL identique à celui du disque, et pas seulement même contenu : celui-ci parcourt ses candidats déjà
    // triés par population décroissante, donc sa liste sort dans cet ordre. finish() retrie par population, mais ce
    // tri est STABLE : les ex æquo — et il y en a des milliers, la population étant inconnue pour 90 % des lieux —
    // gardent l'ordre d'insertion, qui décide seul de qui passe la coupe des 20 premiers. Sans ce tri, les deux
    // chemins rendaient bien vingt résultats chacun, mais pas les mêmes (112 saisies sur 4 487).
    return out.sort(ordreCandidat).map(function(k){ return k.m; });
  }
  ...
  var cpScan = function(pref, texte){
    (CP_INDEX.get(pref) || []).forEach(function(entry){
      var ckey = entry.commune.country + '|' + entry.commune.norm;
      if(cpMatchedCommunes[ckey]) return;
      if(entry.cp.toLowerCase().indexOf(texte) === 0){
  ...
  cpScan(qp, q);
  // Codes postaux à TIRET (« cn-110000 », « jp-100-0002 », « pt-3020-578 » : ~1 million d'entrées) : ils sont rangés tels
  // quels, alors que la saisie normalisée remplace le tiret par une espace — aucun n'était trouvé par la recherche EN
  // MÉMOIRE (17e audit du 20/09/2026, trouvé par le nouveau test de recherche). L'index sur disque avait déjà ce repli
  // depuis le 10e audit ; en mémoire il manquait, donc sur un déploiement neuf, tant que l'index n'est pas construit,
  // aucun de ces codes n'était cherchable. Seconde passe avec la saisie brute, seulement si la première ne donne rien.
  if(!candidats.length && rawQuery !== q && rawQuery.indexOf('-') >= 0 && rawQuery.length >= 3){
    cpScan(searchIndexPrefix(rawQuery), rawQuery);
  }
...
// ANTIMÉRIDIEN (18e audit du 21/09/2026). Un écart de longitude calculé bêtement (« to.lon - from.lon ») vaut ~349°
// au lieu de ~-11° pour une étape à cheval sur 180° : le trait échantillonné entre les deux points fait alors le tour
// du globe. Mesuré avant correction, sur une étape de 765 km INTERNE à la Tchoukotka (Vilyuneyskaya 175,25° E →
// Sireniki 173,94° O) : les points tombaient en Finlande, en Islande, au Canada, aux États-Unis, en Suède… Le moteur
// annonçait « Finlande traversée » et posait un rappel de restriction ISLANDAISE (routes F du Hálendi) sur un trajet
// arctique russe, sur 6 graines sur 25. lib/land-grid.js appliquait déjà ce repli de son côté (lonDelta) ; le moteur,
// non. Les deux fonctions ci-dessous le lui donnent : l'écart par le plus court côté, et la remise dans [-180, 180[.
function lonDeltaDeg(lon1, lon2){ var d = lon2 - lon1; if(d > 180) d -= 360; else if(d < -180) d += 360; return d; }
function normLonDeg(lon){ return ((lon + 180) % 360 + 360) % 360 - 180; }
function countriesAlong(from, to){
  var km = haversineKm(from.lat, from.lon, to.lat, to.lon);
  var dLonSeg = lonDeltaDeg(from.lon, to.lon); // voir ANTIMÉRIDIEN ci-dessus
  var n = Math.max(4, Math.min(200, Math.round(km * ROAD_FACTOR / 10)));
  var pts = [];
  for(var i = 0; i < n; i++){
    var f = (i + 0.5) / n, lat = from.lat + (to.lat - from.lat) * f, lon = normLonDeg(from.lon + dLonSeg * f);
...
// roadKmh : vitesse routière du mode en cours (18e audit du 21/09/2026). Sert à comparer les paires de ports d'un
// ferry sur le TEMPS total — à vélo, 80 km/h en dur valorisait 5,3 fois trop les kilomètres de route.
var LEG_CONSTRAINTS = { maxLegKm: 0, electric: false, ferryEnabled: true, roadKmh: 0, ferryClass: null };
...
// Longitudes prises RELATIVEMENT au départ du segment, par le plus court côté du globe (voir ANTIMÉRIDIEN plus
// haut) : sans cela, un segment à cheval sur 180° mesurait ~349° de large et la distance d'un point à ce segment
// était fausse de milliers de kilomètres — c'est par là qu'une restriction islandaise s'appliquait à un trajet
// tchouktche. Identique au calcul précédent tant qu'on ne franchit pas l'antiméridien.
function distToSegmentKm(pLat, pLon, aLat, aLon, bLat, bLon){
  var k = Math.cos(((aLat + bLat) / 2) * Math.PI / 180) * 111, K = 111;
  var ax = 0, ay = aLat * K, bx = lonDeltaDeg(aLon, bLon) * k, by = bLat * K, px = lonDeltaDeg(aLon, pLon) * k, py = pLat * K;
...
function ferryRoadParts(from, to, fromSide, toSide, ferry, roadKmhMode){
  ...
  if(fromList && toList && fromList.length && toList.length && fromList.length <= 64 && toList.length <= 64){
    ...
    // refD et lineSpeed sont constants sur l'appel : calculés UNE fois (17e audit du 20/09/2026 — ferryRoadParts, appelée
    // pour des milliers de candidats par tirage, était devenue 1,7 à 3 fois plus lente au 16e audit en les recalculant
    // pour chaque paire).
    var refDOnce = (fromList.length > 1 || toList.length > 1) && Number(ferry.distanceKm) > 0 ? ferryRefSeaKm(key, ports, fromSide, toSide, Number(ferry.distanceKm)) : 0;
    var lineSpeedOnce = ferry.durationH > 0 && ferry.distanceKm > 0 ? ferry.distanceKm / ferry.durationH : 0;
    var seaHours = function(pi, pj){
      var d = haversineKm(fromList[pi][0], fromList[pi][1], toList[pj][0], toList[pj][1]);
      var refD = refDOnce, lineSpeed = lineSpeedOnce;
    ...
    // Vitesse du MODE quand elle est connue, le garde-fou sinon (18e audit du 21/09/2026). Le commentaire ci-dessus
    // annonçait « celle du mode » depuis le 16e, mais le code prenait 80 km/h quel que soit le transport : à vélo
    // (15 km/h), le critère « temps total » troquait jusqu'à 150 km de route contre des heures de mer en valorisant
    // ces kilomètres 5,3 fois trop vite. Conséquence mesurée : Valletta → Catania choisissait 112 km de route
    // (+ 97 km de mer) alors qu'une paire à 0 km de route existe — et l'étape était ensuite REFUSÉE, ses 112 km
    // dépassant le plafond d'étape de 80 km du vélo. Idem Mariehamn → Stockholm et Dublin → Liverpool.
    var roadKmh = roadKmhMode > 0 ? roadKmhMode : (LEG_CONSTRAINTS.roadKmh > 0 ? LEG_CONSTRAINTS.roadKmh : FERRY_PAIR_ROAD_KMH);
...
// Un itinéraire PASSE-T-IL par une zone à tension ? C'est la seule preuve qui autorise à dire au visiteur « décochez
// Exclure les zones déconseillées » : sans elle, le conseil envoie dans un mur (9e audit du 18/09/2026 pour le tirage
// complet, étendu au diagnostic « hors de portée » le 22/09/2026). Le retour n'est pas compté : il ramène au départ,
// qui a son propre avertissement.
function passeParZone(legs){
...
// route(from, to) = 1,287 × vol(from, to) ≤ 1,287 × (vol(from, p1) + vol(p1, p2) + vol(p2, to)) ≤ capKm + 1,287 × écart(p1, p2),
// (1,287 = ROAD_FACTOR ; le commentaire est resté à 1,17, l'ancienne valeur non sourcée, jusqu'au 18e audit du
// 21/09/2026 — le raisonnement et le code étaient justes, seul le nombre cité ne l'était plus)
...
var DAY_REACH_TRIES_MAX = 25000; // contrôles exacts au plus pour la distance annoncée (17e audit : balayage exhaustif)
var DAY_REACH_PER_BUCKET = 20;  // essais du filet, si le budget tombe avant d'avoir trouvé (voir reachableMax0)
    ...
    if(tollCountryOf(from) || tollCountryOf(to)){
      ...
      var sumParts = function(parts){
        ...
        parts.forEach(function(part){
          ...
          if(!r) return;
          r0.amount += part.km * r; r0.km += part.km;
          if(r0.countries.indexOf(part.country) < 0) r0.countries.push(part.country);
        });
        // TARIF EFFECTIF de l'étape, et non celui de la DERNIÈRE portion facturée (18e audit du 21/09/2026).
        // « r0.rate = r » dans la boucle écrasait le tarif à chaque pays traversé : sur 34 étapes de 401 (8,5 %,
        // toutes à plusieurs pays), amount ≠ tolledKm × rate — Lyon → Lyon annonçait 87,2 € pour 851 km à 0,16 €/km,
        // soit 136 € si on refaisait le calcul. Le champ n'est pas affiché au visiteur, mais il sert d'étalon au
        // contrôle « un seul arrondi » du 16e audit, qui ne tenait que parce que son unique cas est mono-pays.
        // Moyenne pondérée par les kilomètres : amount = tolledKm × rate redevient vrai partout.
        r0.rate = r0.km > 0 ? r0.amount / r0.km : 0;
        return r0;
      };
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  function finalizeHop(fromPoint, toPoint, distanceKm, country){
    ...
    if(crossing) return finalizeFerryLeg(transportKey, crossing, ferryRoadParts(fromPoint, toPoint, fromZone, toZone, crossing, speed), speed, tollEnabled, fromPoint, toPoint);
    ...
    if(fromLandmass !== toLandmass){
      ...
      if(route) return finalizeFerryLeg(transportKey, route, ferryRoadParts(fromPoint, toPoint, fromLandmass, toLandmass, route, speed), speed, tollEnabled, fromPoint, toPoint);
    }
  ...
  if(days <= 1){
    ...
    // annoncée : le plus lointain lieu qui passe le contrôle exact du tirage (balayage exhaustif, voir reachableMax0).
    ...
    var reachableMax0 = function(below){
      ...
      // Balayage EXHAUSTIF, du plus lointain au plus proche, jusqu'au premier lieu faisable (17e audit du 20/09/2026).
      // Mesuré sur 584 annonces « hors de portée » : l'échantillonnage par tranches du 16e audit donnait la bonne
      // distance dans 94,2 % des cas seulement (écart jusqu'à 71 km, La Calera/MX en électrique : 223 annoncés pour 294
      // faisables), et le parcours complet ne coûte RIEN au médian — une fois sur deux le premier candidat essayé passe.
      // Pire cas mesuré du balayage : 1 156 ms, sous le budget DAY_REACH_MS. Garde-fous conservés : DAY_REACH_TRIES_MAX
      // contrôles exacts, DAY_REACH_MS, et le budget du tirage. Si le budget tombe avant tout succès, on échantillonne le
      // reste par tranches plutôt que de perdre l'annonce (« introuvable » serait moins utile que « hors de portée »).
      list.sort(function(a, b){ return b.distKm - a.distKm; });
      var best = 0, tries = 0, i0 = 0, fin = Date.now() + DAY_REACH_MS;
      for(; i0 < list.length && !best && tries < DAY_REACH_TRIES_MAX && Date.now() < fin && !tripTimeUp(); i0++){
        tries++;
        if(evPick0(list[i0])) best = list[i0].distKm;
      }
      // EXACT ou non (17e audit du 20/09/2026) : la boucle ci-dessus est sortie soit parce qu'elle a trouvé, soit parce
      // qu'elle a tout parcouru — dans ces deux cas la distance rendue EST le maximum. Sortie sur un garde-fou
      // (DAY_REACH_TRIES_MAX, DAY_REACH_MS, budget du tirage), elle ne l'est pas : le filet ci-dessous n'échantillonne
      // que le reste de la liste et peut rendre nettement moins (Tallinn en van, campagne d'invariants : 276 km rendus
      // pour 299 faisables, le budget étant tombé au milieu du balayage). L'appelant transmet le drapeau au client, et
      // la contre-épreuve des tests n'exige « X est le maximum » que lorsqu'il est vrai.
      var exact0 = !!best || i0 >= list.length;
      // Filet : budget épuisé sans succès — échantillon régulier du reste (une chance sur DAY_REACH_PER_BUCKET par tranche).
      if(!best && i0 < list.length){
        var pas0 = Math.max(1, Math.ceil((list.length - i0) / DAY_REACH_PER_BUCKET));
        for(var k0 = i0; k0 < list.length && !best && !tripTimeUp(); k0 += pas0){ if(evPick0(list[k0])) best = list[k0].distKm; }
      }
      return { below: Math.floor(best), durationBlocks: !aboveTimeOk, anyAbove: anyAbove, exact: exact0 };
    };
    ...
    if(candidates.length===0){
      ...
      if(minDist > 0 && !tripTimeUp()){
        ...
        LAST_TRIP_DIAGNOSTIC = (reach0.below > 0 && reach0.durationBlocks && reach0.anyAbove) ? { minDistanceUnreachable: true, returnCapKm: reach0.below, returnCapExact: reach0.exact }
    ...
    if(!picked0){
      ...
      if(minDist > 0 && !tripTimeUp()){
        ...
        var reach1 = reachableMax0(minDist);
        LAST_TRIP_DIAGNOSTIC = reach1.below > 0 ? { minDistanceUnreachable: true, returnCapKm: reach1.below, returnCapExact: reach1.exact }
  ...
  if(minDist > 0 && maxPossibleStops >= 2){
    ...
    if(1 + Math.ceil(Math.max(0, minDist - capForReturn) / hopRoadForReturn) > maxPossibleStops){
      // returnCapKm n'a PAS le même sens ici que pour un aller-retour, et c'est voulu : le message affiché au
      // visiteur (error.minDistanceTooFar) dit « on ne peut pas s'éloigner d'au moins {min} puis revenir dans le
      // rayon/temps de retour CHOISI ({radius}) » — {radius} y est la contrainte de retour qu'il a lui-même réglée,
      // pas une distance atteignable. C'est bien ce qu'on rend : min(rayon max, distance max, étape max).
      // returnCapExact: FAUX (18e audit du 21/09/2026). Le 17e l'avait posé à vrai avec la mention « exact par
      // construction », ce qui était une affirmation sans objet : le drapeau signifie « X est la plus grande
      // distance atteignable, mesurée » (voir reachableMax0 et la contre-épreuve X + 5 km), et cette valeur-ci
      // n'est pas une distance atteignable — pour 120 km de rayon de retour et 5 journées, 720 km d'éloignement
      // restent faisables par sauts successifs. Le drapeau doit donc dire non, sans quoi un contrôle qui rejouerait
      // le tirage à X + 5 km conclurait à tort que le moteur se trompe.
      LAST_TRIP_DIAGNOSTIC = { minDistanceUnreachable: true, returnCapKm: Math.round(Math.min(capForReturn, LEG_CONSTRAINTS.maxLegKm || capForReturn)), returnCapExact: false };
      return [];
    }
  }
...
// 18e audit du 21/09/2026 — CE PARAGRAPHE ÉTAIT FAUX SUR TROIS POINTS, et il argumentait pour un choix de
// conception que le code ne fait plus. Il disait : « init() fait tout le travail CPU (parsing de ~562 000 communes
// + ~87 000 alias…) de façon SYNCHRONE, en un seul passage — volontairement […] Un blocage ponctuel de ~1-2 s au
// tout premier démarrage est un compromis assumé ». Or :
//   - les volumes datent d'avant le passage au monde entier : 4 802 122 lieux et 1 717 740 alias aujourd'hui,
//     soit 8,5 et 20 fois plus ;
//   - le chargement n'est plus synchrone depuis longtemps — init() rend une promesse et rend la main à la boucle
//     d'événements entre chaque pays (voir yieldToEventLoop juste en dessous), précisément pour que le serveur
//     continue de répondre pendant le chargement ;
//   - il ne coûte pas 1 à 2 s mais ~30 s et ~4,4 Go (mesuré par tests/search.test.js), quand l'index de recherche
//     sur disque n'est pas disponible.
// Le vrai compromis est décrit par le commentaire qui suit.
...
function generateTrip(params){
  ...
  try {
    ...
    AVOID_TENSION = true;
    LEG_CONSTRAINTS = { maxLegKm: 0, electric: false, ferryEnabled: true, roadKmh: 0, ferryClass: null };
  ...
  LEG_CONSTRAINTS = { maxLegKm: maxLegKm, electric: !!TRANSPORT[transportKey].electric, ferryEnabled: ferryEnabled,
    roadKmh: TRANSPORT[transportKey].speed, ferryClass: TRANSPORT[transportKey].ferryClass };
  ...
  if(diagnostic && diagnostic.minDistanceUnreachable && !(days <= 1 && avoidTension && !TRIP_TIMED_OUT && Date.now() <= deadline)){
    return { legs: [], minDistanceUnreachable: true, returnCapKm: diagnostic.returnCapKm, returnCapExact: diagnostic.returnCapExact !== false, maxLegKm: maxLegKm };
  }
  ...
  if(legs.length === 0 && avoidTension && !TRIP_TIMED_OUT && Date.now() <= deadline){
    ...
    if(legsSansFiltre.length){
      if(passeParZone(legsSansFiltre)) tensionBlocked = true;
    ...
    if(days <= 1 && !legs.length && !tensionBlocked){
      ...
      if(diagnostic && diagnostic.minDistanceNotFound && diagSansFiltre && diagSansFiltre.minDistanceUnreachable){
        // Ce plafond vient du tirage SANS le filtre des zones à tension : avec le filtre, le premier tirage n'a trouvé
        // aucun candidat. Le visiteur à qui l'on annonçait « réduisez à X km au plus » recevait, à X km, un message
        // tout autre (« décochez Exclure les zones déconseillées ») — mesuré sur 28 annonces « hors de portée » sur
        // 268, soit 10,4 % (19e audit du 21/09/2026). Les deux informations sont donc rendues ensemble, et
        // returnCapExact passe à false : ce plafond n'est pas la plus grande distance atteignable AVEC ses réglages.
        //
        // MAIS le drapeau n'est plus posé SANS PREUVE (22/09/2026). Il l'était ici dès que le tirage sans filtre
        // rendait un plafond, sans vérifier que le filtre bloquait vraiment à ce plafond — mesuré sur 6 000 tirages :
        // 3 annonces sur 90 conseillaient de décocher alors que l'itinéraire passait même AVEC le filtre (Miquillo de
        // Rio Grande à Porto Rico, Dosé au Togo, Sulby à l'île de Man). Le drapeau suit désormais la même règle que
        // partout ailleurs dans cette fonction : il faut PASSER par une zone taguée. Un tirage de plus, au plafond,
        // sans filtre — dans un chemin qui allait de toute façon rendre une réponse vide ; s'il n'y a plus de temps,
        // le drapeau reste à faux plutôt que d'affirmer sans avoir vérifié.
        var capKm = diagSansFiltre.returnCapKm;
        var tensionAuPlafond = false;
        if(capKm > 0 && !TRIP_TIMED_OUT && Date.now() <= deadline){
          AVOID_TENSION = false;
          TRIP_DEADLINE = deadline;
          var diagAvantPlafond = LAST_TRIP_DIAGNOSTIC;
          LAST_TRIP_DIAGNOSTIC = null;
          var legsPlafond = buildItinerary(cityName, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, capKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity);
          LAST_TRIP_DIAGNOSTIC = diagAvantPlafond;
          AVOID_TENSION = true;
          tensionAuPlafond = legsPlafond.length > 0 && passeParZone(legsPlafond);
        }
        TRIP_DEADLINE = 0;
        return { legs: [], minDistanceUnreachable: true, returnCapKm: capKm, returnCapExact: false,
          tensionBlocked: tensionAuPlafond, maxLegKm: maxLegKm };
      }
    }
  }
  ...
  // IMPASSE DÉFINITIVE (24/09/2026). Un tirage vide qui n'est ni un manque de temps, ni le filtre des zones à
  // tension, ni un éloignement introuvable laissait afficher « réessayez, ou élargissez le rayon » : un conseil
  // FAUX pour les masses terrestres d'où RIEN n'est atteignable, quels que soient les réglages (Wallis, Tristan
  // da Cunha, Pitcairn, Clipperton, cinq atolls de Tuvalu… — 31 masses mesurées le 24/09/2026, voir le
  // CHANGELOG). Le drapeau n'est pas posé sur une présomption : un dernier tirage le PROUVE, avec les réglages
  // les plus permissifs que l'interface puisse produire — un seul jour, rayon maximal (3 000 km), aucune
  // distance minimale ni maximale, ferries autorisés, filtre des zones levé et aucun plafond par étape. S'il ne
  // trouve toujours rien, aucun réglage du visiteur n'y changera quoi que ce soit.
  // Comme partout ailleurs dans cette fonction, faute de temps le drapeau reste à faux plutôt que d'affirmer
  // sans avoir vérifié : mieux vaut le vieux message imprécis qu'une impasse annoncée à tort.
  var deadEnd = false;
  if(legs.length === 0 && !timedOut && !tensionBlocked && !TRIP_TIMED_OUT && Date.now() <= deadline){
    var diagAvantImpasse = LAST_TRIP_DIAGNOSTIC, contraintesAvantImpasse = LEG_CONSTRAINTS;
    LEG_CONSTRAINTS = { maxLegKm: 0, electric: false, ferryEnabled: true,
      roadKmh: TRANSPORT[transportKey].speed, ferryClass: null };
    AVOID_TENSION = false;
    TRIP_DEADLINE = deadline;
    var essaiLibre = buildItinerary(cityName, 1, budgetKey, transportKey, false, cityCoord, avoidTent, tripStart,
      3000, avoidNorm, 0, 0, true, preferredCurrency, 1, 1);
    AVOID_TENSION = avoidTension;
    LEG_CONSTRAINTS = contraintesAvantImpasse;
    LAST_TRIP_DIAGNOSTIC = diagAvantImpasse;
    deadEnd = essaiLibre.length === 0 && !TRIP_TIMED_OUT;
    TRIP_TIMED_OUT = false;
  }
  ...
  if(deadEnd) result.deadEnd = true;
  return result;
  }
}
...
module.exports = { init, isReady, isSearchReady, searchCity, generateTrip, loadChargingStations,
  ...
  internals: { normalizeCityName, parseCommunesFile, parseAliasesOfCountry, IDEOGRAPHIC_RE,
    // EMPREINTE EXACTE DU NORMALISATEUR (20e audit du 21/09/2026), pour lib/search-index.js. L'index sur disque
    // se protégeait d'un changement de normalisation en comparant la sortie d'une liste d'échantillons
    // (NORM_PROBE) : cela détecte qu'un caractère cesse d'être traité, jamais qu'un caractère NOUVEAU se met à
    // l'être — aucun échantillon ne le contient. Mesuré : les 32 caractères qu'on pourrait ajouter aux deux jeux
    // passaient tous inaperçus, et l'index déjà construit restait accepté avec ses anciennes clés. C'est
    // exactement la régression de la 16e passe (3 416 alias persans et ourdous devenus introuvables sans erreur).
    // Le texte du normalisateur ET de tous les motifs qu'il emploie est donc joint à l'empreinte : commentaires
    // retirés et blancs réduits, pour qu'un simple commentaire ne force pas une reconstruction de 85 s.
    normalizeCityNameSource: [normalizeCityName.toString().replace(/\/\/[^\n]*/g, ''), ASCII_RE, DIACRITICS_RE,
      INVISIBLE_RE, NORM_DROP_RE, SEP_RE, LIGATURES_RE, JSON.stringify(LIGATURES)].map(String).join('\u0001').replace(/\s+/g, ' '),
