...
function buildRealRoute(startLat, startLon, startLandmass, startZone, maxRadiusKm, numStops, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, shortHop, depNorm){
  ...
  for(var i=0; i<numStops; i++){
    ...
    if(isLast){
      ...
      var preds = [withinMaxDist, zoneOk, notStart, notUsed, reachable, legOk];
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var preds0 = [withinMaxDist0, zoneOk0, notStart0, reachable0, legOk0];
  ...
  // quelques nouveaux essais avant de renoncer à l'éloignement (itinéraire de secours plus proche). Aucun nouvel essai une
  // fois le budget de temps épuisé.
  for(var attempt = 0; attempt < 4 && route.length === 0 && minDist > 0 && !tripTimeUp(); attempt++){
    route = buildRealRoute(cLat, cLon, startLandmass, zoneOf(cityCoord), maxRadiusKm, numStops, avoidNorm, minDist, maxDist, ferryEnabled, false, depNorm);
  }
  if(route.length === 0 && !tripTimeUp()) route = buildRealRoute(cLat, cLon, startLandmass, zoneOf(cityCoord), Math.max(maxRadiusKm, 300), 1, avoidNorm, 0, maxDist, ferryEnabled, false, depNorm);
  ...
  if(route.length === 0 && !tripTimeUp()) route = buildRealRoute(cLat, cLon, startLandmass, zoneOf(cityCoord), Math.max(maxRadiusKm, 300), 1, avoidNorm, 0, maxDist, ferryEnabled, true, depNorm);
