...
function legAllowed(from, to, ignoreMaxLeg, skipEv){
  ...
  if(ferry){
    ...
    var checkEv = !skipEv && LEG_CONSTRAINTS.electric;
    if((!ignoreMaxLeg && LEG_CONSTRAINTS.maxLegKm) || checkEv){
      var parts = sea ? ferryRoadParts(from, to, from.zone, toZone, ferry) : ferryRoadParts(from, to, from.landmass, toLandmass, ferry);
      if(!ignoreMaxLeg && LEG_CONSTRAINTS.maxLegKm && !(parts.km <= LEG_CONSTRAINTS.maxLegKm)) return false; // NaN refusé
      if(checkEv && parts.fromPort && (evPlan(from.lat, from.lon, parts.fromPort.lat, parts.fromPort.lon) === null ||
