...
const { COUNTRIES, TRANSPORT, FERRY_ROUTES, SEA_CROSSINGS, FERRY_PORTS, TOLL_RATE_BY_COUNTRY, TOLL_MIN_DISTANCE_KM,
...
// landKm/speed : partie par la route jusqu'au port puis depuis le port d'arrivée (voir ferryLandKm), affichée à côté de
// la traversée (roadKm, roadTime) ; travelTime/distanceKm restent ceux de la traversée seule.
function finalizeFerryLeg(transportKey, route, landKm, speed){
  ...
  if(landKm >= 1 && speed > 0){
    leg.roadKm = Math.round(landKm);
    leg.roadTime = fmtHours(landKm / speed);
  }
  return leg;
}
...
function buildCommuneGrid(){
  ...
  COMMUNES.forEach(function(c){
    ...
    var cx = Math.floor(c.lat/GRID_CELL_DEG), cy = Math.floor(c.lon/GRID_CELL_DEG), n = cellNum(cx, cy);
...
var LEG_CONSTRAINTS = { maxLegKm: 0, electric: false };
...
function evPlan(aLat, aLon, bLat, bLon){
  ...
  for(var si = 1; si < n; si++){
    var f = si / n, sLat = aLat + (bLat - aLat) * f, sLon = aLon + (bLon - aLon) * f;
...
// ignoreMaxLeg : premier trajet d'un séjour avec « distance d'éloignement » renseignée — choix de l'utilisateur : ce trajet
// peut dépasser la distance max entre étapes pour prendre de l'éloignement dès le départ (les bornes restent exigées).
// skipEv : ne pas vérifier les bornes (filtre de masse sur des milliers de candidats) — voir evLegOk, appliqué au tirage.
// Kilomètres par la route d'un trajet avec traversée : du point de départ à un port de sa rive, plus d'un port de l'autre
// rive jusqu'à l'arrivée (FERRY_PORTS, scripts/build-ferry-ports.js), au plus court. Quand la liaison précise les paires
// de ports réellement desservies (pairs, indices dans l'ordre des rives de la clé), seules ces paires sont envisagées ;
// sinon tous les ports d'une rive sont supposés reliés à tous ceux de l'autre. Un port est son quai OpenStreetMap quand il
// est connu, sinon le centre de sa localité. Liaison sans ports connus : estimation par la distance à vol d'oiseau moins
// la traversée, qui sous-estime la partie terrestre.
function ferryLandKm(from, to, fromSide, toSide, ferry){
  var key = landmassKey(fromSide, toSide);
  var ports = FERRY_PORTS[key];
  if(ports && ports[fromSide] && ports[toSide]){
    var fromList = ports[fromSide], toList = ports[toSide], i, j;
    var dFrom = [], dTo = [];
    for(i = 0; i < fromList.length; i++) dFrom.push(roadDistanceKm(from.lat, from.lon, fromList[i][0], fromList[i][1]));
    for(j = 0; j < toList.length; j++) dTo.push(roadDistanceKm(to.lat, to.lon, toList[j][0], toList[j][1]));
    if(ports.pairs){
      var fromFirst = key.split('|')[0] === fromSide, best = Infinity;
      for(i = 0; i < ports.pairs.length; i++){
        var p = ports.pairs[i];
        best = Math.min(best, dFrom[fromFirst ? p[0] : p[1]] + dTo[fromFirst ? p[1] : p[0]]);
      }
      return best;
    }
    return Math.min.apply(null, dFrom) + Math.min.apply(null, dTo);
  }
  return Math.max(0, haversineKm(from.lat, from.lon, to.lat, to.lon) - (Number(ferry.distanceKm) || 0)) * ROAD_FACTOR;
}
function legAllowed(from, to, ignoreMaxLeg, skipEv){
  ...
  if(ferry){
    ...
    // Yokohama, Lyon → Corse).
    if(!ignoreMaxLeg && LEG_CONSTRAINTS.maxLegKm){
      var landKm = sea ? ferryLandKm(from, to, from.zone, toZone, ferry) : ferryLandKm(from, to, from.landmass, toLandmass, ferry);
      if(landKm > LEG_CONSTRAINTS.maxLegKm) return false;
    }
    return true;
  }
  var road = roadDistanceKm(from.lat, from.lon, to.lat, to.lon);
  if(!ignoreMaxLeg && LEG_CONSTRAINTS.maxLegKm && road > LEG_CONSTRAINTS.maxLegKm) return false;
...
function pickCandidate(candidates, ok){
  ...
  for(var t = 0; t < order.length && t < EV_PICK_TRIES; t++){
...
function borderReach(cc, km){
  ...
  for(var i=0; i<cells.length; i+=2){
    ...
    for(var dx=-latSpan; dx<=latSpan; dx++){
      for(var dy=-lonSpan; dy<=lonSpan; dy++) reach.add(cellNum(cx+dx, cy+dy));
    }
  }
...
function nearCountryWithin(lat, lon, cc, km){
  ...
  for(var dx=-latSpan; dx<=latSpan; dx++){
    for(var dy=-lonSpan; dy<=lonSpan; dy++){
      var cellCountries = COUNTRIES_BY_CELL.get(cellNum(cx+dx, cy+dy));
...
function buildRealRoute(startLat, startLon, startLandmass, startZone, maxRadiusKm, numStops, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, shortHop){
  ...
  for(var i=0; i<numStops; i++){
    ...
    var evPick = null; // vérification des bornes au tirage (voiture électrique)
    ...
    if(isLast){
      ...
      var lastLegOk = function(x){
        return legAllowed(fromCur, x.commune, false, true) &&
          legAllowed({ lat: x.commune.lat, lon: x.commune.lon, landmass: landmassOf(x.commune), zone: zoneOf(x.commune) }, { lat: startLat, lon: startLon, landmass: startLandmass, zone: startZone }, false, true);
      };
      evPick = LEG_CONSTRAINTS.electric ? function(x){
        return legAllowed(fromCur, x.commune) &&
          legAllowed({ lat: x.commune.lat, lon: x.commune.lon, landmass: landmassOf(x.commune), zone: zoneOf(x.commune) }, { lat: startLat, lon: startLon, landmass: startLandmass, zone: startZone });
      } : null;
      ...
      var exemptFirst = isFirst && minDist > 0; // premier trajet avec distance d'éloignement : hors distance max
      var stopIndex = i;
      var legOk = function(x){ return canStillReturn(x, stopIndex) && legAllowed(fromHere, x.commune, exemptFirst, true); };
      evPick = LEG_CONSTRAINTS.electric ? function(x){ return legAllowed(fromHere, x.commune, exemptFirst); } : null;
  ...
  while(route.length){
    ...
    var exemptReturn = route.length === 1 && numStops === 1 && minDist > 0;
    if(legAllowed({ lat: tail.lat, lon: tail.lon, landmass: landmassOf(tail), zone: zoneOf(tail) }, startPoint, exemptReturn)) break;
    route.pop();
  }
  return route;
}
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  function finalizeHop(fromPoint, toPoint, distanceKm, country){
    ...
    if(crossing) return finalizeFerryLeg(transportKey, crossing, ferryLandKm(fromPoint, toPoint, fromZone, toZone, crossing), speed);
    ...
    if(fromLandmass !== toLandmass){
      ...
      if(route) return finalizeFerryLeg(transportKey, route, ferryLandKm(fromPoint, toPoint, fromLandmass, toLandmass, route), speed);
    }
  ...
  if(days <= 1){
    ...
    var legOk0 = function(x){ return legAllowed(fromStart0, x.commune, minDist > 0, true); }; // aller ; le retour a la même longueur
    var evPick0 = LEG_CONSTRAINTS.electric ? function(x){ return legAllowed(fromStart0, x.commune, minDist > 0); } : null;
    ...
    if(candidates.length===0){
      candidates = findNearbyCommunes(cLat, cLon, minHop0, hop*1.6, 0).filter(reachable0).filter(legOk0);
      if(maxDist > 0) candidates = candidates.filter(function(x){ return x.distKm <= maxDist; });
    }
  ...
  // Distance d'éloignement : le chemin du retour se construit étape par étape au hasard et peut aboutir à une impasse ;
  // quelques nouveaux essais avant de renoncer à l'éloignement (itinéraire de secours plus proche).
  for(var attempt = 0; attempt < 4 && route.length === 0 && minDist > 0; attempt++){
    route = buildRealRoute(cLat, cLon, startLandmass, zoneOf(cityCoord), maxRadiusKm, numStops, avoidNorm, minDist, maxDist, ferryEnabled);
  }
  ...
  if(!firstLegInfo.ferryInfo && travelHoursOf(firstLegInfo.travelTime) > FIRST_LEG_REST_HOURS && nights[0] < 2 && maxNightsPerStop >= 2 && totalNights >= 2){
    var donor = -1;
    ...
    if(donor > 0){ nights[donor]--; nights[0]++; }
      ...
      var backOk = legAllowed({ lat: before.lat, lon: before.lon, landmass: landmassOf(before), zone: zoneOf(before) },
      ...
      if(backOk){ route.pop(); nights[0] += nights.pop(); }
        ...
        for(var mi = 1; mi < route.length - 1; mi++){
          ...
          if(legAllowed({ lat: a.lat, lon: a.lon, landmass: landmassOf(a), zone: zoneOf(a) }, b) && reallyAdjacent(zoneOf(a), zoneOf(b))){
...
function generateTrip(params){
  ...
  LEG_CONSTRAINTS = { maxLegKm: maxLegKm, electric: !!TRANSPORT[transportKey].electric };
  ...
  if(diagnostic && diagnostic.minDistanceUnreachable){
    LEG_CONSTRAINTS = { maxLegKm: 0, electric: false };
  ...
  AVOID_TENSION = true;
  LEG_CONSTRAINTS = { maxLegKm: 0, electric: false };
  return { legs: legs, spinPool: spinPool, departureTension: departureTension, tensionBlocked: tensionBlocked, maxLegKm: maxLegKm, notices: notices,
