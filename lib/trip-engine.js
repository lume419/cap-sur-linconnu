...
// Kilomètres par la route d'un trajet avec traversée : du point de départ au port le plus proche de sa rive, plus du
// port le plus proche de l'arrivée jusqu'à celle-ci (FERRY_PORTS, scripts/build-ferry-ports.js). Les deux ports sont
// choisis indépendamment (toutes les paires de ports d'une liaison ne sont pas forcément desservies) et chaque port est
// approché par le centre de sa localité. Liaison sans ports connus : estimation par la distance à vol d'oiseau moins la
// traversée, qui sous-estime la partie terrestre.
function ferryLandKm(from, to, fromSide, toSide, ferry){
  var ports = FERRY_PORTS[landmassKey(fromSide, toSide)];
  if(ports && ports[fromSide] && ports[toSide]){
    var nearest = function(pt, list){
      var best = Infinity;
      for(var i = 0; i < list.length; i++) best = Math.min(best, roadDistanceKm(pt.lat, pt.lon, list[i][0], list[i][1]));
      return best;
    };
    return nearest(from, ports[fromSide]) + nearest(to, ports[toSide]);
  }
