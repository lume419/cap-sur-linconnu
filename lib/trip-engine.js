...
// Une traversée (ferry entre masses terrestres, ou liaison entre zones) peut-elle partir de ce point ? Voir strictPossible.
function ferryPossibleFrom(landmass, zone){
  return !!LEG_CONSTRAINTS.ferryEnabled && (FERRY_SIDES.has(landmass) || SEA_ZONES.has(zone));
}
...
function buildRealRoute(startLat, startLon, startLandmass, startZone, maxRadiusKm, numStops, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, shortHop, depNorm){
  ...
  for(var i=0; i<numStops; i++){
    ...
    if(isLast){
      ...
      var strictPossible = !extendedFirst || legCapKm(0) >= minHop || ferryPossibleFrom(curLandmass, curZone);
...
function finalizeLeg(distanceKm, speed, transportKey, tollEnabled, country, from, to){
  ...
  var tollInfo = null;
  // Sans les deux extrémités (petites étapes d'une même ville, toujours sous TOLL_MIN_DISTANCE_KM), rien à échantillonner.
  if(tollClass && distanceKm >= TOLL_MIN_DISTANCE_KM && from && to){
    ...
    var parts = tollGrid.tolledParts(from.lat, from.lon, to.lat, to.lon, distanceKm * TOLL_ROAD_FACTOR / ROAD_FACTOR, { duPoint: paysDuPoint, extremites: paysEtape, interieur: insideCountry });
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var candidates = (!extended0 || legCapKm(0) >= minHop0 || ferryPossibleFrom(startLandmass, startZone0)) ? search0() : [];
  ...
  route.forEach(function(commune, stopIdx){
    ...
    for(var n=0; n<nightsHere; n++){
      dayCounter++;
      // Journée sur place : petit trajet local de 3 à 14 km, jamais au-delà de la distance max entre étapes (9e audit).
      var distanceKm = n===0 ? Math.round(roadDistanceKm(prevLat, prevLon, commune.lat, commune.lon)) : Math.round(rand(3, Math.min(14, Math.max(3, LEG_CONSTRAINTS.maxLegKm || 14))));
