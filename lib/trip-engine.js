...
var SPEED_REF_KMH = 79.4, COUNTRY_SPEED = {};
try {
...
function countrySpeedFactor(from, to, country){
  var list = [];
  [from && from.country, to && to.country, country].forEach(function(cc){ if(cc && COUNTRY_SPEED[cc] && list.indexOf(cc) < 0) list.push(cc); });
  if(!list.length) return 1;
  return list.reduce(function(sum, cc){ return sum + COUNTRY_SPEED[cc]; }, 0) / list.length;
}
function finalizeLeg(distanceKm, speed, transportKey, tollEnabled, country, from, to){
  ...
  if(tollClass && from && to){
    ...
    if(tollCountryOf(from) || tollCountryOf(to)){
      ...
      if(hi.countries.length && hi.amount > 0){
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var dayDrive0 = function(x){ return 2 * x.distKm / speed <= DAY_TRIP_MAX_HOURS; };
    ...
    if(candidates.length===0){
      ...
      var dayCapKm = Math.floor(DAY_TRIP_MAX_HOURS / 2 * speed);
