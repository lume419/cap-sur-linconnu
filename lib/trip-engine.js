...
function searchCommunes(query, limit, preferCountry, lang){
  ...
  function pushMatch(c, cp, alias){
    // Lieu SANS code postal (21/09/2026) : la clé retomberait sur « pays|nom| » et deux homonymes sans code se
    // masqueraient l'un l'autre — le défaut décrit ci-dessus, mais aggravé par les 106 000 lieux que la publication
    // sans code vient d'ajouter. Faute de code, c'est la COORDONNÉE qui distingue : elle ne collisionne jamais entre
    // deux lieux différents, et reste identique pour un même lieu trouvé par son nom, son code ou un alias.
    var key = c.country + '|' + c.norm + '|' + (cp || (c.lat + ',' + c.lon));
    ...
    if(prev){
      ...
      // Lieu plus peuplé portant la même clé : il remplace le précédent, à sa place dans la liste. Les noms
      // alternatifs déjà proposés étaient ceux de l'AUTRE lieu : ils ne le suivent pas.
  ...
  if(legs.length === 0 && avoidTension && !TRIP_TIMED_OUT && Date.now() <= deadline){
    ...
    if(days <= 1 && !legs.length && !tensionBlocked){
      ...
      if(diagnostic && diagnostic.minDistanceNotFound && diagSansFiltre && diagSansFiltre.minDistanceUnreachable){
        ...
        return { legs: [], minDistanceUnreachable: true, returnCapKm: diagSansFiltre.returnCapKm, returnCapExact: false,
          tensionBlocked: true, maxLegKm: maxLegKm };
      }
    }
  }
