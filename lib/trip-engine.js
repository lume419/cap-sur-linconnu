...
function legAllowed(from, to, ignoreMaxLeg, skipEv){
  ...
  var ferry = (toLandmass !== from.landmass && ferryRouteFor(from.landmass, toLandmass)) ||
    (from.zone && seaCrossingFor(from.zone, to.zone !== undefined ? to.zone : zoneOf(to)));
  if(ferry){
    // Trajet avec traversée : la partie par la route (jusqu'au port, puis depuis le port d'arrivée) compte aussi dans le
    // maximum entre étapes. Les ports n'étant pas géolocalisés, elle est estimée par la distance à vol d'oiseau moins
    // la traversée — sans ça, tout lieu de l'autre rive était accepté (à vélo : Sapporo → Yokohama, Lyon → Corse).
    if(!ignoreMaxLeg && LEG_CONSTRAINTS.maxLegKm){
      var landKm = Math.max(0, haversineKm(from.lat, from.lon, to.lat, to.lon) - (Number(ferry.distanceKm) || 0)) * ROAD_FACTOR;
