...
function reallyAdjacent(countryA, countryB){
  if(!countryA || !countryB || countryA === countryB) return true;
  if(!NEW_BATCH_COUNTRIES.has(countryA) && !NEW_BATCH_COUNTRIES.has(countryB)) return true; // hors périmètre de ce correctif
  return ADJACENT_PAIRS.has(countryA + '|' + countryB) || ADJACENT_PAIRS.has(countryB + '|' + countryA);
}

function buildRealRoute(startLat, startLon, startLandmass, startCountry, maxRadiusKm, numStops, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled){
  var route = [];
  var used = {};
  if(avoidNorm) used[avoidNorm] = true;
  var curLat = startLat, curLon = startLon;
  var curLandmass = startLandmass;
  var curCountry = startCountry;
  ...
  function reachable(x){
    ...
    if(!reallyAdjacent(curCountry, x.commune.country)) return false;
  ...
  for(var i=0; i<numStops; i++){
    ...
    if(isLast){
      ...
      function reachableLast(x){ return reachable(x) && reallyAdjacent(startCountry, x.commune.country); }
    ...
    curCountry = chosen.commune.country;
  }
  return route;
}
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    function reachable0(x){
      var toLandmass = landmassOf(x.commune);
      if(!reallyAdjacent(cityCoord.country, x.commune.country)) return false;
  ...
  var route = buildRealRoute(cLat, cLon, startLandmass, cityCoord.country, maxRadiusKm, numStops, avoidNorm, minDist, maxDist, ferryEnabled);
  if(route.length === 0) route = buildRealRoute(cLat, cLon, startLandmass, cityCoord.country, Math.max(maxRadiusKm, 300), 1, null, 0, maxDist, ferryEnabled);
