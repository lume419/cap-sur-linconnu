...
function ferryRouteForPair(route, parts){
  ...
  return Object.assign({}, route, { distanceKm: km, durationH: Math.round(km / ferryMedianSpeed(km) * 10) / 10,
...
var DAY_REACH_TRIES = 12; // contrôles exacts au plus, deux fois (plus lointains + échantillon), pour la distance annoncée
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var reachableMax0 = function(below){
      ...
      var tries = list.slice(0, DAY_REACH_TRIES);
      var rest = list.slice(DAY_REACH_TRIES), step = Math.max(1, Math.ceil(rest.length / DAY_REACH_TRIES));
      for(var k = 0; k < rest.length; k += step) tries.push(rest[k]);
      var best = 0;
      for(var t = 0; t < tries.length && !tripTimeUp(); t++){ if(evPick0(tries[t])){ best = tries[t].distKm; break; } }
