...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var reachableMax0 = function(below){
      var rMax = Math.ceil(DAY_TRIP_MAX_HOURS / 2 * speed * (transportKey === 'velo' ? 1 : MAX_COUNTRY_SPEED) * dayMoto0);
      var r = Math.min(rMax, searchCap0, below + 30);
      var placeOk = [withinMaxDist0, withinRadius0, zoneOk0, notStart0, reachable0];
      var near = filterCandidates(findNearbyCommunes(cLat, cLon, floor0, r, 0), placeOk);
      var best = 0, aboveTimeOk = false;
      near.forEach(function(x){
        var timeOk = dayDrive0(x);
        if(x.distKm < below){ if(timeOk && x.distKm > best) best = x.distKm; }
        else if(timeOk) aboveTimeOk = true;
      });
      return { below: Math.floor(best), durationBlocks: !aboveTimeOk };
    };
    ...
    if(candidates.length===0){
      ...
      if(minDist > 0 && !tripTimeUp()){
        ...
        LAST_TRIP_DIAGNOSTIC = (reach0.below > 0 && reach0.durationBlocks) ? { minDistanceUnreachable: true, returnCapKm: reach0.below }
