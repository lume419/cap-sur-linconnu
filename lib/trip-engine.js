...
function evPlan(aLat, aLon, bLat, bLon){
  ...
  var stops = [], pLat = aLat, pLon = aLon, guard = 0, plan = null;
  for(;;){
    var left = haversineKm(pLat, pLon, bLat, bLon);
    if(left <= reach){ plan = stops; break; }
    if(++guard > 40) break;
    var best = null, bestLeft = left - 20; // progresser d'au moins 20 km
    eachChargerNear(pLat, pLon, reach, function(lat, lon){
      var l = haversineKm(lat, lon, bLat, bLon);
      if(l >= bestLeft) return;
      if(distToSegmentKm(lat, lon, pLat, pLon, bLat, bLon) > CHARGER_CORRIDOR_KM) return;
      best = [lat, lon]; bestLeft = l;
    });
    if(!best) break;
    stops.push(best); pLat = best[0]; pLon = best[1];
  }
...
function legAllowed(from, to){
  ...
  if(LEG_CONSTRAINTS.maxLegKm && road > LEG_CONSTRAINTS.maxLegKm) return false;
  if(LEG_CONSTRAINTS.electric && evPlan(from.lat, from.lon, to.lat, to.lon) === null) return false;
  return true;
}
...
function buildRealRoute(startLat, startLon, startLandmass, startZone, maxRadiusKm, numStops, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, shortHop){
  ...
  for(var i=0; i<numStops; i++){
    ...
    if(isLast){
      ...
      var legOk = function(x){ return legAllowed(fromHere, x.commune); };
  ...
  while(route.length){
    ...
    if(legAllowed({ lat: tail.lat, lon: tail.lon, landmass: landmassOf(tail), zone: zoneOf(tail) }, startPoint)) break;
    route.pop();
  }
  return route;
}
...
function buildItinerary(city, days, budgetKey, transportKey, tollEnabled, cityCoord, avoidTent, tripStart, maxRadiusKm, avoidNorm, minDistanceKm, maxDistanceKm, ferryEnabled, preferredCurrency, minDaysPerCity, maxDaysPerCity){
  ...
  if(days <= 1){
    ...
    var legOk0 = function(x){ return legAllowed(fromStart0, x.commune); }; // aller ; le retour a la même longueur
  ...
  route.forEach(function(commune, stopIdx){
    ...
    for(var n=0; n<nightsHere; n++){
      ...
      var legInfo = n===0 ? finalizeHop(prevPoint, commune, distanceKm, commune.country) : finalizeLeg(distanceKm, speed, transportKey, tollEnabled, tollCountryOf(commune), null, null);
