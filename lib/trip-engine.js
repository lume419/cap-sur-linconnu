...
// tapant EXACTEMENT le même caractère (18e audit du 21/09/2026). Mesure sur les données publiées : 40 696 lieux et
// 1 112 alias portent une apostrophe modificative — « Chervonyy Donets‘ » (UA), « Jaganʻ » (PL) —, 150 un accent grave
// (« T`lminci », MK), 79 un tiret demi-cadratin (« Nago–Torbole », IT), 13 un accent aigu. Aucun clavier ne produit
// ces caractères à la place de l'apostrophe ou du trait d'union ordinaires. Conséquence mesurée : 14 335 lieux d'un
// même pays portent le MÊME nom écrit avec deux apostrophes différentes — au Yémen, les 232 groupes fusionnés par ce
// correctif réunissent tous deux graphies d'un seul nom (« Shay`ān » et « Shay‘ān », « Ar Rubū` » et « Ar Rubū‘ ») :
// chercher l'une ne rendait jamais les lieux écrits avec l'autre.
...
// Malaga), introuvables jusqu'ici en tapant la forme non vocalisée, la seule que l'on tape en pratique.
...
// l'écart entre la plus lente et la plus rapide de ces sept lignes vaut 21 heures : la durée annoncée pour une
  ...
  if(legs.length === 0 && avoidTension && !TRIP_TIMED_OUT && Date.now() <= deadline){
    ...
    if(days <= 1 && !legs.length && !tensionBlocked){
      ...
      if(diagnostic && diagnostic.minDistanceNotFound && diagSansFiltre && diagSansFiltre.minDistanceUnreachable){
        TRIP_DEADLINE = 0;
        return { legs: [], minDistanceUnreachable: true, returnCapKm: diagSansFiltre.returnCapKm, returnCapExact: diagSansFiltre.returnCapExact !== false, maxLegKm: maxLegKm };
      }
    }
  }
