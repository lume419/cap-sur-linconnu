...
function searchCommunes(query, limit){
  ...
  if(q.length === 2 && IDEOGRAPHIC_RE.test(q)){
    var shortSeen = {}, shortMatches = [];
    (SHORT_IDEOGRAPHIC_INDEX.get(q) || []).forEach(function(e){
      var c = e.commune, key = c.country + '|' + c.norm + '|' + c.cps[0];
      if(shortSeen[key]) return;
      shortSeen[key] = true;
      shortMatches.push({name:c.name, cp:c.cps[0], allCps:c.cps, pop:c.pop, lat:c.lat, lon:c.lon, dept:c.dept, country:c.country});
    });
    shortMatches.sort(function(a,b){ return b.pop - a.pop; });
    return shortMatches.slice(0, limit);
  }
...
function init(communesRaw, aliasesRaw, featuredRaw){
  // Durée de chaque étape affichée dans les journaux du serveur : avec ~4 millions de lieux, un
  // ralentissement d'une étape (ex. bandes frontalières des zones à tension) se voit immédiatement.
  ...
  step('grille');
  buildTensionIndex();
  step('zones à tension');
  buildSearchIndex();
  step('index de recherche');
