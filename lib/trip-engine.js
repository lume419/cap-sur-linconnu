...
function searchCommunes(query, limit, preferCountry, lang){
  ...
  function pushMatch(c, cp, alias){
    ...
    // La liste par clé reste minuscule (un à trois lieux) : la comparer n'a pas de coût mesurable.
    var key = c.country + '|' + c.norm + '|' + (cp || '');
    var groupe = seenKeys[key];
    if(!groupe) groupe = seenKeys[key] = [];
    var prev = null;
    for(var gi = 0; gi < groupe.length; gi++){
      if(haversineKm(c.lat, c.lon, groupe[gi].lat, groupe[gi].lon) <= FUSION_MAX_KM){ prev = groupe[gi]; break; }
    }
    if(prev){
