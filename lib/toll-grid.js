...
// paysDuPoint (facultatif, fourni par le moteur) : fonction (lat, lon) -> pays où se trouve ce point, ou null. Quand
// elle est donnée, un point n'est facturé qu'au barème de SON pays, et un point dont le pays est inconnu n'est pas
// facturé du tout — jamais de facturation à tort.
const SAMPLE_KM = 10;
const MAX_SAMPLES = 200;
function tolledParts(fromLat, fromLon, toLat, toLon, roadKm, paysDuPoint){
  ...
  for(let i = 0; i < n; i++){
    ...
    const lat = fromLat + (toLat - fromLat) * f, lon = fromLon + (toLon - fromLon) * f;
    let seul;
    if(paysDuPoint){
      seul = paysDuPoint(lat, lon);
      if(!seul) continue;
    }
    const cc = cellCountry(lat, lon, seul);
    if(!cc) continue;
    hits++;
