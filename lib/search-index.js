...
// 4 (23/09/2026) : une 9e colonne par lieu, la commune de rattachement d'un lieu-dit français. Un index de
// version 3 est refusé et reconstruit — il lirait la colonne suivante à la place.
const VERSION = 4;
const ENTRY_SIZE = 12;
const TYPE_CP = 0, TYPE_NAME = 1, TYPE_ALIAS = 2;
// Deux fiches de même nom et de même code postal à moins de FUSION_MAX_KM sont le même lieu vu deux fois : une seule
// suggestion. Au-delà, ce sont deux villes (22/09/2026). Même valeur que dans lib/trip-engine.js, et pour la même
// raison : le dédoublonnage des générateurs travaille à 0,01° (~1 km), les vrais homonymes sont bien plus loin.
const FUSION_MAX_KM = 10;
function distKm(lat1, lon1, lat2, lon2){
  var r = Math.PI / 180;
  var x = Math.sin((lat2 - lat1) * r / 2) * Math.sin((lat2 - lat1) * r / 2) +
...
function normSignature(engineInternals){
  ...
  // Les échantillons ci-dessus attrapent un caractère qui cesse d'être traité ; ils ne peuvent PAS attraper un
  // caractère qui se met à l'être, puisque aucun ne le contient (20e audit du 21/09/2026). Le TEXTE du
  // normalisateur et de ses motifs est donc joint : toute modification, ajout compris, change l'empreinte et
  // l'index déjà construit est refusé plutôt que relu avec des clés périmées.
  return NORM_PROBE.map(function(x){ return f(x); }).join('\u0001') +
    '\u0002' + String((engineInternals && engineInternals.normalizeCityNameSource) || '');
}
...
function build(dataDir, finalDir, engineInternals, log){
  ...
  communeFiles.forEach(function(f){
    ...
    eachLine(raw, function(line){
      ...
      // 6e champ facultatif du fichier de données : la commune de rattachement (voir parseCommunesFile).
      var commune = parts[5] || '';
      var norm = normalize(name);
      var placeId = placeCount++;
      var placeLine = [cc, clean(norm), clean(name), cps.map(clean).join(','), pop, lat, lon, clean(dept || ''), clean(commune)].join('\t') + '\n';
      ...
      // Code postal FACULTATIF (21/09/2026) : un code vide n'est pas indexé, il ne correspondrait à aucune saisie.
      for(var i = 0; i < cps.length; i++) if(cps[i]) addEntry(cps[i].toLowerCase(), TYPE_CP, placeId, i, pop);
...
function open(outDir, dataDir, engineInternals){
  ...
  function placeAt(id){
    ...
    return { country: p[0], norm: p[1], name: p[2], cps: p[3].split(','), pop: +p[4], lat: +p[5], lon: +p[6], dept: p[7], commune: p[8] || null };
  }
  ...
  function search(query, limit, preferCountry, lang){
    ...
    for(var j = 0; j < list.length && (out.length < limit || list[j].place === lastPlace); j++){
      ...
      // Même règle que la recherche en mémoire (22/09/2026) : deux fiches de même nom et de même code postal ne sont
      // fusionnées que si elles sont AU MÊME ENDROIT, à moins de FUSION_MAX_KM. Au-delà, ce sont deux villes, et les
      // deux sont proposées — 89,7 % des lieux jusqu'ici masqués étaient à plus de 10 km de celui qui survivait.
      var key = pl.country + '|' + pl.norm + '|' + (cp || '');
      var groupe = seen[key] || (seen[key] = []);
      var prev = null;
      // Test d'enveloppe avant la distance exacte, comme dans lib/trip-engine.js : un groupe peut compter 512 fiches
      // (« Xincun », code CN-30), et un écart de latitude coûte une soustraction là où haversine coûte quatre
      // fonctions trigonométriques. Résultat identique.
      var dLatMax = FUSION_MAX_KM / 111.32;
      for(var gi = 0; gi < groupe.length; gi++){
        var g = groupe[gi];
        if(pl.lat - g.lat > dLatMax || g.lat - pl.lat > dLatMax) continue;
        if(distKm(pl.lat, pl.lon, g.lat, g.lon) <= FUSION_MAX_KM){ prev = g; break; }
      }
      if(prev){
      ...
      var r = { name: pl.name, cp: cp, allCps: pl.cps, pop: pl.pop, lat: pl.lat, lon: pl.lon, dept: pl.dept, country: pl.country, commune: pl.commune || null };
...
module.exports = { normSignature: normSignature, build: build, open: open, dataSignature: dataSignature, mergePreferred: mergePreferred, aliasLangRank: aliasLangRank,
  // topK est exporté depuis le 23/09/2026 : la recherche EN MÉMOIRE de lib/trip-engine.js emploie désormais la même
  // sélection bornée, sans quoi elle trierait des dizaines de milliers de candidats à chaque frappe (« san » : 1,35 s).
  offerAlias: offerAlias, finishAlias: finishAlias, topK: topK };
