...
function build(dataDir, finalDir, engineInternals, log){
  ...
  function flushAliasTexts(){
    ...
    fs.writeSync(aliasTextFd, aliasLines.join(''));
    ...
    fs.writeSync(aliasOffFd, offBuf);
  ...
  flushAliasTexts();
  fs.writeSync(aliasOffFd, u32(aliasBytes));
...
function open(outDir, dataDir, engineInternals){
  ...
  // Plages de numéros de lieux par pays, calculées au premier besoin : pour chaque bloc, recherche exponentielle
  // puis dichotomie de la fin du bloc (lecture des 2 premiers octets de quelques lignes seulement).
...
// Suggestions avec un pays prioritaire (langue d'interface) : ses lieux d'abord, mais pas QUE lui — jusqu'à
// PREFERRED_OTHERS places sont gardées pour les meilleurs lieux des autres pays (par population), afin qu'une grande
// ville étrangère reste proposée. Places non utilisées d'un côté rendues à l'autre. Mêmes règles pour la recherche en
// mémoire du moteur (lib/trip-engine.js).
var PREFERRED_OTHERS = 3;
function mergePreferred(preferredResults, otherResults, limit){
  var nOther = Math.min(otherResults.length, Math.max(PREFERRED_OTHERS, limit - preferredResults.length));
  var nPref = Math.min(preferredResults.length, limit - nOther);
  nOther = Math.min(otherResults.length, limit - nPref);
  return preferredResults.slice(0, nPref).concat(otherResults.slice(0, nOther));
}
...
module.exports = { build: build, open: open, dataSignature: dataSignature, mergePreferred: mergePreferred, aliasLangRank: aliasLangRank,
