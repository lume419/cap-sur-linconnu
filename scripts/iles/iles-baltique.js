// Groupe « baltique » — îles de DK, DE, NL, EE, FI, SE, NO (voir rapport). Généré par scratchpad/zones/balt/build.js.
module.exports = {
 "landmass": {
  "DK": {
   "fallthrough": true,
   "rules": [
    {
     "key": "*",
     "match": {
      "box": [
       [
        55.0233,
        55.0433,
        11.3467,
        11.3867
       ],
       [
        55.9388,
        55.9588,
        10.4244,
        10.4644
       ],
       [
        56.8818,
        56.9018,
        9.0708,
        9.1108
       ],
       [
        54.9538,
        54.9738,
        10.4676,
        10.5076
       ],
       [
        55.0556,
        55.0756,
        10.2303,
        10.2703
       ]
      ]
     },
     "note": "Îles sans ferry pour véhicules de visiteurs (Livø, Tunø, Bjørnø, Vejrø, Hjortø) : chaque lieu isolé."
    },
    {
     "key": "venoe",
     "match": {
      "box": [
       [
        56.5423,
        56.5623,
        8.607,
        8.647
       ]
      ]
     },
     "note": "Venø : Kleppen–Venø."
    },
    {
     "key": "strynoe",
     "match": {
      "box": [
       [
        54.8944,
        54.9144,
        10.5919,
        10.6319
       ]
      ]
     },
     "note": "Strynø : Rudkøbing–Strynø."
    },
    {
     "key": "skaroe",
     "match": {
      "box": [
       [
        54.9971,
        55.0171,
        10.4491,
        10.4891
       ]
      ]
     },
     "note": "Skarø : Svendborg–Skarø."
    },
    {
     "key": "omoe",
     "match": {
      "box": [
       [
        55.1516,
        55.1716,
        11.1354,
        11.1754
       ]
      ]
     },
     "note": "Omø : Stigsnæs–Omø."
    },
    {
     "key": "lyoe",
     "match": {
      "box": [
       [
        55.0329,
        55.0529,
        10.1333,
        10.1733
       ]
      ]
     },
     "note": "Lyø : Faaborg–Lyø."
    },
    {
     "key": "hjarnoe",
     "match": {
      "box": [
       [
        55.8172,
        55.8372,
        10.0571,
        10.0971
       ]
      ]
     },
     "note": "Hjarnø : Snaptun–Hjarnø."
    },
    {
     "key": "endelave",
     "match": {
      "box": [
       [
        55.7473,
        55.7673,
        10.251,
        10.291
       ]
      ]
     },
     "note": "Endelave : Snaptun–Endelave."
    },
    {
     "key": "egholm",
     "match": {
      "box": [
       [
        57.0587,
        57.0787,
        9.8168,
        9.8568
       ]
      ]
     },
     "note": "Egholm (Limfjord, Aalborg) : Egholmfærgen."
    },
    {
     "key": "drejoe",
     "match": {
      "box": [
       [
        54.9597,
        54.9797,
        10.4003,
        10.4403
       ]
      ]
     },
     "note": "Drejø : Svendborg–Skarø–Drejø."
    },
    {
     "key": "barsoe",
     "match": {
      "box": [
       [
        55.1067,
        55.1267,
        9.53,
        9.57
       ]
      ]
     },
     "note": "Barsø : Barsø Landing–Barsø ; point du lieu légèrement hors du contour OSM."
    },
    {
     "key": "askoe",
     "match": {
      "box": [
       [
        54.888,
        54.908,
        11.4721,
        11.5121
       ]
      ]
     },
     "note": "Askø (+ Lilleø par digue) : Bandholm–Askø."
    },
    {
     "key": "anholt",
     "match": {
      "box": [
       [
        56.6936,
        56.7136,
        11.5255,
        11.5655
       ]
      ]
     },
     "note": "Anholt : Grenaa–Anholt."
    },
    {
     "key": "agersoe",
     "match": {
      "box": [
       [
        55.2022,
        55.2222,
        11.1695,
        11.2095
       ]
      ]
     },
     "note": "Agersø : Stigsnæs–Agersø."
    },
    {
     "key": "femoe",
     "match": {
      "box": [
       [
        54.9603,
        54.9904,
        11.5123,
        11.5529
       ]
      ]
     },
     "note": "Femø : Kragenæs–Femø."
    },
    {
     "key": "baagoe",
     "match": {
      "box": [
       [
        55.2956,
        55.3239,
        9.7884,
        9.8446
       ]
      ]
     },
     "note": "Bågø (Baagø) : Assens–Bågø."
    },
    {
     "key": "fejoe",
     "match": {
      "box": [
       [
        54.932,
        54.963,
        11.3667,
        11.4845
       ]
      ]
     },
     "note": "Fejø : Kragenæs–Fejø."
    },
    {
     "key": "avernakoe",
     "match": {
      "box": [
       [
        55.0027,
        55.0424,
        10.237,
        10.3322
       ]
      ]
     },
     "note": "Avernakø + Korshavn (reliées par la digue Drejet) : Faaborg–Avernakø."
    },
    {
     "key": "sejeroe",
     "match": {
      "box": [
       [
        55.8581,
        55.9038,
        11.1166,
        11.2125
       ]
      ]
     },
     "note": "Sejerø : Havnsø–Sejerø."
    },
    {
     "key": "fanoe",
     "match": {
      "box": [
       [
        55.3391,
        55.456,
        8.3479,
        8.4836
       ]
      ]
     },
     "note": "Fanø : FanøLinjen Esbjerg–Nordby."
    },
    {
     "key": "oroe",
     "match": {
      "box": [
       [
        55.7527,
        55.8031,
        11.7747,
        11.8598
       ]
      ]
     },
     "note": "Orø (Isefjord) : Hammer Bakke–Orø."
    },
    {
     "key": "furDk",
     "match": {
      "box": [
       [
        56.8059,
        56.8361,
        8.9854,
        9.075
       ]
      ]
     },
     "note": "Fur (Limfjord) : Branden–Fur, aucun pont."
    },
    {
     "key": "laesoe",
     "match": {
      "box": [
       [
        57.244,
        57.328,
        10.9051,
        11.1533
       ]
      ]
     },
     "note": "Læsø : Læsøfærgen depuis Frederikshavn."
    },
    {
     "key": "samsoe",
     "match": {
      "box": [
       [
        55.7841,
        55.9745,
        10.5126,
        10.67
       ]
      ]
     },
     "note": "Samsø : aucun pont ; Hou–Sælvig et Kalundborg–Ballen."
    },
    {
     "key": "aeroe",
     "match": {
      "box": [
       [
        54.8166,
        54.9663,
        10.2043,
        10.5373
       ]
      ]
     },
     "note": "Ærø (Ærøskøbing, Marstal, Søby) : aucun pont ; Ærøfærgerne."
    }
   ]
  },
  "DE": {
   "default": "continental",
   "rules": [
    {
     "key": "*",
     "match": {
      "box": [
       [
        53.7601,
        53.7801,
        7.6748,
        7.7148
       ],
       [
        54.2324,
        54.2524,
        13.8907,
        13.9307
       ],
       [
        54.5408,
        54.5608,
        8.7988,
        8.8388
       ],
       [
        53.7355,
        53.7555,
        7.4617,
        7.5017
       ],
       [
        54.1714,
        54.1914,
        7.8663,
        7.9063
       ],
       [
        54.666,
        54.686,
        8.6837,
        8.7237
       ],
       [
        54.1943,
        54.2143,
        13.7505,
        13.7905
       ],
       [
        53.3767,
        53.3847,
        8.4814,
        8.4974
       ]
      ]
     },
     "note": "Îles sans voitures ou sans liaison publique (Langeoog, Spiekeroog, Helgoland, Oland et Nordstrandischmoor — lorries —, Strohauser Plate, Ruden, Greifswalder Oie) : chaque lieu isolé."
    },
    {
     "key": "wangerooge",
     "match": {
      "box": [
       [
        53.7777,
        53.8005,
        7.8324,
        7.9207
       ]
      ]
     },
     "note": "Wangerooge : sans voitures — isolée."
    },
    {
     "key": "norderney",
     "match": {
      "box": [
       [
        53.698,
        53.72,
        7.1372,
        7.2517
       ]
      ]
     },
     "note": "Norderney : Reederei Frisia Norddeich–Norderney (voitures admises)."
    },
    {
     "key": "juist",
     "match": {
      "box": [
       [
        53.6655,
        53.6879,
        6.9431,
        7.0158
       ]
      ]
     },
     "note": "Juist : sans voitures (Frisia passagers) — isolée."
    },
    {
     "key": "groede",
     "match": {
      "box": [
       [
        54.6217,
        54.6433,
        8.6967,
        8.7375
       ]
      ]
     },
     "note": "Hallig Gröde : passagers seulement — isolée."
    },
    {
     "key": "kirr",
     "match": {
      "box": [
       [
        54.4163,
        54.428,
        12.6916,
        12.7297
       ]
      ]
     },
     "note": "Kirr (Barther Bodden) : aucun pont ni ferry public — isolée."
    },
    {
     "key": "baltrum",
     "match": {
      "box": [
       [
        53.7173,
        53.7384,
        7.3458,
        7.4047
       ]
      ]
     },
     "note": "Baltrum : sans voitures — isolée."
    },
    {
     "key": "borkum",
     "match": {
      "box": [
       [
        53.5543,
        53.6136,
        6.6715,
        6.7732
       ]
      ]
     },
     "note": "Borkum : AG Ems Emden–Borkum (voitures admises)."
    },
    {
     "key": "amrum",
     "match": {
      "box": [
       [
        54.617,
        54.6897,
        8.313,
        8.4094
       ]
      ]
     },
     "note": "Amrum : W.D.R. Dagebüll–Wittdün (via Wyk)."
    },
    {
     "key": "hiddensee",
     "match": {
      "box": [
       [
        54.5131,
        54.6025,
        13.0667,
        13.1483
       ]
      ]
     },
     "note": "Hiddensee : île sans voitures (ferries passagers seulement) — isolée."
    },
    {
     "key": "hooge",
     "match": {
      "box": [
       [
        54.5492,
        54.5855,
        8.5063,
        8.5825
       ]
      ]
     },
     "note": "Hallig Hooge : W.D.R. passagers, voitures de résidents seulement — isolée."
    },
    {
     "key": "langeness",
     "match": {
      "box": [
       [
        54.6081,
        54.6592,
        8.4967,
        8.6762
       ]
      ]
     },
     "note": "Hallig Langeneß : W.D.R. passagers, véhicules de résidents seulement ; lorry (Lorenzenweg) — isolée."
    },
    {
     "key": "foehr",
     "match": {
      "box": [
       [
        54.6699,
        54.7413,
        8.3861,
        8.587
       ]
      ]
     },
     "note": "Föhr : W.D.R. Dagebüll–Wyk."
    },
    {
     "key": "pellworm",
     "match": {
      "box": [
       [
        54.4852,
        54.5591,
        8.5742,
        8.713
       ]
      ]
     },
     "note": "Pellworm : NPDG Strucklahnungshörn–Pellworm."
    },
    {
     "key": "sylt",
     "match": {
      "box": [
       [
        54.7473,
        55.0546,
        8.2678,
        8.4692
       ]
      ]
     },
     "note": "Sylt : aucun pont routier (digue Hindenburgdamm ferroviaire seulement) ; train-auto Niebüll–Westerland ou ferry Rømø–List."
    }
   ]
  },
  "EE": {
   "default": "continental",
   "rules": [
    {
     "key": "*",
     "match": {
      "box": [
       [
        58.3658,
        58.3858,
        21.8294,
        21.8694
       ],
       [
        59.3067,
        59.3433,
        23.9133,
        23.9867
       ],
       [
        59.5349,
        59.5809,
        25.1958,
        25.294
       ],
       [
        58.2038,
        58.2238,
        24.0998,
        24.1398
       ],
       [
        58.6233,
        58.6433,
        22.9967,
        23.0367
       ],
       [
        58.9286,
        58.9486,
        23.365,
        23.405
       ],
       [
        58.6244,
        58.6444,
        23.4172,
        23.4572
       ],
       [
        58.1347,
        58.1547,
        22.4525,
        22.4925
       ]
      ]
     },
     "note": "Îlots sans ferry pour véhicules (Pakri, Vilsandi, Manilaid, Rammu, Kõinastu, Kesselaid, Vahase, Hobulaid, Koipsi) : chaque lieu isolé."
    },
    {
     "key": "abruka",
     "match": {
      "box": [
       [
        58.1556,
        58.1756,
        22.4931,
        22.5331
       ]
      ]
     },
     "note": "Abruka : Roomassaare–Abruka."
    },
    {
     "key": "ruhnu",
     "match": {
      "box": [
       [
        57.7872,
        57.8103,
        23.2094,
        23.2683
       ]
      ]
     },
     "note": "Ruhnu."
    },
    {
     "key": "piirissaar",
     "match": {
      "box": [
       [
        58.3592,
        58.3867,
        27.4944,
        27.5481
       ]
      ]
     },
     "note": "Piirissaar (lac Peïpous)."
    },
    {
     "key": "naissaar",
     "match": {
      "box": [
       [
        59.5308,
        59.5683,
        24.502,
        24.5522
       ]
      ]
     },
     "note": "Naissaar : bateaux passagers saisonniers seulement — isolée."
    },
    {
     "key": "prangli",
     "match": {
      "box": [
       [
        59.6127,
        59.6447,
        24.9881,
        25.0321
       ]
      ]
     },
     "note": "Prangli : Leppneeme–Prangli."
    },
    {
     "key": "kihnu",
     "match": {
      "box": [
       [
        58.1072,
        58.1583,
        23.9544,
        24.0375
       ]
      ]
     },
     "note": "Kihnu : Munalaid–Kihnu."
    },
    {
     "key": "vormsi",
     "match": {
      "box": [
       [
        58.962,
        59.0392,
        23.1208,
        23.3793
       ]
      ]
     },
     "note": "Vormsi : Rohuküla–Sviby."
    },
    {
     "key": "hiiumaa",
     "match": {
      "box": [
       [
        58.6789,
        59.0919,
        22.038,
        23.0767
       ]
      ]
     },
     "note": "Hiiumaa : Rohuküla–Heltermaa (TS Laevad), Sõru–Triigi vers Saaremaa."
    },
    {
     "key": "saaremaa",
     "match": {
      "box": [
       [
        57.9071,
        58.685,
        21.8586,
        23.4244
       ]
      ]
     },
     "note": "Saaremaa + Muhu (chaussée Väinatamm) : ferry Virtsu–Kuivastu (TS Laevad)."
    }
   ]
  },
  "NL": {
   "fallthrough": true,
   "rules": [
    {
     "key": "*",
     "match": {
      "near": [
       {
        "name": "Tiengemeten",
        "lat": 51.7417,
        "lon": 4.3181,
        "km": 0.7
       }
      ]
     },
     "note": "Tiengemeten (Haringvliet) : aucun pont ; bac pour piétons et cyclistes seulement (véhicules des gestionnaires), isolée. Les cinq îles Wadden habitées restent gérées par la logique existante (champ région = commune, vérifié : aucun lieu d'une commune Wadden hors de son île, aucun lieu insulaire rangé ailleurs)."
    }
   ]
  },
  "NO": {
   "default": "continental",
   "rules": [
    {
     "key": "*",
     "match": {
      "box": [
       [
        71.0422,
        71.0727,
        23.9748,
        24.0354
       ],
       [
        70.995,
        71.0151,
        23.877,
        23.9288
       ],
       [
        71.012,
        71.032,
        23.9885,
        24.0285
       ],
       [
        71.0666,
        71.0975,
        24.777,
        24.8236
       ],
       [
        69.9217,
        69.9433,
        21.6967,
        21.7414
       ],
       [
        71.0677,
        71.0877,
        24.682,
        24.722
       ],
       [
        70.3303,
        70.3503,
        22.8163,
        22.8563
       ],
       [
        70.7821,
        70.8021,
        27.1882,
        27.2282
       ],
       [
        70.3238,
        70.3438,
        21.4307,
        21.4707
       ],
       [
        69.9631,
        69.9831,
        20.6454,
        20.6854
       ],
       [
        70.34,
        70.36,
        21.78,
        21.82
       ],
       [
        70.9883,
        71.0083,
        24.0965,
        24.1365
       ],
       [
        71.0709,
        71.0944,
        23.9945,
        24.0782
       ],
       [
        70.308,
        70.328,
        22.8483,
        22.8883
       ],
       [
        66.4733,
        66.4933,
        12.88,
        12.92
       ],
       [
        64.4733,
        64.4933,
        11.2133,
        11.2533
       ],
       [
        59.74,
        59.76,
        5.1967,
        5.2367
       ],
       [
        58.6019,
        58.6219,
        9.0411,
        9.0811
       ],
       [
        63.76,
        63.7879,
        8.8406,
        8.9
       ],
       [
        63.6627,
        63.6707,
        9.0253,
        9.0413
       ],
       [
        68.2699,
        68.2899,
        14.9121,
        14.9521
       ],
       [
        60.4394,
        60.4594,
        4.9028,
        4.9428
       ],
       [
        65.7876,
        65.81,
        12.5133,
        12.5597
       ],
       [
        61.49,
        61.51,
        5.2133,
        5.2533
       ],
       [
        66.4067,
        66.4267,
        13.03,
        13.07
       ],
       [
        66.54,
        66.56,
        12.83,
        12.87
       ],
       [
        58.99,
        59.0254,
        9.767,
        9.8367
       ],
       [
        70.0031,
        70.0231,
        21.6231,
        21.6631
       ],
       [
        67.395,
        67.415,
        13.879,
        13.919
       ],
       [
        68.9067,
        68.9267,
        14.83,
        14.87
       ],
       [
        67.046,
        67.066,
        13.7589,
        13.7989
       ],
       [
        68.09,
        68.11,
        13.7133,
        13.7533
       ],
       [
        66.5749,
        66.5977,
        12.2076,
        12.2611
       ],
       [
        59.4279,
        59.4479,
        5.0702,
        5.1102
       ],
       [
        68.7233,
        68.7433,
        16.63,
        16.67
       ],
       [
        66.64,
        66.6752,
        13.047,
        13.1033
       ],
       [
        60.8233,
        60.8433,
        5.18,
        5.22
       ],
       [
        58.2031,
        58.2231,
        6.5582,
        6.5982
       ],
       [
        64.94,
        64.96,
        10.9467,
        10.9867
       ],
       [
        66.4067,
        66.4267,
        13.9467,
        13.9867
       ],
       [
        65.1075,
        65.1275,
        12.3963,
        12.4363
       ],
       [
        66.6067,
        66.6267,
        12.98,
        13.02
       ],
       [
        58.6091,
        58.6291,
        9.0758,
        9.1158
       ],
       [
        69.0233,
        69.0433,
        16.5133,
        16.5533
       ],
       [
        63.5567,
        63.5767,
        8.4133,
        8.4533
       ],
       [
        69.0127,
        69.0207,
        17.1587,
        17.1747
       ],
       [
        68.9067,
        68.9267,
        14.9133,
        14.9533
       ],
       [
        67.64,
        67.66,
        15.0467,
        15.0867
       ],
       [
        63.2233,
        63.2433,
        8.1967,
        8.2367
       ],
       [
        65.7567,
        65.7933,
        12.4467,
        12.5033
       ],
       [
        63.646,
        63.654,
        8.7253,
        8.7413
       ],
       [
        66.7518,
        66.7718,
        12.4624,
        12.5024
       ],
       [
        66.3537,
        66.3767,
        12.5467,
        12.6283
       ],
       [
        58.6226,
        58.6426,
        9.1097,
        9.1497
       ],
       [
        60.3233,
        60.3433,
        4.88,
        4.92
       ],
       [
        59.9567,
        59.9767,
        5.8967,
        5.9367
       ],
       [
        57.9807,
        58.0007,
        7.5737,
        7.6137
       ],
       [
        63.5733,
        63.5933,
        8.4467,
        8.5033
       ],
       [
        61.1233,
        61.1433,
        4.7633,
        4.8033
       ],
       [
        60.9293,
        60.9373,
        4.8087,
        4.8247
       ],
       [
        67.4233,
        67.4433,
        13.9633,
        14.0033
       ],
       [
        60.1322,
        60.1522,
        5.1639,
        5.2039
       ],
       [
        61.3733,
        61.3933,
        4.9633,
        5.0033
       ],
       [
        60.1067,
        60.1267,
        5.0467,
        5.0867
       ],
       [
        63.7118,
        63.7318,
        8.8588,
        8.8988
       ],
       [
        70.09,
        70.11,
        19.5133,
        19.5533
       ],
       [
        66.4733,
        66.4933,
        12.93,
        12.97
       ],
       [
        64.4567,
        64.4767,
        11.2467,
        11.2867
       ],
       [
        68.8643,
        68.8933,
        14.8633,
        14.9441
       ],
       [
        57.99,
        58.01,
        7.3467,
        7.3867
       ],
       [
        58.996,
        59.004,
        11.042,
        11.058
       ],
       [
        70.103,
        70.1267,
        19.3437,
        19.3867
       ],
       [
        66.1567,
        66.1767,
        12.3633,
        12.4033
       ],
       [
        68.24,
        68.26,
        14.78,
        14.82
       ],
       [
        59.19,
        59.21,
        10.7633,
        10.8033
       ],
       [
        66.5733,
        66.5933,
        13.03,
        13.07
       ],
       [
        59.0067,
        59.0267,
        11.08,
        11.12
       ],
       [
        59.6811,
        59.7011,
        5.11,
        5.15
       ],
       [
        69.0636,
        69.0933,
        16.48,
        16.5269
       ],
       [
        69.04,
        69.06,
        16.48,
        16.52
       ],
       [
        68.7962,
        68.8276,
        14.753,
        14.8367
       ],
       [
        67.3734,
        67.3934,
        14.2346,
        14.2746
       ],
       [
        61.19,
        61.2145,
        4.8114,
        4.8867
       ],
       [
        59.5797,
        59.5997,
        5.1301,
        5.1701
       ],
       [
        66.5233,
        66.5433,
        12.23,
        12.27
       ],
       [
        61.6567,
        61.6767,
        4.83,
        4.87
       ],
       [
        67.2672,
        67.2872,
        13.9836,
        14.0236
       ],
       [
        68.2259,
        68.2459,
        14.827,
        14.867
       ],
       [
        62.8808,
        62.9008,
        6.8028,
        6.8428
       ],
       [
        66.0717,
        66.0917,
        12.5654,
        12.6054
       ],
       [
        63.6567,
        63.6767,
        8.5467,
        8.5867
       ],
       [
        64.3195,
        64.3395,
        10.3974,
        10.4374
       ],
       [
        65.0357,
        65.0557,
        12.1469,
        12.1869
       ],
       [
        63.44,
        63.46,
        8.2967,
        8.3367
       ],
       [
        66.7439,
        66.7639,
        13.2399,
        13.2799
       ],
       [
        66.39,
        66.41,
        13.08,
        13.12
       ],
       [
        63.6233,
        63.6433,
        9.0133,
        9.0533
       ],
       [
        67.0652,
        67.0852,
        13.8357,
        13.8757
       ],
       [
        67.9758,
        67.9958,
        15.4514,
        15.4914
       ],
       [
        58.5759,
        58.5959,
        9.0197,
        9.0597
       ],
       [
        67.1827,
        67.2027,
        14.4895,
        14.5295
       ],
       [
        65.0548,
        65.0748,
        11.6774,
        11.7174
       ],
       [
        68.1964,
        68.2164,
        16.392,
        16.432
       ],
       [
        64.2862,
        64.3062,
        10.2755,
        10.3155
       ],
       [
        62.0078,
        62.0278,
        5.1854,
        5.2254
       ],
       [
        60.5443,
        60.5643,
        4.9853,
        5.0253
       ],
       [
        62.0446,
        62.0526,
        5.3106,
        5.3266
       ],
       [
        67.0318,
        67.0518,
        13.9414,
        13.9814
       ],
       [
        58.9818,
        58.9898,
        5.7457,
        5.7617
       ],
       [
        58.9682,
        59.0005,
        5.788,
        5.8566
       ],
       [
        68.8179,
        68.8516,
        14.735,
        14.7758
       ],
       [
        70.1305,
        70.1544,
        23.2101,
        23.288
       ],
       [
        58.1367,
        58.1567,
        8.2352,
        8.2752
       ],
       [
        68.1616,
        68.1816,
        14.6596,
        14.6996
       ],
       [
        59.8744,
        59.9007,
        10.6736,
        10.7426
       ],
       [
        58.9834,
        58.9998,
        10.0413,
        10.0603
       ],
       [
        58.9917,
        58.9947,
        9.8486,
        9.8546
       ],
       [
        59.01,
        59.0229,
        10.0942,
        10.1149
       ],
       [
        59.0279,
        59.0309,
        9.8344,
        9.8404
       ],
       [
        59.1388,
        59.172,
        10.3343,
        10.3745
       ],
       [
        59.3687,
        59.3887,
        10.5063,
        10.5463
       ]
      ]
     },
     "note": "Îles habitées ou îlots sans pont, tunnel ni bac pour véhicules (bateaux passagers ou aucune desserte) : chaque lieu isolé."
    },
    {
     "key": "utsira",
     "match": {
      "box": [
       [
        59.2957,
        59.3157,
        4.8662,
        4.9062
       ]
      ]
     },
     "note": "Utsira : Haugesund–Utsira."
    },
    {
     "key": "torget",
     "match": {
      "box": [
       [
        65.4576,
        65.4776,
        12.1133,
        12.1533
       ]
      ]
     },
     "note": "Torget (Brønnøy) : Brønnøysund–Sauren/Stortorgnes."
    },
    {
     "key": "stott",
     "match": {
      "box": [
       [
        66.9148,
        66.9348,
        13.4233,
        13.4633
       ]
      ]
     },
     "note": "Støtt (Innerstøtt) : même ligne."
    },
    {
     "key": "sekken",
     "match": {
      "box": [
       [
        62.6567,
        62.6767,
        7.23,
        7.27
       ]
      ]
     },
     "note": "Sekken : Molde–Sekken."
    },
    {
     "key": "skatoy",
     "match": {
      "box": [
       [
        58.84,
        58.86,
        9.4967,
        9.5367
       ]
      ]
     },
     "note": "Skåtøy (Kragerø) : Kragerø–Skåtøy–Stabbestad."
    },
    {
     "key": "sandsoya",
     "match": {
      "box": [
       [
        62.2414,
        62.2614,
        5.4592,
        5.4992
       ]
      ]
     },
     "note": "Sandsøya (Sande) : Larsnes–Åram–Voksa–Kvamsøya."
    },
    {
     "key": "hisaroy",
     "match": {
      "box": [
       [
        60.99,
        61.01,
        4.98,
        5.02
       ]
      ]
     },
     "note": "Hisarøy (Gulen) : Hisarøy–Mjånes."
    },
    {
     "key": "leroy",
     "match": {
      "box": [
       [
        60.2246,
        60.2446,
        5.1572,
        5.1972
       ]
      ]
     },
     "note": "Lerøy (Øygarden) : Klokkarvik–Lerøy–Bjelkarøy–Hjellestad."
    },
    {
     "key": "leksa",
     "match": {
      "box": [
       [
        63.5655,
        63.5855,
        9.3083,
        9.3483
       ]
      ]
     },
     "note": "Leksa : Garten–Storfosna–Leksa–Værnes."
    },
    {
     "key": "karlsoy",
     "match": {
      "box": [
       [
        69.9927,
        70.0127,
        19.8694,
        19.9094
       ]
      ]
     },
     "note": "Karlsøya : Hansnes–Karlsøy–Vannøy."
    },
    {
     "key": "finnoyaAlesund",
     "match": {
      "box": [
       [
        62.79,
        62.81,
        6.48,
        6.52
       ]
      ]
     },
     "note": "Finnøya (Ålesund) : Småge–Orta–Finnøya–Sandøya–Ona."
    },
    {
     "key": "bolga",
     "match": {
      "box": [
       [
        66.7951,
        66.8151,
        13.2142,
        13.2542
       ]
      ]
     },
     "note": "Bolga : même ligne."
    },
    {
     "key": "traena",
     "match": {
      "box": [
       [
        66.4916,
        66.5116,
        12.0765,
        12.1165
       ]
      ]
     },
     "note": "Træna (Husøya) : Stokkvågen–Træna."
    },
    {
     "key": "mausund",
     "match": {
      "box": [
       [
        63.8636,
        63.8716,
        8.6557,
        8.6717
       ]
      ]
     },
     "note": "Mausund (Frøya) : Sula–Mausund–Dyrøy ; point hors contour OSM."
    },
    {
     "key": "sulaFroya",
     "match": {
      "box": [
       [
        63.8352,
        63.8552,
        8.4386,
        8.4786
       ]
      ]
     },
     "note": "Sula (Frøya) : Sula–Mausund–Dyrøy."
    },
    {
     "key": "andabeloy",
     "match": {
      "box": [
       [
        58.222,
        58.242,
        6.6521,
        6.6921
       ]
      ]
     },
     "note": "Andabeløy : Abelnes–Andabeløy."
    },
    {
     "key": "rebbenesoya",
     "match": {
      "box": [
       [
        70.0205,
        70.0405,
        18.6983,
        18.7383
       ]
      ]
     },
     "note": "Rebbenesøya : Mikkelvik–Bromnes."
    },
    {
     "key": "kvitsoy",
     "match": {
      "box": [
       [
        59.0507,
        59.0723,
        5.3799,
        5.4225
       ]
      ]
     },
     "note": "Kvitsøy : Mekjarvik–Kvitsøy."
    },
    {
     "key": "vengsoya",
     "match": {
      "box": [
       [
        69.8212,
        69.8413,
        18.5056,
        18.5884
       ]
      ]
     },
     "note": "Vengsøya : Bellvik–Vengsøy."
    },
    {
     "key": "vandve",
     "match": {
      "box": [
       [
        66.1567,
        66.1804,
        12.316,
        12.37
       ]
      ]
     },
     "note": "Vandve : Solfjellsjøen–Vandve."
    },
    {
     "key": "huglo",
     "match": {
      "box": [
       [
        59.8232,
        59.8737,
        5.5451,
        5.5922
       ]
      ]
     },
     "note": "Huglo (Stord) : Jektevik–Nordhuglo–Hodnanes."
    },
    {
     "key": "sorburoya",
     "match": {
      "box": [
       [
        63.9653,
        63.9853,
        9.0394,
        9.0794
       ],
       [
        63.7046,
        63.7126,
        8.5009,
        8.5169
       ]
      ]
     },
     "note": "Sørburøya + Nordøya (Frøya) : Dyrøy–Gjæsingen–Sørburøy–Sauøya–Halten."
    },
    {
     "key": "nesoyaRodoy",
     "match": {
      "box": [
       [
        66.581,
        66.61,
        12.6274,
        12.67
       ]
      ]
     },
     "note": "Nesøya/Nordnesøy (Rødøy) : Nordnesøy–Kilboghamn."
    },
    {
     "key": "arnoyGildeskal",
     "match": {
      "box": [
       [
        67.1263,
        67.1632,
        13.9469,
        14.0186
       ]
      ]
     },
     "note": "Sørarnøya + Nordarnøya (Gildeskål, pont) : Sund–Horsdal–Sørarnøy."
    },
    {
     "key": "losna",
     "match": {
      "box": [
       [
        61.1067,
        61.1307,
        5.0467,
        5.1113
       ]
      ]
     },
     "note": "Losna (Solund) : escale de la ligne Rysjedalsvika–Rutledal–Krakhella–Losna."
    },
    {
     "key": "barmoya",
     "match": {
      "box": [
       [
        61.9882,
        62.0091,
        5.2658,
        5.3181
       ]
      ]
     },
     "note": "Barmøya (Stad) : Barmen–Barmsund."
    },
    {
     "key": "rost",
     "match": {
      "box": [
       [
        67.5067,
        67.5275,
        12.0633,
        12.1376
       ]
      ]
     },
     "note": "Røst : Bodø–Værøy–Røst–Moskenes."
    },
    {
     "key": "hugla",
     "match": {
      "box": [
       [
        66.1733,
        66.2117,
        12.93,
        12.983
       ]
      ]
     },
     "note": "Hugla : Nesna–Nesnaøyene."
    },
    {
     "key": "fjelberg",
     "match": {
      "box": [
       [
        59.7067,
        59.752,
        5.6633,
        5.7316
       ]
      ]
     },
     "note": "Fjelbergøya + Borgundøya (Kvinnherad) : Fjelberg–Sydnes–Utbjoa."
    },
    {
     "key": "fedje",
     "match": {
      "box": [
       [
        60.7461,
        60.8267,
        4.6949,
        4.8033
       ]
      ]
     },
     "note": "Fedje : Fedje–Sævrøy."
    },
    {
     "key": "storfosna",
     "match": {
      "box": [
       [
        63.6627,
        63.6778,
        9.3753,
        9.4328
       ]
      ]
     },
     "note": "Storfosna : Garten–Storfosna."
    },
    {
     "key": "joa",
     "match": {
      "box": [
       [
        64.6485,
        64.6767,
        11.23,
        11.286
       ]
      ]
     },
     "note": "Jøa : Ølhammeren–Seierstad."
    },
    {
     "key": "sandoyaPorsgrunn",
     "match": {
      "box": [
       [
        59.0385,
        59.0607,
        9.7078,
        9.7493
       ]
      ]
     },
     "note": "Sandøya (Porsgrunn) : Brevik–Sandøya."
    },
    {
     "key": "uloya",
     "match": {
      "box": [
       [
        69.7785,
        69.91,
        20.5099,
        20.7016
       ]
      ]
     },
     "note": "Uløya : Rotsund–Havnnes–Uløybukt."
    },
    {
     "key": "ombo",
     "match": {
      "box": [
       [
        59.2233,
        59.2767,
        5.93,
        6.0994
       ]
      ]
     },
     "note": "Ombo : Ryfylkeferjen."
    },
    {
     "key": "varaldsoy",
     "match": {
      "box": [
       [
        60.0733,
        60.11,
        5.9633,
        6.0367
       ]
      ]
     },
     "note": "Varaldsøy : Gjermundshamn–Varaldsøy–Årsnes."
    },
    {
     "key": "husevagoy",
     "match": {
      "box": [
       [
        61.896,
        61.9234,
        5.0237,
        5.0913
       ]
      ]
     },
     "note": "Husevågøy : Måløy–Husevågøy–Oldeide."
    },
    {
     "key": "lokta",
     "match": {
      "box": [
       [
        66.139,
        66.1933,
        12.68,
        12.82
       ]
      ]
     },
     "note": "Løkta : Sandnessjøen–Dønna–Løkta."
    },
    {
     "key": "leka",
     "match": {
      "box": [
       [
        65.0567,
        65.11,
        11.6467,
        11.82
       ]
      ]
     },
     "note": "Leka : Skei–Gutvik."
    },
    {
     "key": "onoy",
     "match": {
      "box": [
       [
        66.39,
        66.4433,
        12.8225,
        12.8867
       ]
      ]
     },
     "note": "Onøya + Lurøya (pont) : Stokkvågen–Onøy–Sleneset–Lovund."
    },
    {
     "key": "lovund",
     "match": {
      "box": [
       [
        66.3549,
        66.3801,
        12.3401,
        12.3849
       ]
      ]
     },
     "note": "Lovund : Stokkvågen–Onøy–Sleneset–Lovund."
    },
    {
     "key": "ytteroya",
     "match": {
      "box": [
       [
        63.7745,
        63.8132,
        11.097,
        11.2022
       ]
      ]
     },
     "note": "Ytterøya : Levanger–Hokstad."
    },
    {
     "key": "vaerlandet",
     "match": {
      "box": [
       [
        61.2748,
        61.3267,
        4.6113,
        4.7533
       ]
      ]
     },
     "note": "Værlandet + Bulandet (Askvoll, ponts) : Askvoll–Fure–Værlandet."
    },
    {
     "key": "reinoya",
     "match": {
      "box": [
       [
        69.8406,
        69.9626,
        19.5126,
        19.7875
       ]
      ]
     },
     "note": "Reinøya : Hansnes–Stakkvik (Reinøy)."
    },
    {
     "key": "laukoya",
     "match": {
      "box": [
       [
        70.1217,
        70.1508,
        20.7781,
        20.8296
       ],
       [
        70.0567,
        70.0906,
        20.7829,
        20.87
       ],
       [
        70.102,
        70.122,
        20.7518,
        20.7918
       ]
      ]
     },
     "note": "Laukøya : même ligne."
    },
    {
     "key": "seiland",
     "match": {
      "box": [
       [
        70.5345,
        70.5545,
        23.1283,
        23.1683
       ],
       [
        70.2685,
        70.2885,
        23.1025,
        23.1425
       ],
       [
        70.5136,
        70.5491,
        23.3078,
        23.412
       ],
       [
        70.4595,
        70.4795,
        23.5985,
        23.6385
       ],
       [
        70.5296,
        70.5496,
        23.5166,
        23.5566
       ]
      ]
     },
     "note": "Seiland : aucune liaison pour véhicules (bateaux passagers) — isolée."
    },
    {
     "key": "vaeroy",
     "match": {
      "box": [
       [
        67.6336,
        67.7033,
        12.5738,
        12.7415
       ]
      ]
     },
     "note": "Værøy : Bodø–Værøy–Røst–Moskenes."
    },
    {
     "key": "meloya",
     "match": {
      "box": [
       [
        66.8173,
        66.8348,
        13.3475,
        13.5197
       ]
      ]
     },
     "note": "Meløya : Ørnes–Vassdalsvik–Meløysund–Bolga–Støtt."
    },
    {
     "key": "tomma",
     "match": {
      "box": [
       [
        66.2155,
        66.2933,
        12.7133,
        12.9033
       ]
      ]
     },
     "note": "Tomma : Nesna–Nesnaøyene."
    },
    {
     "key": "atloy",
     "match": {
      "box": [
       [
        61.3067,
        61.3767,
        4.8742,
        5.0367
       ]
      ]
     },
     "note": "Atløy : Askvoll–Gjervik."
    },
    {
     "key": "hidra",
     "match": {
      "box": [
       [
        58.2067,
        58.2493,
        6.511,
        6.6174
       ]
      ]
     },
     "note": "Hidra : Launes–Kvellandstrand."
    },
    {
     "key": "gossa",
     "match": {
      "box": [
       [
        62.7733,
        62.86,
        6.7467,
        6.938
       ]
      ]
     },
     "note": "Gossa (Aukra) : Aukra–Hollingsholmen."
    },
    {
     "key": "donna",
     "match": {
      "box": [
       [
        66.1233,
        66.1466,
        12.4967,
        12.5754
       ],
       [
        66.1639,
        66.1839,
        12.5248,
        12.5648
       ],
       [
        66.0985,
        66.1154,
        12.4803,
        12.4972
       ],
       [
        66.0233,
        66.0433,
        12.3467,
        12.3867
       ],
       [
        66.0739,
        66.0939,
        12.346,
        12.386
       ],
       [
        66.1127,
        66.1207,
        12.592,
        12.608
       ],
       [
        66.0733,
        66.0955,
        12.4284,
        12.47
       ],
       [
        66.1694,
        66.1894,
        12.4813,
        12.5213
       ],
       [
        66.0546,
        66.0746,
        12.377,
        12.417
       ]
      ]
     },
     "note": "Dønna : Sandnessjøen–Dønna–Løkta."
    },
    {
     "key": "soroya",
     "match": {
      "box": [
       [
        70.4764,
        70.8212,
        22.0889,
        23.4489
       ]
      ]
     },
     "note": "Sørøya (Hasvik) : Øksfjord–Hasvik."
    },
    {
     "key": "arnoyaSkjervoy",
     "match": {
      "box": [
       [
        70.1266,
        70.1466,
        20.3667,
        20.4067
       ],
       [
        70.2181,
        70.2381,
        20.6225,
        20.6625
       ],
       [
        70.1281,
        70.1481,
        20.7442,
        20.7842
       ],
       [
        70.0514,
        70.0714,
        20.5385,
        20.5785
       ],
       [
        70.0299,
        70.0567,
        20.5653,
        20.6404
       ],
       [
        70.0362,
        70.06,
        20.4133,
        20.4857
       ],
       [
        70.04,
        70.06,
        20.68,
        20.72
       ],
       [
        70.1835,
        70.2035,
        20.5113,
        20.5513
       ],
       [
        70.0541,
        70.0741,
        20.4725,
        20.5125
       ],
       [
        70.1067,
        70.1267,
        20.73,
        20.77
       ],
       [
        70.0688,
        70.0888,
        20.7563,
        20.7963
       ]
      ]
     },
     "note": "Arnøya : Storstein–Nikkeby–Lauksundskaret."
    },
    {
     "key": "vannoya",
     "match": {
      "box": [
       [
        70.0627,
        70.0827,
        19.9621,
        20.0021
       ],
       [
        70.1067,
        70.1267,
        19.98,
        20.02
       ],
       [
        70.0802,
        70.111,
        20.0446,
        20.1277
       ],
       [
        70.1748,
        70.2206,
        19.5542,
        19.6524
       ],
       [
        70.24,
        70.26,
        19.53,
        19.57
       ],
       [
        70.1733,
        70.1933,
        19.7967,
        19.8367
       ],
       [
        70.0733,
        70.0933,
        19.6967,
        19.7367
       ],
       [
        70.04,
        70.06,
        19.8633,
        19.9033
       ],
       [
        70.1233,
        70.1433,
        19.68,
        19.72
       ],
       [
        70.2067,
        70.2376,
        19.7093,
        19.7533
       ],
       [
        70.1342,
        70.1542,
        19.9136,
        19.9536
       ]
      ]
     },
     "note": "Vannøya : Hansnes–Karlsøy–Vannøy."
    },
    {
     "key": "midsund",
     "match": {
      "box": [
       [
        62.7371,
        62.7571,
        6.8933,
        6.9333
       ],
       [
        62.6876,
        62.7433,
        6.676,
        6.7611
       ],
       [
        62.6621,
        62.7032,
        6.6106,
        6.6954
       ],
       [
        62.6638,
        62.6838,
        6.7146,
        6.7546
       ],
       [
        62.64,
        62.66,
        6.5467,
        6.5867
       ],
       [
        62.6471,
        62.6671,
        6.5969,
        6.6369
       ],
       [
        62.6738,
        62.6938,
        6.7504,
        6.7904
       ]
      ]
     },
     "note": "Otrøya + Midøya (Midsund, pont) : Brattvåg–Dryna."
    },
    {
     "key": "solund",
     "match": {
      "box": [
       [
        60.9854,
        61.16,
        4.6461,
        5.0371
       ],
       [
        61.0567,
        61.0767,
        5.0467,
        5.1533
       ],
       [
        61.1733,
        61.1933,
        5.0467,
        5.1033
       ]
      ]
     },
     "note": "Solund (Sula, Steinsundøyna, Daløyna, Ytre Sula, reliées par ponts) : Rysjedalsvika–Rutledal–Krakhella."
    },
    {
     "key": "austevoll",
     "match": {
      "box": [
       [
        59.9738,
        60.1432,
        5.0709,
        5.3247
       ]
      ]
     },
     "note": "Austevoll (Huftarøy, Selbjørn, Stolmen, Hundvåko, Storakalsøy… reliées par ponts) : Krokeide–Hufthamar."
    },
    {
     "key": "tysnes",
     "match": {
      "box": [
       [
        60.0335,
        60.0746,
        5.48,
        5.5816
       ],
       [
        59.994,
        60.0267,
        5.5088,
        5.5533
       ],
       [
        59.9325,
        60.0075,
        5.4575,
        5.5367
       ],
       [
        60.0086,
        60.0525,
        5.4131,
        5.4986
       ],
       [
        59.9418,
        59.9767,
        5.6055,
        5.6533
       ],
       [
        59.9025,
        59.9225,
        5.4631,
        5.5031
       ],
       [
        60.04,
        60.0864,
        5.58,
        5.6978
       ],
       [
        59.99,
        60.0234,
        5.6739,
        5.7533
       ],
       [
        59.9233,
        59.9433,
        5.5967,
        5.6367
       ],
       [
        59.99,
        60.01,
        5.3633,
        5.4033
       ],
       [
        60.0567,
        60.0767,
        5.3967,
        5.4367
       ],
       [
        60.0067,
        60.0267,
        5.38,
        5.42
       ],
       [
        59.9733,
        59.9978,
        5.3967,
        5.4552
       ],
       [
        60.0472,
        60.0672,
        5.342,
        5.382
       ],
       [
        60.0053,
        60.0253,
        5.335,
        5.375
       ],
       [
        60.0733,
        60.0933,
        5.3633,
        5.4033
       ]
      ]
     },
     "note": "Tysnesøya + Reksteren (pont) : aucun lien fixe avec le continent ; Halhjem–Våge."
    }
   ]
  },
  "SE": {
   "fallthrough": true,
   "rules": [
    {
     "key": "faro",
     "match": {
      "cpPrefix": [
       "624 66"
      ]
     },
     "note": "Fårö (code 624 66, 10 lieux, tous sur l’île) : Fårösundsleden depuis Gotland. Placée avant la règle Gotland (codes 62…)."
    },
    {
     "key": "visingso",
     "match": {
      "box": [
       [
        57.99,
        58.11,
        14.26,
        14.4
       ]
      ]
     },
     "note": "Visingsö (lac Vättern) : Visingsöleden depuis Gränna ; boîte vérifiée (le code 560 34 couvre aussi la rive)."
    },
    {
     "key": "ven",
     "match": {
      "cpPrefix": [
       "260 13"
      ]
     },
     "note": "Ven (5 lieux) : île sans voitures pour les visiteurs (Ventrafiken ne prend que les véhicules autorisés) — isolée."
    },
    {
     "key": "ivo",
     "match": {
      "near": [
       {
        "name": "Ivö",
        "lat": 56.1,
        "lon": 14.4,
        "km": 0.5
       }
      ]
     },
     "note": "Ivö (lac Ivösjön) : Ivöleden."
    },
    {
     "key": "holmon",
     "match": {
      "near": [
       {
        "name": "Holmön",
        "lat": 63.8,
        "lon": 20.8667,
        "km": 0.5
       }
      ]
     },
     "note": "Holmön : Holmöleden depuis Norrfjärden."
    },
    {
     "key": "*",
     "match": {
      "box": [
       [
        60.1567,
        60.1767,
        18.6133,
        18.6533
       ],
       [
        59.9293,
        59.9373,
        18.8587,
        18.8747
       ],
       [
        59.846,
        59.854,
        19.1087,
        19.1247
       ],
       [
        59.74,
        59.76,
        19.0967,
        19.1367
       ],
       [
        59.7793,
        59.7873,
        18.992,
        19.008
       ],
       [
        57.5628,
        57.5828,
        11.7636,
        11.8036
       ],
       [
        59.5567,
        59.5767,
        18.68,
        18.72
       ],
       [
        59.1733,
        59.1933,
        18.6494,
        18.6894
       ],
       [
        60.1733,
        60.1933,
        18.48,
        18.52
       ],
       [
        59.4186,
        59.4386,
        18.4486,
        18.4886
       ],
       [
        59.2585,
        59.2665,
        18.742,
        18.758
       ],
       [
        58.9507,
        58.9707,
        18.287,
        18.327
       ],
       [
        59.2793,
        59.2915,
        18.742,
        18.758
       ],
       [
        58.1733,
        58.1933,
        11.38,
        11.42
       ],
       [
        59.2662,
        59.2862,
        18.9078,
        18.9478
       ],
       [
        59.34,
        59.3656,
        18.8786,
        18.92
       ],
       [
        58.34,
        58.36,
        11.2467,
        11.2867
       ],
       [
        57.6091,
        57.6226,
        11.7642,
        11.7822
       ],
       [
        59.4469,
        59.4669,
        18.6597,
        18.6997
       ],
       [
        59.271,
        59.2832,
        18.7253,
        18.7497
       ],
       [
        57.6124,
        57.6207,
        11.7776,
        11.7997
       ],
       [
        57.9238,
        57.9318,
        11.603,
        11.619
       ],
       [
        59.4066,
        59.4266,
        18.3461,
        18.3861
       ],
       [
        59.6169,
        59.6369,
        18.9978,
        19.0378
       ],
       [
        59.39,
        59.41,
        18.8467,
        18.8867
       ],
       [
        59.2543,
        59.2623,
        18.7587,
        18.7747
       ],
       [
        59.34,
        59.36,
        18.7133,
        18.7533
       ],
       [
        59.4393,
        59.4521,
        18.682,
        18.7024
       ],
       [
        59.5528,
        59.5728,
        18.7656,
        18.8056
       ],
       [
        59.5527,
        59.5607,
        18.6492,
        18.6652
       ],
       [
        59.2786,
        59.2986,
        18.8933,
        18.9333
       ],
       [
        59.4169,
        59.4369,
        18.8894,
        18.9294
       ],
       [
        59.5278,
        59.5478,
        18.7492,
        18.7892
       ],
       [
        59.4291,
        59.4365,
        18.5848,
        18.5963
       ],
       [
        58.8706,
        58.8906,
        18.0956,
        18.1356
       ],
       [
        59.1733,
        59.1933,
        18.6633,
        18.7033
       ],
       [
        59.1519,
        59.1719,
        18.6171,
        18.6571
       ],
       [
        57.883,
        57.891,
        11.5742,
        11.5902
       ],
       [
        58.9594,
        58.9794,
        18.3129,
        18.3529
       ],
       [
        59.5942,
        59.6142,
        18.6864,
        18.7264
       ],
       [
        59.4067,
        59.4267,
        18.88,
        18.92
       ],
       [
        59.29,
        59.31,
        18.7633,
        18.8033
       ],
       [
        59.14,
        59.16,
        18.78,
        18.82
       ],
       [
        59.4317,
        59.4517,
        18.8975,
        18.9375
       ],
       [
        59.2858,
        59.3058,
        18.7522,
        18.7922
       ],
       [
        58.8567,
        58.8767,
        11.03,
        11.07
       ],
       [
        59.29,
        59.31,
        17.8633,
        17.9033
       ],
       [
        57.6175,
        57.6375,
        11.7867,
        11.8267
       ],
       [
        58.9056,
        58.9256,
        18.1442,
        18.1842
       ],
       [
        58.0989,
        58.1189,
        11.3417,
        11.3817
       ],
       [
        58.9417,
        58.9617,
        18.2869,
        18.3269
       ],
       [
        57.7021,
        57.7101,
        11.6776,
        11.6936
       ],
       [
        58.7567,
        58.7767,
        11.13,
        11.17
       ],
       [
        59.4575,
        59.4775,
        18.7294,
        18.7694
       ],
       [
        57.79,
        57.81,
        11.6967,
        11.7367
       ],
       [
        59.4067,
        59.4267,
        18.6633,
        18.7033
       ],
       [
        59.6294,
        59.6494,
        18.7619,
        18.8019
       ],
       [
        59.3233,
        59.3475,
        18.8619,
        18.9033
       ],
       [
        59.4189,
        59.4389,
        18.8328,
        18.8728
       ],
       [
        59.6681,
        59.6933,
        18.9967,
        19.0383
       ],
       [
        59.2512,
        59.2598,
        18.3572,
        18.3769
       ],
       [
        58.2293,
        58.2373,
        11.4003,
        11.4163
       ],
       [
        57.8222,
        57.8302,
        11.6813,
        11.6973
       ],
       [
        59.4683,
        59.4883,
        18.7942,
        18.8342
       ],
       [
        59.1567,
        59.1767,
        18.6467,
        18.6867
       ],
       [
        59.4581,
        59.4781,
        18.6167,
        18.6567
       ],
       [
        59.406,
        59.426,
        18.3135,
        18.3535
       ],
       [
        57.596,
        57.604,
        11.7912,
        11.8072
       ],
       [
        59.296,
        59.304,
        18.6753,
        18.6913
       ],
       [
        59.4536,
        59.4772,
        18.7044,
        18.7453
       ],
       [
        57.6353,
        57.6553,
        11.7617,
        11.8017
       ],
       [
        59.3233,
        59.3433,
        17.53,
        17.57
       ],
       [
        59.4603,
        59.4803,
        18.4264,
        18.4664
       ],
       [
        59.396,
        59.416,
        18.8658,
        18.9058
       ],
       [
        57.6379,
        57.6579,
        11.7736,
        11.8136
       ],
       [
        59.4783,
        59.4983,
        18.7025,
        18.7425
       ],
       [
        59.43,
        59.45,
        18.6306,
        18.6706
       ],
       [
        59.446,
        59.454,
        17.5253,
        17.5413
       ],
       [
        59.7293,
        59.7373,
        19.0753,
        19.0913
       ],
       [
        59.2664,
        59.2864,
        18.7717,
        18.8117
       ],
       [
        59.2557,
        59.2637,
        18.7253,
        18.7413
       ],
       [
        59.1886,
        59.21,
        18.6619,
        18.7061
       ],
       [
        59.3877,
        59.3957,
        18.6337,
        18.6497
       ],
       [
        59.3877,
        59.3957,
        18.6906,
        18.7066
       ],
       [
        59.3877,
        59.3957,
        18.6753,
        18.6913
       ],
       [
        59.4306,
        59.4506,
        18.865,
        18.905
       ],
       [
        59.4219,
        59.4447,
        18.8994,
        18.9397
       ],
       [
        59.315,
        59.335,
        18.8467,
        18.8867
       ],
       [
        59.2969,
        59.3169,
        18.8314,
        18.8714
       ],
       [
        59.2831,
        59.3031,
        18.7744,
        18.8144
       ],
       [
        59.5063,
        59.5143,
        18.5012,
        18.5172
       ],
       [
        59.4464,
        59.4664,
        18.7164,
        18.7564
       ],
       [
        58.8258,
        58.8458,
        17.9081,
        17.9481
       ],
       [
        58.9328,
        58.9528,
        18.2192,
        18.2592
       ],
       [
        59.59,
        59.61,
        18.6967,
        18.7367
       ],
       [
        58.8733,
        58.8933,
        18.1133,
        18.1533
       ],
       [
        58.9669,
        58.9869,
        18.3415,
        18.3815
       ],
       [
        58.6456,
        58.6656,
        11.2133,
        11.2533
       ],
       [
        58.6627,
        58.6707,
        11.242,
        11.258
       ],
       [
        58.7543,
        58.7623,
        11.167,
        11.183
       ],
       [
        57.8567,
        57.8767,
        11.58,
        11.62
       ],
       [
        57.8483,
        57.8683,
        11.58,
        11.62
       ],
       [
        57.8622,
        57.8822,
        11.5633,
        11.6033
       ],
       [
        57.8622,
        57.8822,
        11.605,
        11.645
       ],
       [
        57.9983,
        58.0183,
        11.4633,
        11.5033
       ],
       [
        58.0067,
        58.0267,
        11.4633,
        11.5033
       ],
       [
        58.0668,
        58.0748,
        11.4587,
        11.4747
       ],
       [
        58.096,
        58.104,
        11.5753,
        11.5913
       ],
       [
        58.14,
        58.16,
        11.3633,
        11.4033
       ],
       [
        58.1543,
        58.1665,
        11.3753,
        11.3997
       ],
       [
        58.2377,
        58.2457,
        11.8503,
        11.8663
       ],
       [
        58.2275,
        58.2475,
        11.5133,
        11.5533
       ],
       [
        59.1066,
        59.1146,
        18.2865,
        18.3025
       ]
      ]
     },
     "note": "Îles des archipels de Stockholm et de Bohuslän/Göteborg sans pont ni bac routier ouvert aux visiteurs (îles sans voitures, bateaux Waxholmsbolaget/Styrsöbolaget passagers) : chaque lieu isolé."
    },
    {
     "key": "arno",
     "match": {
      "box": [
       [
        59.49,
        59.51,
        17.18,
        17.22
       ]
      ]
     },
     "note": "Arnö (lac Mälaren) : Arnöleden."
    },
    {
     "key": "hogmarso",
     "match": {
      "box": [
       [
        59.6361,
        59.6561,
        18.8172,
        18.8572
       ]
      ]
     },
     "note": "Högmarsö : bac privé Svartnö–Högmarsö."
    },
    {
     "key": "nordo",
     "match": {
      "box": [
       [
        57.7483,
        57.7801,
        11.6132,
        11.6599
       ]
      ]
     },
     "note": "Rörö + Källö-Knippla (Öckerö) : Nordöleden depuis Burö (Hälsö)."
    },
    {
     "key": "rindo",
     "match": {
      "box": [
       [
        59.3869,
        59.41,
        18.3743,
        18.4533
       ]
      ]
     },
     "note": "Rindö : Vaxholmsleden / Oxdjupsleden (aucun pont)."
    },
    {
     "key": "bjorkoOckero",
     "match": {
      "box": [
       [
        57.7222,
        57.76,
        11.6592,
        11.7033
       ]
      ]
     },
     "note": "Björkö (Öckerö) : Björköleden."
    },
    {
     "key": "bohusMalmon",
     "match": {
      "box": [
       [
        58.3293,
        58.3457,
        11.317,
        11.3413
       ]
      ]
     },
     "note": "Bohus-Malmön : Bohus-Malmönleden."
    },
    {
     "key": "lyr",
     "match": {
      "box": [
       [
        58.0608,
        58.0808,
        11.4967,
        11.5367
       ],
       [
        58.0752,
        58.0832,
        11.492,
        11.508
       ],
       [
        58.071,
        58.079,
        11.5253,
        11.5413
       ],
       [
        58.0918,
        58.0998,
        11.5003,
        11.5163
       ]
      ]
     },
     "note": "Lyr : Lyrleden."
    },
    {
     "key": "hamburgo",
     "match": {
      "box": [
       [
        58.5266,
        58.5415,
        11.2587,
        11.2747
       ],
       [
        58.546,
        58.554,
        11.242,
        11.258
       ],
       [
        58.5293,
        58.5373,
        11.242,
        11.258
       ],
       [
        58.5516,
        58.5596,
        11.2503,
        11.2663
       ],
       [
        58.5485,
        58.5515,
        11.2637,
        11.2697
       ]
      ]
     },
     "note": "Hamburgö : Hamburgsundsleden."
    },
    {
     "key": "ockero",
     "match": {
      "box": [
       [
        57.6816,
        57.714,
        11.6378,
        11.6588
       ],
       [
        57.7241,
        57.7441,
        11.6381,
        11.6781
       ]
      ]
     },
     "note": "Hönö + Öckerö + Hälsö (ponts) : Hönöleden depuis Lilla Varholmen."
    },
    {
     "key": "yxlan",
     "match": {
      "box": [
       [
        59.6214,
        59.6414,
        18.8439,
        18.8839
       ],
       [
        59.5692,
        59.5917,
        18.7586,
        18.8086
       ],
       [
        59.6563,
        59.6643,
        18.9328,
        18.9488
       ],
       [
        59.6299,
        59.6379,
        18.8884,
        18.9044
       ],
       [
        59.6069,
        59.6269,
        18.825,
        18.865
       ],
       [
        59.6002,
        59.6112,
        18.8384,
        18.8644
       ],
       [
        59.6421,
        59.6501,
        18.8964,
        18.9124
       ],
       [
        59.6286,
        59.6486,
        18.8661,
        18.9061
       ],
       [
        59.6243,
        59.6323,
        18.882,
        18.898
       ],
       [
        59.5986,
        59.6186,
        18.8122,
        18.8522
       ],
       [
        59.646,
        59.654,
        18.9087,
        18.933
       ]
      ]
     },
     "note": "Yxlan : Furusundsleden."
    },
    {
     "key": "graso",
     "match": {
      "box": [
       [
        60.3233,
        60.3433,
        18.5133,
        18.5617
       ],
       [
        60.3377,
        60.3457,
        18.5087,
        18.5247
       ],
       [
        60.346,
        60.354,
        18.492,
        18.508
       ],
       [
        60.3067,
        60.3267,
        18.5133,
        18.5533
       ],
       [
        60.3733,
        60.3933,
        18.4467,
        18.4867
       ],
       [
        60.3567,
        60.3767,
        18.4633,
        18.5033
       ],
       [
        60.3567,
        60.3767,
        18.4467,
        18.4867
       ],
       [
        60.34,
        60.36,
        18.4633,
        18.5033
       ],
       [
        60.34,
        60.36,
        18.43,
        18.47
       ],
       [
        60.34,
        60.3649,
        18.446,
        18.4867
       ],
       [
        60.3733,
        60.3933,
        18.38,
        18.42
       ]
      ]
     },
     "note": "Gräsö : Gräsöleden depuis Öregrund."
    },
    {
     "key": "orno",
     "match": {
      "box": [
       [
        59.0206,
        59.1232,
        18.3467,
        18.5021
       ]
      ]
     },
     "note": "Ornö : bac Dalarö–Ornö (Ornö Sjötrafik)."
    },
    {
     "key": "blido",
     "match": {
      "box": [
       [
        59.5844,
        59.6125,
        18.8839,
        18.9328
       ],
       [
        59.6275,
        59.6475,
        18.9042,
        18.9442
       ],
       [
        59.6092,
        59.6347,
        18.9436,
        18.9894
       ],
       [
        59.5836,
        59.6058,
        18.8467,
        18.8942
       ],
       [
        59.6269,
        59.6469,
        18.9292,
        18.9692
       ],
       [
        59.605,
        59.6283,
        18.8717,
        18.92
       ],
       [
        59.6167,
        59.6367,
        18.8792,
        18.9192
       ],
       [
        59.5769,
        59.5969,
        18.8311,
        18.8711
       ],
       [
        59.625,
        59.645,
        18.9486,
        18.9886
       ],
       [
        59.5969,
        59.6169,
        18.8522,
        18.8922
       ],
       [
        59.5906,
        59.6106,
        18.8636,
        18.9036
       ],
       [
        59.6214,
        59.6414,
        18.8881,
        18.9281
       ],
       [
        59.6164,
        59.6364,
        18.9053,
        18.9453
       ],
       [
        59.6042,
        59.6242,
        18.8928,
        18.9328
       ],
       [
        59.6383,
        59.6583,
        18.9372,
        18.9772
       ],
       [
        59.5733,
        59.5933,
        18.855,
        18.895
       ]
      ]
     },
     "note": "Blidö : Blidöleden depuis Yxlan."
    },
    {
     "key": "blido",
     "match": {
      "near": [
       {
        "name": "Fagernäs",
        "lat": 59.6503,
        "lon": 18.9411,
        "km": 0.18
       },
       {
        "name": "Lagervik",
        "lat": 59.6467,
        "lon": 18.9333,
        "km": 0.15
       },
       {
        "name": "Bergshyddan",
        "lat": 59.6433,
        "lon": 18.9292,
        "km": 0.31
       }
      ]
     },
     "note": "Blidö : Blidöleden depuis Yxlan. (lieux non isolables par boîte)"
    },
    {
     "key": "ljustero",
     "match": {
      "box": [
       [
        59.4953,
        59.5611,
        18.5967,
        18.7478
       ],
       [
        59.4257,
        59.5059,
        18.5042,
        18.6072
       ]
      ]
     },
     "note": "Ljusterö (nord + sud) : Ljusteröleden."
    }
   ]
  },
  "FI": {
   "default": "continental",
   "rules": [
    {
     "key": "*",
     "match": {
      "box": [
       [
        59.7711,
        59.7911,
        21.3505,
        21.3905
       ],
       [
        60.04,
        60.06,
        22.88,
        22.92
       ],
       [
        60.14,
        60.16,
        22.2467,
        22.2867
       ],
       [
        60.2298,
        60.2498,
        21.9383,
        21.9783
       ],
       [
        60.24,
        60.26,
        21.53,
        21.57
       ],
       [
        60.1386,
        60.1586,
        22.1,
        22.14
       ],
       [
        60.3332,
        60.3641,
        21.6483,
        21.7162
       ],
       [
        59.944,
        59.9675,
        21.7372,
        21.7809
       ],
       [
        60.2067,
        60.2356,
        22.3107,
        22.3833
       ],
       [
        60.4733,
        60.4933,
        21.88,
        21.92
       ],
       [
        60.09,
        60.11,
        21.6467,
        21.6867
       ],
       [
        59.9067,
        59.9267,
        21.8633,
        21.9033
       ],
       [
        60.2949,
        60.3149,
        21.28,
        21.32
       ],
       [
        60.4768,
        60.4968,
        21.7404,
        21.7804
       ],
       [
        60.4039,
        60.4239,
        21.4097,
        21.4497
       ],
       [
        60.231,
        60.251,
        21.3206,
        21.3606
       ],
       [
        60.3641,
        60.3841,
        21.3252,
        21.3652
       ],
       [
        60.2173,
        60.2373,
        21.8002,
        21.8402
       ],
       [
        60.2417,
        60.2617,
        21.7341,
        21.7741
       ],
       [
        59.9415,
        59.9675,
        22.3379,
        22.3814
       ],
       [
        60.0218,
        60.0298,
        22.6707,
        22.6867
       ],
       [
        60.1707,
        60.1907,
        21.478,
        21.518
       ],
       [
        59.89,
        59.91,
        23.33,
        23.37
       ],
       [
        60.2483,
        60.2683,
        21.3742,
        21.4142
       ],
       [
        60.1423,
        60.1623,
        21.286,
        21.326
       ],
       [
        60.0439,
        60.0639,
        21.7791,
        21.8191
       ],
       [
        59.8861,
        59.91,
        23.0967,
        23.1444
       ],
       [
        60.2864,
        60.3127,
        21.5371,
        21.5875
       ],
       [
        60.1733,
        60.1933,
        22.3133,
        22.3533
       ],
       [
        60.4098,
        60.4298,
        21.1981,
        21.2381
       ],
       [
        60.2994,
        60.3194,
        21.2077,
        21.2477
       ],
       [
        60.3631,
        60.3831,
        21.7162,
        21.7562
       ],
       [
        59.8939,
        59.9139,
        23.0083,
        23.0483
       ],
       [
        60.0861,
        60.1061,
        21.5428,
        21.5828
       ],
       [
        60.2078,
        60.2338,
        21.4094,
        21.472
       ],
       [
        60.0772,
        60.0972,
        22.0662,
        22.1062
       ],
       [
        60.115,
        60.135,
        21.8606,
        21.9006
       ],
       [
        60.1806,
        60.2006,
        22.2511,
        22.2911
       ],
       [
        60.4174,
        60.4386,
        21.3558,
        21.4033
       ],
       [
        60.0605,
        60.0805,
        22.0321,
        22.0721
       ],
       [
        60.1633,
        60.1833,
        22.2814,
        22.3214
       ]
      ]
     },
     "note": "Îles de l’archipel sans pont ni bac routier public (desserte par navires de liaison « yhteysalus » ou bateaux privés) : chaque lieu isolé."
    },
    {
     "key": "velkua",
     "match": {
      "box": [
       [
        60.4132,
        60.4572,
        21.5937,
        21.6556
       ]
      ]
     },
     "note": "Velkuanmaa (Naantali) : bac routier Palva – Velkuanmaa."
    },
    {
     "key": "inio",
     "match": {
      "box": [
       [
        60.3677,
        60.4135,
        21.3449,
        21.42
       ]
      ]
     },
     "note": "Iniö (code 23390) : bac routier Heponiemi (Kustavi) – Kannvik."
    },
    {
     "key": "hitis",
     "match": {
      "box": [
       [
        59.844,
        59.9023,
        22.399,
        22.5431
       ]
      ]
     },
     "note": "Hitis/Rosala (codes 25940, 25950) : liaison Kasnäs – Rosala/Hitis."
    },
    {
     "key": "houtskar",
     "match": {
      "box": [
       [
        60.1746,
        60.2844,
        21.253,
        21.4606
       ]
      ]
     },
     "note": "Houtskär (+ Hyppeis, Mossala ; codes 21760, 21770) : bac routier Galtby (Korpo) – Kittuis."
    },
    {
     "key": "korpo",
     "match": {
      "box": [
       [
        60.1019,
        60.27,
        21.4633,
        21.6899
       ]
      ]
     },
     "note": "Korpo + Norrskata (codes 21710, 21720, 21750) : bac routier Nagu (Pärnäs) – Korpo (Retais). Norrskata rangée avec Korpo (limite)."
    },
    {
     "key": "nagu",
     "match": {
      "box": [
       [
        60.1179,
        60.2197,
        21.691,
        21.9458
       ]
      ]
     },
     "note": "Nagu (Storlandet, Lillandet… codes postaux 21660, 21661, 21670, hors îles extérieures) : bac routier Lillmälö (Pargas) – Prostvik."
    }
   ]
  },
  "LT": {
   "default": "continental",
   "rules": [
    {
     "key": "neringa",
     "match": {
      "box": [
       [
        55.25,
        55.72,
        20.9,
        21.132
       ]
      ]
     },
     "note": "Isthme de Courlande côté lituanien (Neringa : Nida, Preila, Pervalka ; Klaipėda : Smiltynė, Kopgalis, Alksnynė — 10 lieux). Aucune route vers le reste de la Lituanie : accès par le bac Klaipėda–Smiltynė. La route continue vers l'oblast de Kaliningrad (poste Nida–Morskoïe), non retenue comme lien (frontière extérieure de l'UE à franchissement restreint)."
    }
   ]
  }
 },
 "ferries": [
  {
   "a": "continental",
   "b": "aeroe",
   "routeKey": "svendborgAeroeskoebing", "priceCovers": "vehicle", "priceCoversByClass": {"2":null,"5":null}, "coversSource": "grille Ærøfærgerne : « personbil uden fører » ; autocamper et MC sans mention du conducteur", "durationEstimated": true,
   "name": "Svendborg ↔ Ærøskøbing (Ærø)",
   "operator": "Ærøfærgerne (Ærø Kommune)",
   "durationH": 1.25,
   "distanceKm": 27,
   "priceByClass": {
    "1": 19,
    "2": 44.15,
    "5": 6.02,
    "foot": 9.1
   },
   "currency": "DKK",
   "original": {
    "1": 142,
    "2": 330,
    "5": 45,
    "foot": 68
   },
   "source": "https://www.aeroe-ferry.dk/priser",
   "date": "2026-09-16",
   "note": "Billetpriser 3 janv. 2026 – 4 janv. 2027, « Enkelt » : personbil uden fører 142 ; autocamper under 6 m 330 ; MC 45 ; voksen 68 DKK. Autocamper et MC sans mention de conducteur inclus : lus comme véhicule seul. Mêmes tarifs Faaborg–Søby et Fynshav–Søby. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "samsoe",
   "routeKey": "houSaelvig", "priceCovers": "vehicleAndOccupants", "coversSource": "grille Samsø : « bil inkl. op til ni personer », « autocamper inkl. ni personer », « MC inkl. to personer »", "durationEstimated": true,
   "name": "Hou ↔ Sælvig (Samsø)",
   "operator": "Samsø Rederi (Samsø Kommune)",
   "durationH": 1,
   "distanceKm": 23,
   "priceByClass": {
    "1": 68.9,
    "2": 68.9,
    "5": 24.48,
    "foot": 14.18
   },
   "currency": "DKK",
   "original": {
    "1": 515,
    "2": 515,
    "5": 183,
    "foot": 106
   },
   "source": "https://www.tilsamsoe.dk/priser-ruten-hou-samso/",
   "date": "2026-09-16",
   "note": "Période « Standard » (27 juin – 9 août 2026, haute saison), enkeltbillet : bil ≤6 m inkl. op til ni personer 515 ; autocamper ≤6 m inkl. ni personer 515 ; MC inkl. to personer 183 ; voksen købt online 106 DKK. Tarifs repris tels quels, occupants compris (11e audit, 19/09/2026 ; avant : conducteur déduit, 409 / 409 / 77, ce qui laissait les huit autres occupants inclus). Période « Billig » (reste de l’année) : bil 262, MC 128, voksen 66. Kalundborg–Ballen non retenue (une liaison par paire). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "laesoe",
   "routeKey": "frederikshavnLaesoe", "priceCovers": "vehicleAndOccupants", "priceCoversByClass": {"5":"vehicle"}, "coversSource": "grille Læsøfærgen : personbil et autocamper « inkl. 7 personer » ; « motorcykel ekskl. fører »",
   "name": "Frederikshavn ↔ Vesterø Havn (Læsø)",
   "operator": "Læsøfærgen (Læsø Kommune)",
   "durationH": 1.5,
   "distanceKm": 30,
   "priceByClass": {
    "1": 69.57,
    "2": 81.61,
    "5": 18.06,
    "foot": 11.37
   },
   "currency": "DKK",
   "original": {
    "1": 520,
    "2": 610,
    "5": 135,
    "foot": 85
   },
   "source": "https://www.laesoe-line.dk/files/downloads/Fartplan%20og%20prisliste%20L%C3%A6s%C3%B8f%C3%A6rgen%202026.pdf",
   "date": "2026-09-16",
   "note": "Prisliste 2026, enkeltrejser, « Standard » (toute l’année) : personbil <1,9 m inkl. 7 personer 520 ; autocamper <6 m inkl. 7 personer 610 ; motorcykel ekskl. fører 135 ; voksen 85 DKK. Tarifs « inkl. 7 personer » repris tels quels, occupants compris (11e audit, 19/09/2026 ; avant : conducteur déduit, 435 / 525) ; moto hors conducteur. Traversée 90 min (fiche horaire) ; distance : ordre de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "fanoe",
   "routeKey": "esbjergNordby", "priceCovers": "vehicleAndOccupants", "coversSource": "grille FanøLinjen : « lav bil … inkl. op til 9 personer »", "durationEstimated": true,
   "name": "Esbjerg ↔ Nordby (Fanø)",
   "operator": "FanøLinjen (Molslinjen, pour Fanø Kommune)",
   "durationH": 0.2,
   "distanceKm": 3,
   "priceByClass": {
    "1": 21.74,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "DKK",
   "original": {
    "1": 162.5,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.fanoelinjen.dk/priser",
   "date": "2026-09-16",
   "note": "Seul tarif lisible sans exécuter la page : billet « Standard » tur/retur 325 DKK, lav bil <1,95 m et <6 m inkl. op til 9 personer, onglet « Lavsæson 2026 » affiché par défaut → 162,5 par traversée, passagers inclus (non déductibles, tarif piéton adulte non affiché). Autocamper, MC, piéton et tarifs moyenne/haute saison chargés dynamiquement : null. 12 min (page officielle). Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "anholt",
   "routeKey": "grenaaAnholt", "priceCovers": "vehicle", "priceCoversByClass": {"2":null,"5":null}, "coversSource": "grille Anholtfærgen : « bil under 6 m ekskl. chauffør » ; camping-car et MC sans mention", "durationEstimated": true,
   "name": "Grenaa ↔ Anholt",
   "operator": "Grenaa-Anholt Færgefart (Norddjurs Kommune)",
   "durationH": 2.75,
   "distanceKm": 45,
   "priceByClass": {
    "1": 278.4,
    "2": 304.49,
    "5": 92.64,
    "foot": 13.51
   },
   "currency": "DKK",
   "original": {
    "1": 2081,
    "2": 2276,
    "5": 692.5,
    "foot": 101
   },
   "source": "https://www.anholtfergen.dk/priser/",
   "date": "2026-09-16",
   "note": "Toute l’année, tur/retur : bil under 6 m ekskl. chauffør 4 162 ; campingvogn m.m. under 6 m 4 552 ; MC 1 385 → moitié par traversée. Voksen enkelt 101 DKK (+ supplément carburant temporaire de 40 DKK depuis mars 2026, non ajouté). Distance : ordre de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "sejeroe",
   "routeKey": "havnsoeSejeroe", "priceCovers": "vehicle", "priceCoversByClass": {"5":null}, "coversSource": "grille Sejerøfærgen : « bil ekskl. fører », « autocamper ekskl. fører » ; MC sans mention", "durationEstimated": true,
   "name": "Havnsø ↔ Sejerø",
   "operator": "Sejerøbugtens Færger (Kalundborg Kommune)",
   "durationH": 1,
   "distanceKm": 17,
   "priceByClass": {
    "1": 16.72,
    "2": 41.47,
    "5": 10.03,
    "foot": 8.7
   },
   "currency": "DKK",
   "original": {
    "1": 125,
    "2": 310,
    "5": 75,
    "foot": 65
   },
   "source": "https://www.kalundborg.dk/service-and-selvbetjening/trafik-veje-and-groenne-omraader/trafik/sejeroebugtens-faerger/billetpriser-sejeroefaergen",
   "date": "2026-09-16",
   "note": "Enkeltbilletter 2026 (première colonne) : bil ekskl. fører 125 ; autocamper ekskl. fører 310 ; motorcykel 75 ; voksen 65 DKK. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "furDk",
   "routeKey": "brandenFur", "priceCovers": "vehicleAndOccupants", "coversSource": "grille Furfærgen : toutes les personnes voyagent gratuitement", "durationEstimated": true,
   "name": "Branden ↔ Fur",
   "operator": "Furfærgen (Skive Kommune)",
   "durationH": 0.05,
   "distanceKm": 0.5,
   "priceByClass": {
    "1": 8.03,
    "2": 8.03,
    "5": 4.01,
    "foot": 0
   },
   "currency": "DKK",
   "original": {
    "1": 60,
    "2": 60,
    "5": 30,
    "foot": 0
   },
   "source": "https://furfaerge.dk/media/uinhhje2/takstblad-2026.pdf",
   "date": "2026-09-16",
   "note": "Takstblad 2026 : « Alle billetter er returbilletter » ; personbil, varevogn et autocamper <3 500 kg 120 ; motorcykel 60 DKK → moitié par traversée ; toutes les personnes gratuites. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "fejoe",
   "routeKey": "kragenaesFejoe", "priceCovers": "vehicle", "priceCoversByClass": {"2":null,"5":null}, "coversSource": "grille Lolland Færgefart : « biler ekskl. fører » ; campingvogn/autocamper et MC sans mention", "durationEstimated": true,
   "name": "Kragenæs ↔ Fejø",
   "operator": "Lolland Færgefart (Lolland Kommune)",
   "durationH": 0.25,
   "distanceKm": 3,
   "priceByClass": {
    "1": 10.7,
    "2": 10.7,
    "5": 6.69,
    "foot": 4.35
   },
   "currency": "DKK",
   "original": {
    "1": 80,
    "2": 80,
    "5": 50,
    "foot": 32.5
   },
   "source": "https://lollandfaergefart.lolland.dk/om-lolland-faergefart/priser",
   "date": "2026-09-16",
   "note": "Prisoversigt au 1er janvier 2026, « alle takster er tur/retur » : biler ekskl. fører 160 ; campingvogn/autocamper 160 ; motorcykel 100 ; passager 12 ans et + 65 DKK → moitié par traversée. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "femoe",
   "routeKey": "kragenaesFemoe", "priceCovers": "vehicle", "priceCoversByClass": {"2":null,"5":null}, "coversSource": "grille Lolland Færgefart : « biler ekskl. fører » ; campingvogn/autocamper et MC sans mention", "durationEstimated": true,
   "name": "Kragenæs ↔ Femø",
   "operator": "Lolland Færgefart (Lolland Kommune)",
   "durationH": 0.83,
   "distanceKm": 10,
   "priceByClass": {
    "1": 10.7,
    "2": 10.7,
    "5": 6.69,
    "foot": 4.35
   },
   "currency": "DKK",
   "original": {
    "1": 80,
    "2": 80,
    "5": 50,
    "foot": 32.5
   },
   "source": "https://lollandfaergefart.lolland.dk/om-lolland-faergefart/priser",
   "date": "2026-09-16",
   "note": "Même grille que Fejø (tur/retur 160 / 160 / 100 / 65 DKK, divisés par deux). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "askoe",
   "routeKey": "bandholmAskoe", "priceCovers": "vehicle", "priceCoversByClass": {"2":null,"5":null}, "coversSource": "grille Lolland Færgefart : « biler ekskl. fører » ; campingvogn/autocamper et MC sans mention", "durationEstimated": true,
   "name": "Bandholm ↔ Askø",
   "operator": "Lolland Færgefart (Lolland Kommune)",
   "durationH": 0.6,
   "distanceKm": 7,
   "priceByClass": {
    "1": 10.7,
    "2": 10.7,
    "5": 6.69,
    "foot": 4.35
   },
   "currency": "DKK",
   "original": {
    "1": 80,
    "2": 80,
    "5": 50,
    "foot": 32.5
   },
   "source": "https://lollandfaergefart.lolland.dk/om-lolland-faergefart/priser",
   "date": "2026-09-16",
   "note": "Même grille que Fejø. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "endelave",
   "routeKey": "snaptunEndelave", "priceCovers": "vehicle", "priceCoversByClass": {"5":null}, "coversSource": "grille Endelavefærgen : « bil uden fører » (bas et haut) ; MC sans mention", "durationEstimated": true,
   "name": "Snaptun ↔ Endelave",
   "operator": "Endelavefærgen (Horsens Kommune)",
   "durationH": 1.17,
   "distanceKm": 18,
   "priceByClass": {
    "1": 16.59,
    "2": 20.87,
    "5": 8.29,
    "foot": 8.29
   },
   "currency": "DKK",
   "original": {
    "1": 124,
    "2": 156,
    "5": 62,
    "foot": 62
   },
   "source": "https://mf-endelave.dk/priseroginformation/priser2026",
   "date": "2026-09-16",
   "note": "Priser 2026 « pr. tur » : bil uden fører lav (≤2,20 m) 124 ; bil uden fører høj 156 (autocamper <6 m réservé comme bil høj) ; motorcykel 62 ; voksen 62 DKK. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "oroe",
   "routeKey": "holbaekOroe", "priceCovers": "vehicle", "coversSource": "grille Orø-færgen : takster « ekskl. fører »", "durationEstimated": true,
   "name": "Holbæk ↔ Orø",
   "operator": "Færgefarten Orø-Holbæk (Holbæk Kommune)",
   "durationH": 0.9,
   "distanceKm": 12,
   "priceByClass": {
    "1": 12.58,
    "2": 24.62,
    "5": 5.35,
    "foot": 5.75
   },
   "currency": "DKK",
   "original": {
    "1": 94,
    "2": 184,
    "5": 40,
    "foot": 43
   },
   "source": "https://holbaek.dk/service-og-selvbetjening/personlige-forhold/betaling-opkraevning-og-takster/takster/oroe-faergen",
   "date": "2026-09-16",
   "note": "Takster 2026 « enkelt », ekskl. fører : personbil 94 ; autocamper 184 ; motorcykel 40 ; voksen 43 DKK. La traversée courte Hammer Bakke–Orø (Østre Færge, Frederikssund) existe aussi, mais sa page tarifaire est protégée par un contrôle anti-robot : non lue. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "venoe",
   "routeKey": "kleppenVenoe", "priceCovers": "vehicleAndOccupants", "coversSource": "grille Venø : « car incl. max. 8 passengers », « motorcycle incl. passenger »",
   "name": "Kleppen ↔ Venø",
   "operator": "Venø Færgefart (Struer Kommune)",
   "durationH": 0.05,
   "distanceKm": 0.3,
   "priceByClass": {
    "1": 6.02,
    "2": 6.02,
    "5": 4.01,
    "foot": 1.67
   },
   "currency": "DKK",
   "original": {
    "1": 45,
    "2": 45,
    "5": 30,
    "foot": 12.5
   },
   "source": "https://venoefaergefart.dk/en/ferry-service-prices/",
   "date": "2026-09-16",
   "note": "Prix « RETURN » : car incl. max. 8 passengers 90 ; motorcycle incl. passenger 60 ; adult 25 DKK. Division par deux, occupants compris : 45 / 30 ; piéton 12,5 (11e audit, 19/09/2026 ; avant : conducteur déduit, 32,5 / 17,5). Pas de tarif camping-car distinct (véhicules <3 500 kg) : tarif voiture. 2 min. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "agersoe",
   "routeKey": "stigsnaesAgersoe", "priceCovers": "vehicle", "coversSource": "grille Agersø/Omø : « uden fører og passagerer »", "durationEstimated": true,
   "name": "Stigsnæs ↔ Agersø",
   "operator": "Agersø Omø Færgerne (Slagelse Kommune)",
   "durationH": 0.25,
   "distanceKm": 4,
   "priceByClass": {
    "1": 6.29,
    "2": 6.29,
    "5": 1.34,
    "foot": 2.94
   },
   "currency": "DKK",
   "original": {
    "1": 47,
    "2": 47,
    "5": 10,
    "foot": 22
   },
   "source": "https://aofaerger.slagelse.dk/da/priser/priser-for-agersoe-faergen-2026-pr-1-maj/",
   "date": "2026-09-16",
   "note": "Priser 2026 pr. 1. maj, « tur/retur og uden fører og passagerer » : biler 94 ; motorcykel 20 ; voksen 44 DKK → moitié. Camping-car ≤3 500 kg au tarif « biler » (au-delà : 188). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "omoe",
   "routeKey": "stigsnaesOmoe", "priceCovers": "vehicle", "coversSource": "grille Agersø/Omø : « uden fører og passagerer »", "durationEstimated": true,
   "name": "Stigsnæs ↔ Omø",
   "operator": "Agersø Omø Færgerne (Slagelse Kommune)",
   "durationH": 1,
   "distanceKm": 15,
   "priceByClass": {
    "1": 11.57,
    "2": 11.57,
    "5": 2.14,
    "foot": 4.88
   },
   "currency": "DKK",
   "original": {
    "1": 86.5,
    "2": 86.5,
    "5": 16,
    "foot": 36.5
   },
   "source": "https://aofaerger.slagelse.dk/da/priser/priser-for-omoe-faergen-2026-pr-1-maj/",
   "date": "2026-09-16",
   "note": "Priser 2026 pr. 1. maj, tur/retur sans conducteur : biler 173 ; motorcykel 32 ; voksen 73 DKK → moitié. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "egholm",
   "routeKey": "aalborgEgholm", "priceCovers": "vehicle", "coversSource": "grille Egholmfærgen : « uden fører » pour voitures et moto", "durationEstimated": true,
   "name": "Aalborg ↔ Egholm",
   "operator": "Egholmfærgen (Aalborg Kommune)",
   "durationH": 0.17,
   "distanceKm": 1,
   "priceByClass": {
    "1": 6.02,
    "2": 8.23,
    "5": 3.88,
    "foot": 2.61
   },
   "currency": "DKK",
   "original": {
    "1": 45,
    "2": 61.5,
    "5": 29,
    "foot": 19.5
   },
   "source": "https://www.aalborg.dk/mit-liv/trafik-og-parkering/trafik-og-veje/faerger-og-broer/egholmfaergen/",
   "date": "2026-09-16",
   "note": "Tur/retur : personbil under 6 m uden fører 90 ; personbil over 6 m uden fører 123 ; motorcykel uden fører 58 ; person fra 16 år 39 DKK → moitié par traversée. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "strynoe",
   "routeKey": "rudkoebingStrynoe", "priceCovers": null, "coversSource": "grille : inclusion du conducteur non précisée", "durationEstimated": true,
   "name": "Rudkøbing ↔ Strynø",
   "operator": "Strynø-Rudkøbing Færgefart (Langeland Kommune)",
   "durationH": 0.75,
   "distanceKm": 7,
   "priceByClass": {
    "1": 15.05,
    "2": 22.61,
    "5": 7.56,
    "foot": 6.02
   },
   "currency": "DKK",
   "original": {
    "1": 112.5,
    "2": 169,
    "5": 56.5,
    "foot": 45
   },
   "source": "https://langelandkommune.dk/p/Stryn%C3%B8-Rudk%C3%B8bing-f%C3%A6rgefart-Sejlplan-og-priser-06.01.2025.pdf",
   "date": "2026-09-16",
   "note": "Seule grille publiée, liée depuis la page officielle « Sejlplan og priser » (document mis à jour le 06.01.2025) : retur, biler op til 5 m 225 (+113 par tranche de 2,5 m au-delà → 338 pour un camping-car de 5–7,5 m) ; motorcykler 113 ; voksne 90 DKK → moitié par traversée. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "avernakoe",
   "routeKey": "faaborgAvernakoe", "priceCovers": "vehicle", "priceCoversByClass": {"5":null}, "coversSource": "grille Ø-Færgen : « alle køretøjer under 6 m ekskl. fører » ; MC sans mention", "durationEstimated": true,
   "name": "Faaborg ↔ Avernakø",
   "operator": "Ø-Færgen (Faaborg-Midtfyn Kommune)",
   "durationH": 0.5,
   "distanceKm": 8,
   "priceByClass": {
    "1": 17.73,
    "2": 17.73,
    "5": 3.34,
    "foot": 8.7
   },
   "currency": "DKK",
   "original": {
    "1": 132.5,
    "2": 132.5,
    "5": 25,
    "foot": 65
   },
   "source": "https://oefaergen.fmk.dk/priser/priser/",
   "date": "2026-09-16",
   "note": "Priser Højsæson 2026 (27 juin – 9 août), tur/retur : alle køretøjer under 6 m ekskl. fører 265 ; cykel/knallert/motorcykel 50 ; voksen 130 DKK → moitié. Basse saison : 125 / 25 / 80. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "lyoe",
   "routeKey": "faaborgLyoe", "priceCovers": "vehicle", "priceCoversByClass": {"5":null}, "coversSource": "grille Ø-Færgen : « alle køretøjer under 6 m ekskl. fører » ; MC sans mention", "durationEstimated": true,
   "name": "Faaborg ↔ Lyø",
   "operator": "Ø-Færgen (Faaborg-Midtfyn Kommune)",
   "durationH": 0.8,
   "distanceKm": 10,
   "priceByClass": {
    "1": 17.73,
    "2": 17.73,
    "5": 3.34,
    "foot": 8.7
   },
   "currency": "DKK",
   "original": {
    "1": 132.5,
    "2": 132.5,
    "5": 25,
    "foot": 65
   },
   "source": "https://oefaergen.fmk.dk/priser/priser/",
   "date": "2026-09-16",
   "note": "Même grille qu’Avernakø (Faaborg III dessert les deux îles). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "drejoe",
   "routeKey": "svendborgDrejoe", "priceCovers": null, "coversSource": "grille : inclusion du conducteur non précisée", "durationEstimated": true,
   "name": "Svendborg ↔ Drejø (via Skarø)",
   "operator": "M/F Højestene (Svendborg Havn)",
   "durationH": 1.25,
   "distanceKm": 14,
   "priceByClass": {
    "1": 18.33,
    "2": 27.49,
    "5": 9.16,
    "foot": 6.69
   },
   "currency": "DKK",
   "original": {
    "1": 137,
    "2": 205.5,
    "5": 68.5,
    "foot": 50
   },
   "source": "https://www.svendborghavn.dk/da/faerger/billetpriser/",
   "date": "2026-09-16",
   "note": "Billetpriser 2026 (ud- og hjemtur) : bil 5 m 274 ; bil 7,5 m 411 ; motorcykel 137 ; voksen 100 DKK → moitié. Sejladsen annoncée comme suspendue jusqu’au 21/9/2026 (reprise prévue). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "skaroe",
   "routeKey": "svendborgSkaroe", "priceCovers": null, "coversSource": "grille : inclusion du conducteur non précisée", "durationEstimated": true,
   "name": "Svendborg ↔ Skarø",
   "operator": "M/F Højestene (Svendborg Havn)",
   "durationH": 0.6,
   "distanceKm": 7,
   "priceByClass": {
    "1": 18.33,
    "2": 27.49,
    "5": 9.16,
    "foot": 6.69
   },
   "currency": "DKK",
   "original": {
    "1": 137,
    "2": 205.5,
    "5": 68.5,
    "foot": 50
   },
   "source": "https://www.svendborghavn.dk/da/faerger/billetpriser/",
   "date": "2026-09-16",
   "note": "Même grille que Drejø. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "hjarnoe",
   "routeKey": "snaptunHjarnoe", "durationEstimated": true,
   "name": "Snaptun ↔ Hjarnø",
   "operator": "Hjarnø Færgefart (Hedensted Kommune)",
   "durationH": 0.1,
   "distanceKm": 1,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "DKK",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://xn--hjarnfrgefart-8fb2y.dk/priser/",
   "date": "2026-09-16",
   "note": "Ferry pour voitures ; page « Priser 2025-2026 » protégée par un contrôle anti-robot (non contourné) : tarif non lu. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "baagoe",
   "routeKey": "assensBaagoe", "priceCovers": "vehicle", "priceCoversByClass": {"5":null}, "coversSource": "grille Bågøfærgen : « bil/bus/campingvogn … ekskl. fører » ; MC sans mention", "durationEstimated": true,
   "name": "Assens ↔ Bågø",
   "operator": "Bågøfærgen (Assens Havn)",
   "durationH": 0.67,
   "distanceKm": 10,
   "priceByClass": {
    "1": 13.58,
    "2": 17.99,
    "5": 4.95,
    "foot": 4.01
   },
   "currency": "DKK",
   "original": {
    "1": 101.5,
    "2": 134.5,
    "5": 37,
    "foot": 30
   },
   "source": "https://www.assenshavn.dk/assens-baagoe-faergen/priser-baagoefaergen",
   "date": "2026-09-16",
   "note": "2026, « alle billetter er returbilletter » : bil/bus/campingvogn ≤5 m ekskl. fører 203 ; >5 m 269 ; motorcykel 74 ; voksen 60 DKK → moitié. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "barsoe",
   "routeKey": "barsoeLanding", "priceCovers": "vehicle", "coversSource": "grille Aabenraa : « privat biler … ekskl. fører » ; moto gratuite, pilote au tarif passager", "durationEstimated": true,
   "name": "Barsø Landing ↔ Barsø",
   "operator": "Barsøfærgen (Aabenraa Kommune)",
   "durationH": 0.1,
   "distanceKm": 1.5,
   "priceByClass": {
    "1": 5.35,
    "2": 5.35,
    "5": 0,
    "foot": 1.81
   },
   "currency": "DKK",
   "original": {
    "1": 40,
    "2": 40,
    "5": 0,
    "foot": 13.5
   },
   "source": "https://aabenraa.dk/media/r03dszw1/takster-2026.pdf",
   "date": "2026-09-16",
   "note": "Takster 2026 d’Aabenraa Kommune : privat biler under 3 500 kg ekskl. fører 64,00 hors TVA (25 % ajoutés → 80) ; motorcykler 0 ; voksen 27 DKK ; la rubrique indique « dobbelttur » (aller-retour) : moitié par traversée, lecture appliquée aussi aux véhicules. Camping-car <3 500 kg au tarif voiture. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 7,4748 DKK."
  },
  {
   "a": "continental",
   "b": "sylt",
   "routeKey": "niebuellWesterland", "priceCovers": "vehicleAndOccupants", "coversSource": "Sylt Shuttle (train-auto) : tarif par véhicule, passagers inclus", "durationEstimated": true, "mode": "train",
   "name": "Niebüll ↔ Westerland (Sylt Shuttle)",
   "operator": "DB Sylt Shuttle",
   "durationH": 0.75,
   "distanceKm": 38,
   "priceByClass": {
    "1": 75,
    "2": 95,
    "5": 39,
    "foot": null
   },
   "currency": "EUR",
   "original": {
    "1": 75,
    "2": 95,
    "5": 39,
    "foot": null
   },
   "source": "https://www.syltshuttle.de/syltshuttle-de/fahrkarten/tarif_2023-2049320",
   "date": "2026-09-16",
   "note": "Train-auto par le Hindenburgdamm (aucune route). Tarif au 01.01.2026, vente sur place : véhicule ≤6 m einfache Fahrt 75 € (passagers inclus, non déductibles) ; campingfahrzeuge ≤10 m Hin- und Rückfahrt 190 € → 95 ; Motorrad einfache Fahrt 39 €. Pas de passager sans véhicule (train régional) : null. Le ferry Rømø–List (SyltExpress) relie aussi Sylt au continent danois (même paire de masses) : non retenu. Durée/distance : ordres de grandeur."
  },
  {
   "a": "continental",
   "b": "foehr",
   "routeKey": "dagebuellWyk", "priceCovers": null, "coversSource": "seul le tarif passager est publié", "durationEstimated": true,
   "name": "Dagebüll ↔ Wyk auf Föhr",
   "operator": "W.D.R. (Wyker Dampfschiffs-Reederei)",
   "durationH": 0.83,
   "distanceKm": 13,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": 10.6
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": 10.6
   },
   "source": "https://www.faehre.de/faehrverkehr/preise/fahrzeuge",
   "date": "2026-09-16",
   "note": "Tarif véhicules calculé par un formulaire selon la date et la longueur au millimètre (aucune grille lisible sans l’exécuter) : véhicules null. Passager adulte einfache Fahrt 10,60 € (https://www.faehre.de/faehrverkehr/preise/personen)."
  },
  {
   "a": "continental",
   "b": "amrum",
   "routeKey": "dagebuellWittduen", "priceCovers": null, "coversSource": "seul le tarif passager est publié", "durationEstimated": true,
   "name": "Dagebüll ↔ Wittdün (Amrum)",
   "operator": "W.D.R. (Wyker Dampfschiffs-Reederei)",
   "durationH": 2,
   "distanceKm": 30,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": 14.7
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": 14.7
   },
   "source": "https://www.faehre.de/faehrverkehr/preise/fahrzeuge",
   "date": "2026-09-16",
   "note": "Comme Föhr : tarif véhicules non lisible (formulaire). Passager adulte einfache Fahrt 14,70 €. Ligne via Wyk."
  },
  {
   "a": "foehr",
   "b": "amrum",
   "routeKey": "wykWittduen", "durationEstimated": true,
   "name": "Wyk (Föhr) ↔ Wittdün (Amrum)",
   "operator": "W.D.R. (Wyker Dampfschiffs-Reederei)",
   "durationH": 0.75,
   "distanceKm": 12,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.faehre.de/faehrverkehr/preise/fahrzeuge",
   "date": "2026-09-16",
   "note": "Même navire que Dagebüll–Amrum, escale de Wyk ; tarifs non lisibles (formulaire)."
  },
  {
   "a": "continental",
   "b": "pellworm",
   "routeKey": "strucklahnungshoernPellworm", "priceCovers": "vehicle", "coversSource": "grille NPDG : tarif Pkw « personnes non comprises »", "durationEstimated": true,
   "name": "Strucklahnungshörn ↔ Pellworm",
   "operator": "NPDG (Neue Pellwormer Dampfschiffahrts-GmbH)",
   "durationH": 0.6,
   "distanceKm": 10,
   "priceByClass": {
    "1": 60.75,
    "2": 81,
    "5": 11,
    "foot": 8.25
   },
   "currency": "EUR",
   "original": {
    "1": 60.75,
    "2": 81,
    "5": 11,
    "foot": 8.25
   },
   "source": "https://www.faehre-pellworm.de/faehrfahrten/fahrpreise",
   "date": "2026-09-16",
   "note": "Preise gültig ab 01.01.2026 ; tarif Pkw selon la longueur, 2,70 € par dm, « Hin- und Rückfahrt », personnes non comprises : voiture de 4,5 m 121,50 → 60,75 ; camping-car de 6 m (Kfz-Tarif) 162,00 → 81,00 ; Kraftrad Hin & Rück 22,00 → 11,00 ; adulte Hin & Rück 16,50 → 8,25 (einfach 9,50). Longueurs types = choix de modélisation. Durée/distance : ordres de grandeur."
  },
  {
   "a": "continental",
   "b": "borkum",
   "routeKey": "emdenBorkum", "priceCovers": "vehicle", "coversSource": "véhicule seul : grille AG EMS « inkl. Fahrer », adulte déduit ; moto non tarifée", "durationEstimated": true,
   "name": "Emden ↔ Borkum",
   "operator": "AG EMS",
   "durationH": 2,
   "distanceKm": 50,
   "priceByClass": {
    "1": 90.03,
    "2": 132.85,
    "5": null,
    "foot": 26.75
   },
   "currency": "EUR",
   "original": {
    "1": 90.03,
    "2": 132.85,
    "5": null,
    "foot": 26.75
   },
   "source": "https://www.ag-ems.de/tarife/",
   "date": "2026-09-16",
   "note": "Preise 2026 (extrait, Stand 11/2025), Hin- und Rückfahrt : PKW « A » (<2 m) inkl. Fahrer 51,90 €/m ; PKW « B » (>2 m) 53,20 €/m ; adulte Hin & Rück 53,50 €. Voiture de 4,5 m : 233,55 − 53,50 = 180,05 → 90,03 par traversée ; camping-car de 6 m : 319,20 − 53,50 → 132,85 ; piéton 26,75. Moto non listée : null. Durée ~2 h (ferry) ; distance : ordre de grandeur."
  },
  {
   "a": "continental",
   "b": "norderney",
   "routeKey": "norddeichNorderney", "durationEstimated": true,
   "name": "Norddeich ↔ Norderney",
   "operator": "AG Reederei Norden-Frisia",
   "durationH": 0.9,
   "distanceKm": 8,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "variable",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.reederei-frisia.de/",
   "date": "2026-09-16",
   "note": "Transport de voitures en tarifs « Basic / Flex / Plus » variant selon le jour et l’heure de départ (jusqu’à −42 %) : prix fixé à la réservation."
  },
  {
   "a": "continental",
   "b": "saaremaa",
   "routeKey": "virtsuKuivastu", "priceCovers": "vehicle", "coversSource": "véhicule seul : praamid.ee, le billet véhicule n'inclut pas le conducteur", "durationEstimated": true,
   "name": "Virtsu ↔ Kuivastu (Muhu/Saaremaa)",
   "operator": "TS Laevad (Praamid.ee)",
   "durationH": 0.5,
   "distanceKm": 7,
   "priceByClass": {
    "1": 12,
    "2": 12,
    "5": 4.85,
    "foot": 4.3
   },
   "currency": "EUR",
   "original": {
    "1": 12,
    "2": 12,
    "5": 4.85,
    "foot": 4.3
   },
   "source": "https://www.praamid.ee/hinnad-ja-soodustused/",
   "date": "2026-09-16",
   "note": "Saaremaa liin : sõiduauto 12 € ; bus ≤23 places et autoelamu (camping-car) 12 € ; mootorratas 4,85 € ; täiskasvanu 4,30 €. Le billet véhicule n’inclut pas le conducteur. Majoration de 20 % pour les véhicules le vendredi après-midi vers les îles et le dimanche après-midi vers le continent (non appliquée). Muhu–Saaremaa : chaussée Väinatamm."
  },
  {
   "a": "continental",
   "b": "hiiumaa",
   "routeKey": "rohukylaHeltermaa", "priceCovers": "vehicle", "coversSource": "véhicule seul : praamid.ee, le billet véhicule n'inclut pas le conducteur", "durationEstimated": true,
   "name": "Rohuküla ↔ Heltermaa (Hiiumaa)",
   "operator": "TS Laevad (Praamid.ee)",
   "durationH": 1.5,
   "distanceKm": 28,
   "priceByClass": {
    "1": 14.3,
    "2": 14.3,
    "5": 6,
    "foot": 4.85
   },
   "currency": "EUR",
   "original": {
    "1": 14.3,
    "2": 14.3,
    "5": 6,
    "foot": 4.85
   },
   "source": "https://www.praamid.ee/hinnad-ja-soodustused/",
   "date": "2026-09-16",
   "note": "Hiiumaa liin : sõiduauto 14,30 € ; autoelamu 14,30 € ; mootorratas 6 € ; täiskasvanu 4,85 €. Même majoration de pointe de 20 %. Distance : ordre de grandeur."
  },
  {
   "a": "hiiumaa",
   "b": "saaremaa",
   "routeKey": "soruTriigi", "durationEstimated": true,
   "name": "Sõru (Hiiumaa) ↔ Triigi (Saaremaa)",
   "operator": "Saaremaa vald / exploitant sous contrat",
   "durationH": 1,
   "distanceKm": 20,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.riigiteataja.ee/akt/126042025008",
   "date": "2026-09-16",
   "note": "Ferry pour véhicules ; tarifs fixés par règlement (Riigi Teataja), page lisible seulement avec JavaScript : non lue."
  },
  {
   "a": "continental",
   "b": "vormsi",
   "routeKey": "rohukylaSviby", "durationEstimated": true,
   "name": "Rohuküla ↔ Sviby (Vormsi)",
   "operator": "Kihnu Veeteed",
   "durationH": 1.5,
   "distanceKm": 12,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://veeteed.com/",
   "date": "2026-09-16",
   "note": "Ferry pour véhicules ; site de l’exploitant entièrement dynamique (système de réservation), aucune grille lisible."
  },
  {
   "a": "continental",
   "b": "kihnu",
   "routeKey": "munalaidKihnu", "durationEstimated": true,
   "name": "Munalaid ↔ Kihnu",
   "operator": "Kihnu Veeteed",
   "durationH": 1,
   "distanceKm": 18,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://veeteed.com/",
   "date": "2026-09-16",
   "note": "Ferry pour véhicules (Kihnu Virve) ; une annexe tarifaire de contrat publiée au Riigi Teataja (véhicule ≤5 m 15 €, passager 4 €) est d’une date antérieure non vérifiable : non utilisée ; site de l’exploitant dynamique."
  },
  {
   "a": "continental",
   "b": "ruhnu",
   "routeKey": "parnuRuhnu", "durationEstimated": true,
   "name": "Pärnu ↔ Ruhnu",
   "operator": "Tuule Liinid (catamaran Runö)",
   "durationH": 3,
   "distanceKm": 95,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://ruhnu.ee/vald-uudised-ja-kontakt/valla-info/transport-saarele/",
   "date": "2026-09-16",
   "note": "Ligne saisonnière (mai–octobre) ; la commune indique que les véhicules ne peuvent pas être chargés à Munalaid, le catamaran prend quelques véhicules au départ de Pärnu sur réservation ; aucune grille véhicules publiée lisible (tuuleliinid.ee). Durée/distance : ordres de grandeur."
  },
  {
   "a": "saaremaa",
   "b": "abruka",
   "routeKey": "roomassaareAbruka", "durationEstimated": true,
   "name": "Roomassaare ↔ Abruka",
   "operator": "Saaremaa Liinid (petit navire ABRO)",
   "durationH": 0.5,
   "distanceKm": 6,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.saaremaavald.ee/uhistransport",
   "date": "2026-09-16",
   "note": "Liaison publique communale par le petit navire ABRO (billets sur saarelaevapiletid.ee) ; un tarif véhicule est cité par des sites tiers mais la grille officielle n’a pas pu être lue : unknown."
  },
  {
   "a": "continental",
   "b": "neringa",
   "routeKey": "klaipedaSmiltyne", "priceCovers": "vehicleAndOccupants", "coversSource": "grille Smiltynės perkėla : les passagers d'un véhicule n'achètent pas de billet", "durationEstimated": true,
   "name": "Klaipėda ↔ Smiltynė (isthme de Courlande)",
   "operator": "AB Smiltynės perkėla",
   "durationH": 0.1,
   "distanceKm": 1,
   "priceByClass": {
    "1": 11.6,
    "2": 23.2,
    "5": 3.85,
    "foot": 0.85
   },
   "currency": "EUR",
   "original": {
    "1": 11.6,
    "2": 23.2,
    "5": 3.85,
    "foot": 0.85
   },
   "source": "https://www.keltas.lt/kainorastis/",
   "date": "2026-09-16",
   "note": "Kainoraštis, prix « į abi puses » (aller-retour, retour valable n'importe quel jour) : lengvasis automobilis (M1, N1) 23,20 ; gyvenamasis automobilis (M1-SA, camping-car) 46,40 ; motociklas 7,70 ; keleivis 1,70 € → moitié par traversée. Les passagers d'un véhicule n'ont pas de billet à acheter. Nouveau bac (Naujoji perkėla) ~10 min."
  },
  {
   "a": "continental",
   "b": "tysnes",
   "routeKey": "halhjemVage", "priceCovers": "vehicleAndOccupants", "coversSource": "grille AutoPASS : conducteur compris, passagers gratuits", "durationEstimated": true,
   "name": "Halhjem ↔ Våge (Tysnes)",
   "operator": "Fjord1",
   "durationH": 0.6,
   "distanceKm": 10,
   "priceByClass": {
    "1": 11.88,
    "2": 25.97,
    "5": 4.24,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 129,
    "2": 282,
    "5": 46,
    "foot": 0
   },
   "source": "https://www.vegvesen.no/globalassets/fag/trafikk/ferje/pris---autopass-regulativ---fullpris---01.01.2026.pdf",
   "date": "2026-09-16",
   "note": "Grille AutoPASS 2026 plein tarif, zone de la liaison lue sur https://www.fergeruter.info/prices/halhjem-vage : AP1 <6 m 129 ; AP2 6–8 m 282 ; MC 46 NOK (conducteur compris) ; passagers et piétons gratuits. Supplément d’été de 50 NOK mai–août non appliqué. Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "austevoll",
   "routeKey": "krokeideHufthamar", "priceCovers": "vehicleAndOccupants", "coversSource": "grille AutoPASS : conducteur compris, passagers gratuits", "durationEstimated": true,
   "name": "Krokeide ↔ Hufthamar (Austevoll)",
   "operator": "Fjord1",
   "durationH": 0.6,
   "distanceKm": 12,
   "priceByClass": {
    "1": 10.13,
    "2": 22.47,
    "5": 3.78,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 110,
    "2": 244,
    "5": 41,
    "foot": 0
   },
   "source": "https://www.vegvesen.no/globalassets/fag/trafikk/ferje/pris---autopass-regulativ---fullpris---01.01.2026.pdf",
   "date": "2026-09-16",
   "note": "Grille AutoPASS 2026 plein tarif, zone 11 (https://www.fergeruter.info/prices/krokeide-hufthamar) : AP1 110 ; AP2 244 ; MC 41 NOK ; passagers gratuits. Liaison redevenue payante le 1er novembre 2025 (Vestland fylkeskommune). Durée ~35 min. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "huglo",
   "routeKey": "jektevikNordhuglo", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Jektevik ↔ Nordhuglo (Huglo)",
   "operator": "Fjord1",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Jektevik–Hodnanes). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "varaldsoy",
   "routeKey": "gjermundshamnVaraldsoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Gjermundshamn ↔ Varaldsøy",
   "operator": "Fjord1",
   "durationH": 0.4,
   "distanceKm": 7,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Gjermundshamn–Årsnes). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "fjelberg",
   "routeKey": "sydnesFjelberg", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Sydnes ↔ Fjelberg",
   "operator": "Fjord1",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "kvitsoy",
   "routeKey": "mekjarvikKvitsoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Mekjarvik ↔ Kvitsøy",
   "operator": "Boreal Sjø",
   "durationH": 0.5,
   "distanceKm": 12,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "utsira",
   "routeKey": "haugesundUtsira", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Haugesund ↔ Utsira",
   "operator": "Rutebåten Utsira",
   "durationH": 1.2,
   "distanceKm": 20,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "ombo",
   "routeKey": "ryfylkeferjenOmbo", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Ryfylkeferjen (Ombo)",
   "operator": "Kolumbus",
   "durationH": 0.8,
   "distanceKm": 15,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "hidra",
   "routeKey": "launesKvellandstrand", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Launes ↔ Kvellandstrand (Hidra)",
   "operator": "Agder fylkeskommune",
   "durationH": 0.2,
   "distanceKm": 2.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "andabeloy",
   "routeKey": "abelnesAndabeloy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Abelnes ↔ Andabeløy",
   "operator": "Boreal Sjø",
   "durationH": 0.1,
   "distanceKm": 1,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "skatoy",
   "routeKey": "krageroSkatoy", "durationEstimated": true,
   "name": "Kragerø ↔ Skåtøy",
   "operator": "Kragerø Fjordbåtselskap (Farte)",
   "durationH": 0.15,
   "distanceKm": 3,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "NOK",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.farte.no/reise/billetter-og-priser/fergebilletter/",
   "date": "2026-09-16",
   "note": "Bac pour véhicules (Kragerø–Skåtøy–Stabbestad), payant pour les non-résidents depuis le 24 mars ; tarifs publiés seulement dans l’application Farte Ferge : non lus. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "sandoyaPorsgrunn",
   "routeKey": "brevikSandoya", "durationEstimated": true,
   "name": "Brevik ↔ Sandøya",
   "operator": "Brevik Fergeselskap (Farte)",
   "durationH": 0.2,
   "distanceKm": 3,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "NOK",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://www.farte.no/reise/billetter-og-priser/fergebilletter/",
   "date": "2026-09-16",
   "note": "Bac pour véhicules ; tarifs publiés seulement dans l’application Farte Ferge : non lus. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "fedje",
   "routeKey": "saevroyFedje", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Sævrøy ↔ Fedje",
   "operator": "Fjord1",
   "durationH": 0.4,
   "distanceKm": 8,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "leroy",
   "routeKey": "klokkarvikLeroy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Klokkarvik ↔ Lerøy",
   "operator": "Skyss",
   "durationH": 0.2,
   "distanceKm": 3,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "hisaroy",
   "routeKey": "mjanesHisaroy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Mjånes ↔ Hisarøy",
   "operator": "Wergeland-Halsvik",
   "durationH": 0.2,
   "distanceKm": 2,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "solund",
   "routeKey": "rutledalKrakhella", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Rutledal ↔ Krakhella (Solund)",
   "operator": "Fjord1",
   "durationH": 0.9,
   "distanceKm": 15,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "losna",
   "routeKey": "rutledalLosna", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Rysjedalsvika/Rutledal ↔ Losna",
   "operator": "Fjord1",
   "durationH": 0.7,
   "distanceKm": 10,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », ligne Rysjedalsvika–Rutledal–Krakhella ; escale de Losna portée par la ligne dans la « Markedsoversikt for ferje » 2026 de Statens vegvesen). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "vaerlandet",
   "routeKey": "askvollVaerlandet", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Askvoll ↔ Værlandet",
   "operator": "Fjord1",
   "durationH": 0.7,
   "distanceKm": 14,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Askvoll–Fure). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "atloy",
   "routeKey": "askvollGjervik", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Askvoll ↔ Gjervik (Atløy)",
   "operator": "Fjord1",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "husevagoy",
   "routeKey": "maloyHusevagoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Måløy ↔ Husevågøy",
   "operator": "Fjord1",
   "durationH": 0.2,
   "distanceKm": 3,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Måløy–Oldeide). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "barmoya",
   "routeKey": "barmsundBarmen", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Barmsund ↔ Barmen",
   "operator": "Vidar Hop Skyssbåter",
   "durationH": 0.1,
   "distanceKm": 1,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "sandsoya",
   "routeKey": "larsnesVoksa", "durationEstimated": true,
   "name": "Larsnes ↔ Voksa (Sandsøya)",
   "operator": "Norled",
   "durationH": 0.4,
   "distanceKm": 6,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Larsnes–Åram). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Møre og Romsdal (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "gossa",
   "b": "finnoyaAlesund",
   "routeKey": "smageFinnoya", "durationEstimated": true,
   "name": "Småge ↔ Finnøya",
   "operator": "Fjord1",
   "durationH": 0.4,
   "distanceKm": 6,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Møre og Romsdal (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "sekken",
   "routeKey": "moldeSekken", "durationEstimated": true,
   "name": "Molde ↔ Sekken",
   "operator": "Fjord1",
   "durationH": 0.4,
   "distanceKm": 7,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Møre og Romsdal (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "midsund",
   "routeKey": "brattvagDryna", "durationEstimated": true,
   "name": "Brattvåg ↔ Dryna (Midsund)",
   "operator": "Fjord1",
   "durationH": 0.4,
   "distanceKm": 7,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Møre og Romsdal (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "gossa",
   "routeKey": "hollingsholmenAukra", "durationEstimated": true,
   "name": "Hollingsholmen ↔ Aukra (Gossa)",
   "operator": "Fjord1",
   "durationH": 0.3,
   "distanceKm": 4,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Møre og Romsdal (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "storfosna",
   "routeKey": "gartenStorfosna", "durationEstimated": true,
   "name": "Garten ↔ Storfosna",
   "operator": "Fosen Linjen",
   "durationH": 0.3,
   "distanceKm": 4,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Trøndelag (01/06-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "leksa",
   "routeKey": "gartenLeksa", "durationEstimated": true,
   "name": "Garten ↔ Leksa",
   "operator": "Fosen Linjen",
   "durationH": 0.8,
   "distanceKm": 14,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Trøndelag (01/06-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "sulaFroya",
   "routeKey": "dyroySula", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Dyrøy ↔ Sula (Frøya)",
   "operator": "Fosen Linjen",
   "durationH": 1,
   "distanceKm": 18,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "mausund",
   "routeKey": "dyroyMausund", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Dyrøy ↔ Mausund",
   "operator": "Fosen Linjen",
   "durationH": 0.6,
   "distanceKm": 10,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "sorburoya",
   "routeKey": "dyroySorburoy", "durationEstimated": true,
   "name": "Dyrøy ↔ Sørburøy",
   "operator": "Fosen Linjen",
   "durationH": 0.5,
   "distanceKm": 27,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. Distance corrigée (17/09/2026) : 27 km à vol d'oiseau entre les terminaux OpenStreetMap de Dyrøy (node/13438193071) et de Sørburøy (node/11048045175), au lieu de 8 ; durée non vérifiée (ligne Dyrøy–Øyrekken à escales). 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Trøndelag (01/06-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "ytteroya",
   "routeKey": "levangerHokstad", "durationEstimated": true,
   "name": "Levanger ↔ Hokstad (Ytterøya)",
   "operator": "FosenNamsos Sjø",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Trøndelag (01/06-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "joa",
   "routeKey": "olhammerenSeierstad", "durationEstimated": true,
   "name": "Ølhammeren ↔ Seierstad (Jøa)",
   "operator": "FosenNamsos Sjø",
   "durationH": 0.2,
   "distanceKm": 2,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Trøndelag (01/06-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "leka",
   "routeKey": "gutvikSkei", "durationEstimated": true,
   "name": "Gutvik ↔ Skei (Leka)",
   "operator": "Torghatten Trafikkselskap",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Trøndelag (01/06-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "torget",
   "routeKey": "bronnoysundTorget", "durationEstimated": true,
   "name": "Brønnøysund ↔ Sauren/Stortorgnes (Torget)",
   "operator": "Vidar Hop Skyssbåter",
   "durationH": 0.2,
   "distanceKm": 3,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "donna",
   "routeKey": "sandnessjoenDonna", "durationEstimated": true,
   "name": "Sandnessjøen ↔ Dønna",
   "operator": "Boreal Sjø",
   "durationH": 0.4,
   "distanceKm": 8,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "lokta",
   "routeKey": "sandnessjoenLokta", "durationEstimated": true,
   "name": "Sandnessjøen ↔ Løkta",
   "operator": "Boreal Sjø",
   "durationH": 0.8,
   "distanceKm": 15,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "vandve",
   "routeKey": "solfjellsjoenVandve", "durationEstimated": true,
   "name": "Solfjellsjøen ↔ Vandve",
   "operator": "Barents",
   "durationH": 0.2,
   "distanceKm": 3,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "hugla",
   "routeKey": "nesnaHugla", "durationEstimated": true,
   "name": "Nesna ↔ Hugla",
   "operator": "Boreal Sjø",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "tomma",
   "routeKey": "nesnaTomma", "durationEstimated": true,
   "name": "Nesna ↔ Tomma",
   "operator": "Boreal Sjø",
   "durationH": 0.5,
   "distanceKm": 8,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "onoy",
   "routeKey": "stokkvaganOnoy", "durationEstimated": true,
   "name": "Stokkvågen ↔ Onøy",
   "operator": "Torghatten Nord",
   "durationH": 0.6,
   "distanceKm": 10,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "lovund",
   "routeKey": "stokkvaganLovund", "durationEstimated": true,
   "name": "Stokkvågen ↔ Lovund",
   "operator": "Torghatten Nord",
   "durationH": 1.5,
   "distanceKm": 30,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "traena",
   "routeKey": "stokkvaganTraena", "durationEstimated": true,
   "name": "Stokkvågen ↔ Træna",
   "operator": "Torghatten Nord",
   "durationH": 2,
   "distanceKm": 40,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "nesoyaRodoy",
   "routeKey": "kilboghamnNordnesoy", "durationEstimated": true,
   "name": "Kilboghamn ↔ Nordnesøy",
   "operator": "Torghatten Nord",
   "durationH": 1.5,
   "distanceKm": 28,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. Distance corrigée (17/09/2026) : 27,8 km entre les quais (https://www.fergeruter.info/timetable/nordnesoy-kilboghamn_rodyoysambandet), au lieu de 7 ; durée non vérifiée (Rødøysambandet à escales). 12e audit (19/09/2026) : 0,4 h pour 28 km donnait 70 km/h, vitesse d'un navire rapide, pas d'un bac à escales ; horaire absent du dépôt et aucune liaison norvégienne de la base n'a de durée sourcée (59 sur 59 estimées) : durée recalculée avec la vitesse médiane des 51 liaisons de la base à durée publiée (durationEstimated absent, hors train-auto) longues de 15 à 45 km, 19,0 km/h -> 28 / 19,0 = 1,47 h, arrondie à 1,5 h (médiane des 160 liaisons à durée publiée : 19,6 km/h, même arrondi). Estimation, toujours marquée durationEstimated. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "meloya",
   "routeKey": "ornesMeloysund", "durationEstimated": true,
   "name": "Ørnes ↔ Meløysund",
   "operator": "Torghatten Nord",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "bolga",
   "routeKey": "ornesBolga", "durationEstimated": true,
   "name": "Ørnes ↔ Bolga",
   "operator": "Torghatten Nord",
   "durationH": 0.7,
   "distanceKm": 12,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "stott",
   "routeKey": "ornesStott", "durationEstimated": true,
   "name": "Ørnes ↔ Støtt",
   "operator": "Torghatten Nord",
   "durationH": 1.2,
   "distanceKm": 22,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "arnoyGildeskal",
   "routeKey": "sundSorarnoy", "durationEstimated": true,
   "name": "Sund ↔ Sørarnøy",
   "operator": "Torghatten Nord",
   "durationH": 0.3,
   "distanceKm": 4,
   "priceStatus": "variable",
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK. 11e audit (18/09/2026) : ligne soumise aux « summer rates 2026 » de Nordland (01/05-31/08/2026, https://autopassferje.no/summer-rates-2026/) — tarif majoré en été, y compris sur les bacs normalement gratuits, montant non publié par liaison : gratuité non garantie, priceStatus variable."
  },
  {
   "a": "continental",
   "b": "vaeroy",
   "routeKey": "bodoVaeroy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Bodø ↔ Værøy",
   "operator": "Torghatten Nord",
   "durationH": 4,
   "distanceKm": 105,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Bodø–Moskenes). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "rost",
   "routeKey": "bodoRost", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Bodø ↔ Røst",
   "operator": "Torghatten Nord",
   "durationH": 4,
   "distanceKm": 110,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes », sauf Bodø–Moskenes). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "vengsoya",
   "routeKey": "bellvikVengsoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Bellvik ↔ Vengsøy",
   "operator": "Torghatten Nord",
   "durationH": 0.3,
   "distanceKm": 4,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "rebbenesoya",
   "routeKey": "mikkelvikBromnes", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Mikkelvik ↔ Bromnes (Rebbenesøy)",
   "operator": "Torghatten Nord",
   "durationH": 0.5,
   "distanceKm": 8,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "karlsoy",
   "routeKey": "hansnesKarlsoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Hansnes ↔ Karlsøy",
   "operator": "Torghatten Nord",
   "durationH": 0.6,
   "distanceKm": 10,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "vannoya",
   "routeKey": "hansnesVannoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Hansnes ↔ Vannøy",
   "operator": "Torghatten Nord",
   "durationH": 1,
   "distanceKm": 18,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "reinoya",
   "routeKey": "hansnesReinoy", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Hansnes ↔ Stakkvik (Reinøy)",
   "operator": "Torghatten Nord",
   "durationH": 0.3,
   "distanceKm": 4,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "uloya",
   "routeKey": "rotsundHavnnes", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Rotsund ↔ Havnnes (Uløya)",
   "operator": "Torghatten Nord",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "laukoya",
   "routeKey": "storsteinNikkeby", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Storstein ↔ Nikkeby (Laukøya)",
   "operator": "Torghatten Nord",
   "durationH": 0.4,
   "distanceKm": 7,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "arnoyaSkjervoy",
   "routeKey": "storsteinLauksundskaret", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Storstein ↔ Lauksundskaret (Arnøya)",
   "operator": "Torghatten Nord",
   "durationH": 0.3,
   "distanceKm": 5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "continental",
   "b": "soroya",
   "routeKey": "oksfjordHasvik", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Øksfjord ↔ Hasvik (Sørøya)",
   "operator": "Boreal Sjø",
   "durationH": 1,
   "distanceKm": 20,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "NOK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://autopassferje.no/en/free-ferries-from-july-1st-2022/",
   "date": "2026-09-16",
   "note": "Liaison gratuite pour tous (liste officielle AutoPASS des bacs gratuits, « all routes »). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 10,8595 NOK."
  },
  {
   "a": "gotland",
   "b": "faro",
   "routeKey": "farosundsleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Fårösund ↔ Fårö (Fårösundsleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.15,
   "distanceKm": 1.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "holmon",
   "routeKey": "holmoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Norrfjärden ↔ Holmön (Holmöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.75,
   "distanceKm": 12,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "ivo",
   "routeKey": "ivoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Barum ↔ Ivö (Ivöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.1,
   "distanceKm": 1,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "graso",
   "routeKey": "grasoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Öregrund ↔ Gräsö (Gräsöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.15,
   "distanceKm": 1.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "yxlan",
   "routeKey": "furusundsleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Furusund ↔ Yxlan (Furusundsleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.1,
   "distanceKm": 0.8,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "blido",
   "b": "yxlan",
   "routeKey": "blidoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Yxlan ↔ Blidö (Blidöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.1,
   "distanceKm": 0.8,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "ljustero",
   "routeKey": "ljusteroleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Östanå ↔ Ljusterö (Ljusteröleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.15,
   "distanceKm": 1.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "rindo",
   "routeKey": "vaxholmsleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Vaxholm ↔ Rindö (Vaxholmsleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.1,
   "distanceKm": 1,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "arno",
   "routeKey": "arnoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Oknön ↔ Arnö (Arnöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.1,
   "distanceKm": 0.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "ockero",
   "routeKey": "honoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Lilla Varholmen ↔ Hönö (Hönöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.2,
   "distanceKm": 2,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "bjorkoOckero",
   "b": "continental",
   "routeKey": "bjorkoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Lilla Varholmen ↔ Björkö (Björköleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.2,
   "distanceKm": 2.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "nordo",
   "b": "ockero",
   "routeKey": "nordoleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Burö ↔ Knippla/Rörö (Nordöleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.4,
   "distanceKm": 6,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "lyr",
   "routeKey": "lyrleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Lyresten ↔ Lyr (Lyrleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.1,
   "distanceKm": 0.5,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "hamburgo",
   "routeKey": "hamburgsundsleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Hamburgsund ↔ Hamburgö (Hamburgsundsleden)",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.05,
   "distanceKm": 0.2,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "bohusMalmon",
   "b": "continental",
   "routeKey": "bohusMalmonleden", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Tullboden ↔ Bohus-Malmön",
   "operator": "Trafikverket Färjerederiet",
   "durationH": 0.05,
   "distanceKm": 0.2,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "SEK",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.trafikverket.se/resa-och-trafik/farjetrafik/",
   "date": "2026-09-16",
   "note": "Färjeled du réseau routier public : traversée gratuite pour tous (Trafikverket). Durée/distance : ordres de grandeur. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "visingso",
   "routeKey": "visingsoleden", "priceCovers": "vehicle", "coversSource": "véhicule seul : grille « personbil/husbil inklusive förare », piéton déduit", "durationEstimated": true,
   "name": "Gränna ↔ Visingsö (Visingsöleden)",
   "operator": "Jönköpings kommun (Trafikverket Färjerederiet)",
   "durationH": 0.4,
   "distanceKm": 6,
   "priceByClass": {
    "1": 8.34,
    "2": 15.33,
    "5": null,
    "foot": 3.61
   },
   "currency": "SEK",
   "original": {
    "1": 92.5,
    "2": 170,
    "5": null,
    "foot": 40
   },
   "source": "https://www.jonkoping.se/trafik--stadsplanering/resa-och-kollektivtrafik/visingsotrafiken-farja-mellan-granna-och-visingso/prislista-farjan-mellan-granna-och-visingso",
   "date": "2026-09-16",
   "note": "Prislista (« alla biljetter gäller för tur och retur », prix en ligne) : personbil inklusive förare 265 ; husbil (max 10 m) inklusive förare 420 ; vuxen fotgängare 80 SEK. Conducteur déduit puis moitié : 92,5 / 170 ; piéton 40. Pas de billet moto isolé (seulement une carte 10 trajets bil/MC) : null. Durée ~25 min. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "orno",
   "routeKey": "dalaroOrno", "priceCovers": "vehicle", "coversSource": "véhicule seul : « bil och förare » (conducteur déduit)", "durationEstimated": true,
   "name": "Dalarö ↔ Ornö (Hässelmara)",
   "operator": "Ornö Sjötrafik (Ornöfärjans Samfällighetsförening)",
   "durationH": 0.5,
   "distanceKm": 8,
   "priceByClass": {
    "1": 2.71,
    "2": 8.12,
    "5": 1.58,
    "foot": 2.71
   },
   "currency": "SEK",
   "original": {
    "1": 30,
    "2": 90,
    "5": 17.5,
    "foot": 30
   },
   "source": "https://ornosjotrafik.se/prislista-biljetter/",
   "date": "2026-09-16",
   "note": "« Priser för tur- och returresa », colonne transport de personnes (6 % TVA) : fordon L <6 m, B <2 m 120 ; fordon L <6 m, B >2 m 240 ; moped/MC 95 ; passagerare fyllda 15 år 60 SEK. Tarif véhicule lu comme incluant le conducteur (l’exploitant annonce « bil och förare tur och retur 120 ») : conducteur déduit (60) puis moitié → 30 / 90 / 17,5 ; piéton 30. Durée ~30 min. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "hogmarso",
   "routeKey": "svartnoHogmarso", "durationEstimated": true,
   "name": "Svartnö ↔ Högmarsö",
   "operator": "bac privé (Norrtälje)",
   "durationH": 0.1,
   "distanceKm": 0.5,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "SEK",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://sv.wikipedia.org/wiki/Lista_%C3%B6ver_f%C3%A4rjeleder_i_Sverige",
   "date": "2026-09-16",
   "note": "Bac pour véhicules signalé hors réseau Trafikverket ; aucune grille tarifaire officielle trouvée. Conversion InforEuro septembre 2026 : 1 EUR = 11,0885 SEK."
  },
  {
   "a": "continental",
   "b": "nagu",
   "routeKey": "pargasNagu", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Lillmälö (Pargas) ↔ Prostvik (Nagu)",
   "operator": "Finferries (bac routier / maantielautta)",
   "durationH": 0.25,
   "distanceKm": 2,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "EUR",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.finferries.fi/en/",
   "date": "2026-09-16",
   "note": "Bac routier public faisant partie de la route : gratuit pour tous. Durée/distance : ordres de grandeur."
  },
  {
   "a": "korpo",
   "b": "nagu",
   "routeKey": "naguKorpo", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Pärnäs (Nagu) ↔ Retais (Korpo)",
   "operator": "Finferries (bac routier / maantielautta)",
   "durationH": 0.15,
   "distanceKm": 1,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "EUR",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.finferries.fi/en/",
   "date": "2026-09-16",
   "note": "Bac routier public faisant partie de la route : gratuit pour tous. Durée/distance : ordres de grandeur."
  },
  {
   "a": "houtskar",
   "b": "korpo",
   "routeKey": "korpoHoutskar", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Galtby (Korpo) ↔ Kittuis (Houtskär)",
   "operator": "Finferries (bac routier / maantielautta)",
   "durationH": 0.5,
   "distanceKm": 6,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "EUR",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.finferries.fi/en/",
   "date": "2026-09-16",
   "note": "Bac routier public faisant partie de la route : gratuit pour tous. Durée/distance : ordres de grandeur."
  },
  {
   "a": "continental",
   "b": "inio",
   "routeKey": "kustaviInio", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Heponiemi (Kustavi) ↔ Kannvik (Iniö)",
   "operator": "Finferries (bac routier / maantielautta)",
   "durationH": 0.3,
   "distanceKm": 4,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "EUR",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.finferries.fi/en/",
   "date": "2026-09-16",
   "note": "Bac routier public faisant partie de la route : gratuit pour tous. Durée/distance : ordres de grandeur."
  },
  {
   "a": "continental",
   "b": "velkua",
   "routeKey": "palvaVelkua", "priceCovers": "vehicleAndOccupants", "coversSource": "traversée gratuite pour tous", "durationEstimated": true,
   "name": "Palva ↔ Velkuanmaa",
   "operator": "Finferries (bac routier / maantielautta)",
   "durationH": 0.15,
   "distanceKm": 1,
   "priceByClass": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "currency": "EUR",
   "original": {
    "1": 0,
    "2": 0,
    "5": 0,
    "foot": 0
   },
   "source": "https://www.finferries.fi/en/",
   "date": "2026-09-16",
   "note": "Bac routier public faisant partie de la route : gratuit pour tous. Durée/distance : ordres de grandeur."
  },
  {
   "a": "houtskar",
   "b": "inio",
   "routeKey": "mossalaDalen", "durationEstimated": true,
   "name": "Mossala (Houtskär) ↔ Dalen (Iniö)",
   "operator": "Skärgårdsvägen (liaison subventionnée)",
   "durationH": 1,
   "distanceKm": 10,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://elinvoimakeskus.fi/yhteysalusliikenne",
   "date": "2026-09-16",
   "note": "Liaison saisonnière de la « Skärgårdens Ringväg » : gratuite seulement pour les résidents de Houtskär et d’Iniö, les autres paient un tarif fixé par l’exploitant, non publié de façon lisible."
  },
  {
   "a": "continental",
   "b": "hitis",
   "routeKey": "kasnasRosala", "durationEstimated": true,
   "name": "Kasnäs ↔ Rosala/Hitis",
   "operator": "Finferries (navire de liaison)",
   "durationH": 0.75,
   "distanceKm": 12,
   "priceByClass": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "priceStatus": "unknown",
   "currency": "EUR",
   "original": {
    "1": null,
    "2": null,
    "5": null,
    "foot": null
   },
   "source": "https://elinvoimakeskus.fi/yhteysalusliikenne",
   "date": "2026-09-16",
   "note": "Navire de liaison prenant des voitures ; passagers gratuits, tarif voiture fixé à la réservation (booking.finferries.fi), sans grille lisible."
  }
 ]
};
