// Alias multilingues des 24 territoires d'OCÉANIE (build-oceanie-communes.js). Même méthode que build-golfe-aliases.js :
// rapprochement nom + coordonnées à 4 décimales sur le fichier publié. GeoNames range les noms en translittération
// latine ; alias utiles surtout pour les noms en langues locales (māori, samoan, fidjien…) et en français.
const fs = require('fs');
const path = require('path');

const COUNTRIES = ['AU', 'NZ', 'PG', 'SB', 'VU', 'FJ', 'WS', 'TO', 'TV', 'KI', 'NR', 'MH', 'FM', 'PW', 'NU', 'CK', 'TK', 'GU', 'MP', 'AS', 'UM', 'PN', 'NF', 'HM'];
const KEEP_FEATURE_CODES = new Set(['PPL','PPLA','PPLA2','PPLA3','PPLA4','PPLA5','PPLC','PPLF','PPLG','PPLL','PPLS']);
function norm(s){ return String(s || '').trim().toLowerCase().replace(/\s+/g, ' '); }

// Langues retenues pour la RECHERCHE : langues des 24 territoires (anglais, māori, langues polynésiennes, micronésiennes
// et mélanésiennes, hindi fidjien, chamorro…), plus les grandes langues déjà gérées (visiteurs asiatiques et européens).
// HM (îles, classe T) n'a pas d'alias : l'étape 2 ne relit que les lieux habités.
const SUPPORTED_LANGS = new Set([
  'fr', 'en', 'de', 'it', 'es', 'pt', 'nl', 'ru', 'zh', 'zh-Hans', 'zh-Hant', 'ja', 'ko', 'hi', 'id', 'ms', 'tl', 'fil',
  'mi', 'sm', 'to', 'fj', 'hif', 'bi', 'tpi', 'ho', 'pis', 'tvl', 'gil', 'na', 'mh', 'ch', 'cal', 'pau', 'chk', 'pon', 'kos', 'yap',
  'niu', 'rar', 'tkl', 'pih', 'ty', 'haw'
]);
