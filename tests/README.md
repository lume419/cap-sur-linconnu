...
| `npm test` | tout sauf les générateurs, tailles par défaut | ~7 min (+ 2 à 5 min si le serveur doit reconstruire son index de recherche) |
...
Durées mesurées (`npm test`) : i18n < 1 s ; péages ~40 s ; invariants ~2,5 min ; performances ~2 min ; serveur ~1 min de démarrage + ~1 min de tests ; générateurs ~35 s.
...
- `toll.test.js` : pays sans barème jamais facturés (intérieur et transfrontalier), transit par l'Alsace (DE, CH) et autour de la Bosnie (HR↔HR), 38 liaisons françaises de référence (médiane dans [0,85 ; 1,15]), témoins facturés / non facturés.
...
- `helpers/` : chargement du moteur avec accès à ses fonctions internes (compilation en mémoire, sans modifier `lib/`) et vérificateur d'invariants, client HTTP brut, `fetch` simulé, démarrage du serveur, extraction du texte d'un PDF.
