...
Durées mesurées (`npm test`, relevé du 18/09/2026) : i18n < 1 s ; péages ~40 s ; invariants ~2,5 min ; performances ~2 min ; serveur ~1 min de démarrage + ~1 min de tests ; générateurs ~35 s. Ajouté le 20/09/2026 : données ~45 s (~20 s de plus qu'au 16e audit, bornes de données) ; recherche ~35 s.
...
avec source déclarée et pays réellement à péage — jusqu'ici seule la France était ancrée. Chaque borne a été prouvée
par mutation. Réparé au passage : la boucle des alias de fusion codait « `ja;` » en dur (elle ne pouvait donc rien
